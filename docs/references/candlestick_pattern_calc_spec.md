...
- **`Body_t = 0`인 봉** (시가와 종가가 같은 봉). 실체를 분모로 쓰는 그림자 배수가 정의되지
  않는다. **그림자 배수 비교는 그림자가 양수이면 참으로, 그림자도 0이면 거짓으로 본다.**
  곧 `LS_t ≥ m · Body_t`는 `Body_t = 0`일 때 `LS_t > 0`이면 참이다. 이 규약이 있어야 실체가
  0인 잠자리형 도지가 Takuri와 Hammer의 모양 요건을 통과한다. 근거는 §2.7에 적었다.
...
배수가 2가 아닌 자리가 셋 있고 모두 원전이 그 값을 준다. Takuri는 아래그림자가 실체의 **세
배 이상**이고(Morris 3장, "at least three times the length of the body"), Shooting Star는
위그림자가 실체의 **세 배 이상**이며, Inverted Hammer는 위그림자가 실체의 **두 배 이하**다
(둘 다 Morris 3장). 이 배수들은 §7의 해당 절에 그대로 적었고 여기서 다시 정하지 않는다.
...
(§2.4)가 정의되지 않는다. **그림자 배수 비교는 그림자가 양수이면 참으로 본다.** 곧 실체가
0인 봉의 아래그림자가 양수이면 "아래그림자가 실체의 두 배 이상"과 "세 배 이상"이 모두
참이다. 근거는 이 규약이 없으면 잠자리형 도지가 Takuri의 모양 요건을 통과하지 못하는데,
Morris 2장이 Takuri를 **Tonbo(잠자리형 도지)의 한 갈래**로 정의하므로 원전과 정면으로
어긋나기 때문이다. 그림자도 0이면 비교는 거짓이다.
...
| `<name>_confirm` | **확인.** 원전이 확인을 요구하는 패턴에서 확인까지 끝났는가 | `1.0` 확인됨, `0.0` 미확인 또는 불성립, `NaN` 판정 불가 |
...
4. `US_t ≤ 2.0 · Body_t` — **【원전】** Morris 규칙 3 "usually no more than two times as
   long as the body". **이것은 상한이지 하한이 아니다.** 등호를 허용한다.
5. **갭을 요구하지 않는다.** Morris 규칙 2가 "No gap down is required"라고 명시한다.

**출력.** `pat_inverted_hammer` = 1.0. `_dir` = **+1.0**. `_strength` = 1.0.
`_confirm` = 다음 봉 종가가 이 봉의 실체 상단(`BodyTop_t`)보다 높으면 1.0 (§5.5의 원전
지정 자리, 기한 1봉).
**`min_history`** = 10.
**우리가 정한 것.** 짧은 실체 임계와 "아래쪽 부분"을 §2.5로 옮긴 것이다. 위꼬리 상한 2와
확인의 내용은 원전이 주었다.
...
4. `Doji(t)` — §2.3.
5. 도지 날의 그림자가 지나치게 길지 않다. 곧 `¬LongUpperShadow(t) ∧ ¬LongLowerShadow(t)`
   — §2.4.
...
`_confirm` = 다음 봉 종가 조건.
**`min_history`** = 11.
**우리가 정한 것.** 긴 실체 임계, 도지 허용오차, 그리고 **규칙 5의 "지나치게 길지 않다"를
§2.4의 긴 그림자의 부정으로 옮긴 것**이다. Morris 규칙 4는 "should not be excessively long,
especially in the bullish case"라고만 적고 숫자를 주지 않으며 강세형에만 강조를 두는데,
**이 표준은 그 비대칭을 따르지 않고 양쪽에 같은 기준을 적용한다.** 근거는 비대칭의 근거가
원전에 없고 "especially"가 요건이 아니라 강조이기 때문이다.
...
**출력.** `pat_matching_low` = 1.0. `_dir` = **+1.0**. `_strength` = 1.0. `_confirm` = 다음
봉 종가 조건.
**`min_history`** = 11.
...
3. `Doji(t−1)` **그리고** `GapDnBody(t−2, t−1)` **그리고** 도지의 그림자가 지나치게 길지
   않다(7.3.4 규칙 5와 같다).
4. `White_t`.
...
1. `DownTrend(t)`.
2. `Black_{t−2}` **그리고** `LongBody(t−2)` **그리고** `LongLowerShadow(t−2)` — 첫날은
   아래꼬리가 긴 긴 음봉이며 해머와 비슷한 모양이다.
3. `Black_{t−1}` **그리고** `LongLowerShadow(t−1)` **그리고** `Body_{t−1} < Body_{t−2}`
   **그리고** `L_{t−1} > L_{t−2}` — 둘째 날은 첫날과 같은 모양이되 **더 작고** 저가가 앞날
   저가보다 **위**에 있다.
4. `Black_t` **그리고** `ShortBody(t)` **그리고** `NoUpperShadow(t)` **그리고**
   `NoLowerShadow(t)` — 셋째 날은 **작은 Black Marubozu**다.
5. `L_{t−1} ≤ L_t` **그리고** `H_t ≤ H_{t−1}` — 셋째 날이 앞날의 **고저 범위 안에서** 열고
   닫는다.

**출력.** `pat_three_stars_in_the_south` = 1.0. `_dir` = **+1.0**. `_strength` = 1.0.
`_confirm` = 다음 봉 종가 조건. **`min_history`** = 12.
**우리가 정한 것.** 넷이다. 긴 실체와 짧은 실체와 긴 그림자와 무꼬리의 임계, **"더 작다"를
실체 크기의 엄격 비교로 옮긴 것**, Marubozu 판정, 그리고 **규칙 5의 "range" 해석**이다.
...
Shooting Star 3배, Inverted Hammer 2배 상한), Hikkake의 3봉 기한, Hanging Man과 Inverted
Hammer의 확인 내용이다.** 나머지는 모두 우리가 골랐고 각 자리에 근거를 적었다.
