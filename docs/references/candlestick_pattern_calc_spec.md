...
- **`Body_t = 0`인 봉** (시가와 종가가 같은 봉). 실체를 분모로 쓰는 그림자 배수가 정의되지
  ...
  **두 방향을 가르지 않으면 표준이 깨진다.** 하한만 보고 "그림자가 양수이면 참"이라고
  통일하면, 상한을 쓰는 §7.1.9 Inverted Hammer의 규칙 3(`US_t ≤ 2.0 · Body_t`)이 실체가
  0인 봉에서 언제나 참이 되어 **위그림자가 아무리 긴 묘비형 도지도 Inverted Hammer의 모양
  요건을 통과한다.** 그것은 §7.1.5 Gravestone Doji가 별도 패턴으로 있는 것과 충돌한다.
  하한 쪽 규약은 실체가 0인 잠자리형 도지가 Takuri와 Hammer의 모양 요건을 통과하게 하려고
  둔 것이며, 그 의도는 상한에 그대로 옮겨지지 않는다. 근거는 §2.7에 적었다.
...
- **상한 비교에서는 거짓이다.** 곧 실체가 0인 봉의 위그림자가 양수이면 "위그림자가 실체의
  두 배 이하"는 거짓이다. 이 규약이 없으면 묘비형 도지가 §7.1.9 Inverted Hammer를 통과해
  §7.1.5 Gravestone Doji와 충돌한다.
...
- **그와 별도로, 패턴은 판정에 들어가기 전에 자기 창의 모든 봉을 검사해 퇴화 봉이 하나라도
  ...
  쓰는 규칙(예: §7.3.4 Doji Star 규칙 5의 `¬LongUpperShadow`)에서는 거짓 반환이 오히려
  조건을 만족시켜 버리기 때문이다.
