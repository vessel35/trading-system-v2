...
계약이다. **판정 규칙은 이 문서에 옮겨 적지 않는다.** TA-Lib v0.7.1의 C 소스가
...
- **전체 허용 raw**: §5.2의 `0`, `±80`, `±100`, `±200`. 어댑터 전체 계약이다.
...
- **22000봉 관측 비영 raw**: 일곱 국면 22000봉 포획에서 실제 관측된 비영 값이다.
  표본 결과이며 계약이 아니다. `-`는 그 표본에서 관측되지 않았다는 뜻일 뿐이다.
...
- `2.0.0`: 수작업 원전 기반 규칙을 걷어내고 TA-Lib 동등성으로 공개 계산 계약을
  바꾼 major cutover다.
