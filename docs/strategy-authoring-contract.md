...
파일 하나를 두고 발견 결과만 Factory와 runtime 조립에 주입한다. 정책 등록 표가 없다는
운영 가시성 문제는 별도이므로 이번 배치와 무관하게 남아 있다.
...
**정책 쪽에서 배포 결합 두 자리는 닫혔고 두 자리는 남아 있다.** Factory는 발견된 정책
mapping을 받아 클래스가 선언한 설정으로 생성하며, 등록 mode를 손으로 적은 상수는 없다.
남은 자리는 실행 설정의 자금관리 설정 union과 Engine이 정책에 넘길 변동성을 정책 id로
갈라 고르는 자리다(§5.3.2).
