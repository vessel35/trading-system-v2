...
**series를 하나도 선언하지 않을 수는 없다.** 선언이 비면 series 해석이 "적어도
하나는 풀려야 한다"며 실행을 거부한다. 캔들만 보고 판단하는 전략이라도 최소
하나는 선언해야 하며, **`min_history`만으로 확보 구간이 정해지는 경우는 지금
없다.**

**실제로 막히는 범위는 좁다.** Engine이 자금관리 정책의 요구를 전략 선언과
합친 뒤에 해석하는데, `manual` 정책이 `ATR(14)`을 실행 timeframe으로 요구하므로
**전략이 아무것도 선언하지 않아도 합친 목록이 비지 않는다.** 막히는 것은
`turtle` 정책뿐이며, 그 정책의 유일한 요구가 일봉 `N`이라 실행 timeframe
병합분이 없기 때문이다.

**빈 선언을 허용하는 것이 목표다(2026-08-08 결정).** 지금 제약은 `turtle`을
쓰는 가격행동 전략에게 **쓰지도 않을 series를 하나 선언하도록 강요하는데**,
§2.1은 선언한 series와 실제로 읽는 series가 일치할 것을 요구한다. **한 규칙을
지키려면 다른 규칙을 어겨야 하는 상태**이므로 제약 쪽을 푼다.

**고칠 자리는 셋이며 함께 고쳐야 한다.**

- series 해석이 빈 선언을 거부하지 않게 한다.
- 확보 구간을 정할 때 선언한 series의 warm-up을 모으는 `max`가 빈 목록에서
  예외를 내지 않고 0을 내게 한다.
- 증거 무결성 검사가 `INDICATOR_DEFINITION` 0건을 정상으로 보게 한다. **지금은
  0건이면 곧바로 실패로 판정하므로**, 앞의 둘만 고치면 실행은 되지만 모든 실행이
  무결성 검사에 걸린다.

셋 가운데 마지막을 빠뜨리면 증상이 선언과 무관한 곳에서 나타나므로 함께 적어
둔다.
...
**아직 없는 것은 schema version이다.** `config_schema_version`이 `"1.0.0"`
상수로 박혀 있어 해석 방식이 바뀌어도 값이 그대로다. **목표 상태로는** 과거
실행을 재생할 때 당시 schema version을 써야 하며 최신 기본값으로 조용히
재해석하면 안 된다.
