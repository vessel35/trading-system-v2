...
**전략 쪽은 끝났다.** `trading_plugins`가 §6.2의 두 패키지를 훑어 기본으로 갖춘
전략 위에 발견한 것을 더하며, 두 서비스가 그 레지스트리를 쓴다.

**전략 클래스는 `STRATEGY_ID`를 자기 자리에 선언해야 한다.** 부모에게서 물려받은
것은 선언으로 치지 않는다. 파일이 스스로 주장하지 않은 이름으로 배포되지 않게
하기 위해서다. 같은 id를 둘이 주장하거나 기본 제공 전략의 id를 가져가면 그것은
등록되지 않는다. **조용히 덮어쓰면 결과가 바뀌고도 흔적이 남지 않는다.**

**잘못된 파일 하나가 나머지를 막지 않는다.** 불러올 수 없는 파일은 그것만 빠지고
기록에 남으며, 나머지 전략과 기본 제공 전략은 그대로 쓸 수 있다. 빠진 전략을
실제로 요청하면 **이름을 대며 거부되므로** 조용히 사라지지는 않는다.

**여기에는 파일이 import 도중 프로세스를 끝내는 경우도 든다.** 보통의 예외만
막으면 `sys.exit`를 부른 파일 하나가 서비스를 그대로 내린다. 다만 **멈춰 버리는
...
**셋째가 가장 조심스러웠다.** 자금관리 설정을 두 형으로 정적으로 묶어 두면 mode를
늘릴 때마다 그 파일을 고쳐야 한다. **union은 발견된 정책에서 만들어 낸다.** 발견이
파일 기준이므로 목록이 **배포 시점에 정해지고 실행 중에 바뀌지 않으며**, 그래서
API 스키마가 데이터베이스 상태에 따라 흔들리지 않는다. 기본 제공 두 설정 모델은
...
**기본 제공 둘의 설정 모델은 손으로 쓴 그대로 둔다.** 배포된 정책만 그 클래스의
...
설정 모듈의 import 자체가 실패하고, 그러면 **기본 제공 정책의 검증과 OpenAPI
...
바깥인 실행 설정 조립 시점에** 드러난다. 거기서 터지면 정책 하나가 기본 제공
정책의 검증과 OpenAPI 문서까지 함께 죽인다.

**세워 보는 union은 최종 union이어야 한다.** 정책 하나를 기본 제공 하나와만 묶어
보면, 서로에 대해서만 어긋나는 정책 둘이 각자 통과한 뒤 실제 union에서 부딪힌다.
그래서 정책은 **기본 제공 둘과 앞서 받아들인 전부가 든 union**이 여전히 서고 여전히
자기를 설명할 때에만 받아들인다.
...
**대신 기본 제공 정책의 설정은 `mode` 값이 아니라 형으로 갈라 읽는다.** `mode`가
어떤 이름도 될 수 있으면 그 값만으로는 갈래가 정해지지 않으므로, 기본 제공 두 형에
속하는지를 확인한 뒤 그 설정을 읽는다. 이렇게 하면 셋을 모두 얻는다 — 배포 mode의
이름과 견줄 수 있고, 기본 제공 정책의 설정을 읽을 수 있으며, **배포 정책에 없는
설정을 조건 없이 읽는 것은 여전히 막힌다.**

**정책 하나를 만들다 실패해도 요청 전체가 죽지 않아야 한다.** 전략 목록을 만들 때
정책의 기본값을 얻으려고 정책을 생성하는데, 그것도 배포된 파일이 쓴 코드를 도는
일이다. 실패는 그 mode 하나에 가둬 기록하고, 이름만 돌려준다.

**아직 닫히지 않은 것이 하나 있다.** 그 생성이 여전히 요청마다 일어나므로, 멈추는
정책은 요청을 멈춘다. 예외를 가두는 것으로는 막지 못한다.

**기동 시점에 한 번 만들어 두는 것으로는 멈춤이 사라지지 않는다.** 요청 대신 기동이
멈출 뿐이다. 같은 thread에서 시간 제한 없이 도는 코드는 어디서 돌든 붙잡을 수 없다.
실제로 멈춤을 막으려면 **정책의 코드를 별도 프로세스에서 시간 제한과 함께 돌리거나,
배포 정책의 코드를 아예 부르지 않고 선언만으로 기본값을 얻는 방식**이어야 한다.
어느 쪽으로 갈지는 이 설계가 정하지 않으며, 손대는 범위가 달라 별도 변경으로 둔다.
...
- 기본 제공 둘이 아닌 정책을 선택하면 그 정책의 설정을 JSON으로 적는 입력을
...
- 스윕 축으로 자금관리 설정을 제시하는 것은 기본 제공 둘에 한한다. 배포된 정책의
설정 이름은 화면이 알지 못한다.
...
**제출도 mode마다 갈라야 한다.** 기본 제공 둘이 아닌 것을 전부 Turtle 설정으로
