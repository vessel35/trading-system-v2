# 전략 작성 및 자금 관리 정책 개발 규범 

해당 문서는 전략 작성 및 자금 관리 정책 개발 규범이다. 현재는  trading-system-v2 시스템에 알맞은 상태를 정의하며, 이 규범의 최종 목표는 trading-view 의 전략을 구현하는 것과 동일한 수준까지 발전시키는 것이 목표이며, trading-system-v2의 발전과 함께 진행되어야 한다. 

## 1. 문서 목적

이 문서는 `trading-system-v2`의 신규 전략 작성, 기존 전략 리팩터링, 자금관리 정책 개발 및 관련 코드 리뷰에 적용하는 규범 문서다. 전략 작성자는 이 문서를 따라야 하며, 플랫폼 구현자는 등록·설정 해석·실행·Evidence 단계에서 전략이 규범을 준수하는지 검증해야 한다.
...
아래는 어기면 **그 자리에서 거부되므로 외울 필요가 없다.** 규칙을 기억해서
지키는 것과 어기면 멈추는 것은 다르며, 이 목록에 있는 것은 뒤쪽이다.

- 기반 클래스를 상속하고 `get_metadata`·`get_parameter_schema`·`analyze`를
  채우지 않으면 인스턴스가 만들어지지 않는다.
- 레지스트리에 등록되지 않은 계열을 선언하면 해석 시점에 거부된다. 등록된 지표
  조합과 캔들 패턴 밖은 선언할 수 없다.
- 파라미터 스키마를 어기면 설정 해석이 거부한다. 모르는 이름도, 빠뜨린 필수
  항목도, 형이 맞지 않는 값도 통과하지 못한다.
- 선언하지 않은 자금관리 정책으로 실행하면 거부된다. 청산 신호를 내지 못하는
  전략은 Turtle 정책을 쓸 수 없다.
- 등록과 코드가 어긋나면 실행이 거부된다. 클래스 이름과 모듈 경로와 warm-up과
  지원 timeframe과 계열 선언을 대조하며, 비활성이거나 폐기된 전략도 막힌다.

### 2.1 아직 막히지 않아 스스로 지켜야 하는 것

아래는 규칙으로는 정해져 있으나 어겨도 자동으로 드러나지 않는다. **§9의 필수
테스트로 직접 확인해야 하며, 확인하지 않으면 아무도 대신 잡아 주지 않는다.**
...
- 세 메서드의 시그니처. 이름만 맞으면 통과하고 인자나 반환형이 어긋나도 실행
  중에야 드러난다.
- 전략 판과 파라미터 기본값과 프로파일. 등록 대조가 이 셋은 보지 않는다.

**이 목록은 줄어드는 것이 정상이다.** 공통 검사가 들어오면 여섯 성질이 위쪽
목록으로 옮겨 간다. 옮겨 갈 때 이 절을 함께 고친다.
