...
`MoneyManagementFactory`가 소유하며 Evidence에 기록된다. **받는 이름이나 기본값이나
범위가 바뀌면 이 판을 올린다.**

**판이 필요한 이유는 같은 설정 재실행 때문이다.** 저장되는 것은 사용자가 적은
원본이고 기본값은 해석할 때 채워진다. 그래서 기본값이 바뀌면 **같은 원본이 다른
정책이 되는데**, 화면은 "같은 설정 재실행인데 결과가 다르다"고만 말한다. 두 실행의
판이 다르면 **결과 차이가 코드에서 왔는지 설정을 다르게 읽은 데서 왔는지 갈린다.**

**해석은 두 층이 나눠 한다.** 실행 설정의 자금관리 모델이 기본값과 범위를 먼저
적용하고, 정규화된 결과가 `MoneyManagementFactory`에 간다. **어느 한 층만 바뀌어도
해석이 달라지므로 판은 두 층을 함께 가리킨다.** 한쪽만 고치고 판을 두면 검사가
막는다.

**옛 판으로 읽는 구현은 아직 없다.** `MoneyManagementFactory`는 최신 규칙 하나만
갖고 있고 판을 인자로 받지 않는다. 그러므로 지금은 **판이 서로 다르다는 것을 알 수
있을 뿐, 옛 판으로 다시 읽지는 못한다.**

**판을 두 번째로 올리는 변경은 과거 해석기 보존을 함께 가져와야 한다.** 판마다
해석기를 두고 기록된 판으로 골라 쓰지 않으면, 예전에 유효했던 설정이 오늘의
해석기에서 거부되어 **같은 설정 재실행 자체가 불가능해진다.**
