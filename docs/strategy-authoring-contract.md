...
**전략은 이미 이렇게 된다.** 정책은 아직 설정에서 만들어지는 자리가 남아 있으며
무엇이 남았는지는 §6.5에 있다.
...
**설정 모델은 만든 자리에서 스키마까지 뽑아 본다.** 스키마 생성은 미뤄지므로,
확인하지 않으면 표현할 수 없는 type이 원인에서 먼 곳에서 드러난다.
...
- 처음 값은 전략이 그 mode에 대해 밝힌 기본값에서 채운다. 다른 mode의 기본값은
필드 이름부터 다르므로 가져오지 않는다.
