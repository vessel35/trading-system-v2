...
1. **병행 적재 일치** — 구·신 수집기를 같은 구간에 동시 적재하고(병행 창 시작값 **최소 24시간 = 1분봉
   ...
   확정한 값이라 반올림 여지가 없다. 병행 창 동안 신 수집기는 비교 전용 위치에 적재한다(운영 테이블 이중
   적재는 "확정 캔들마다 1행" 불변식과 충돌한다).
2. **절체 연속성** — 절체 시각 전후로 1분당 1행이 유지되어야 한다(갭 0·중복 0).
...
| 부록.4 | 회귀는 전수 계약(wallet 약 1,279건) · 교정 표면 골든 재수립 규약(검증 로직 보존·차이 전건 표준 수식 설명·실측 대사 시작 기준은 자릿수 반올림 안 차이 0) · 지표 동등성 게이트(첫 대상 `adx_14`·수치+판단 두 겹) · 강제청산 수신 게이트(다섯 수용 케이스 — 신설 회계 경로의 검증) · 수집기 절체 게이트(병행 적재 전 컬럼 차이 0·절체 연속성·복귀 경로) · `fill_timing` 전환 재검증 트리거·기록 · 재복제 가드 |
