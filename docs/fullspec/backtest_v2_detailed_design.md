...
**클래스 정의서의 항목 표준.** 모든 클래스 정의서는 아래 여섯 항목을 이 순서로 쓰되, **해당하는 것만** 둔다
(억지로 "없음"을 채우지 않는다). 그래서 순수 모듈은 필드가, 독립 값 타입은 상속관계가 없을 수 있다.

- **개요** — 한 줄로 "무엇인가" (항상).
- **책임** — 무엇을 소유·수행하고 무엇을 하지 않는가(경계), 그리고 왜 이 설계인가 (항상).
- **상속관계** — 구현·실현·추상 관계(Protocol 구현, ABC 등). 그런 관계가 있을 때만.
- **필드** — 속성·상수의 잔여: 제약·기본값·NULL·검증. 속성이 있을 때만.
- **메서드** — 각 메서드·수식의 의미. 메서드가 있을 때만.
- **불변식** — 이 클래스가 강제·보존하는 불변식. 지킬 때만.
