...
**판단 트리거 변경의 실행-시점 영향(수용).** 분당 폴링을 없애면 판단이 전략 타임프레임 캔들 마감 직후로
고정되어, 현행 대비 신호 방출 시각이 최대 폴링 간격만큼 앞당겨진다. 국면 1에서는 체결이 아직 즉시(immediate)라
진입 시각·가격도 그만큼 이동한다. 판단 산출(같은 캔들 이력에서 같은 신호)은 골든이 보존을 확인하지만, 이 시점
이동은 손익에 닿는 실행상의 변화이며 **수용된 변화**다 — 체결 시점 **규칙** 자체를 바꾸는 것은 여전히
`fill_timing` 전환 하나뿐이다. 치환 후 첫 운영 구간은 신호 방출 시각을 모니터링해 이동 폭이 예상(폴링 간격
이내)과 맞는지 확인한다.
...
| wallet `application.services.paper_trading_service`·`sl_tp_monitor_service`(임포터 각 3곳·1곳, 테스트 제외 기준) | 위임 shim | `core_lib.execution.matcher`·트리거 판정 |
...
(우리가 낸 주문이 아님), ② reduce-only 성격의 체결이며(포지션을 줄임), ③ 거래소가 청산 주문에 부여하는
...
**`Trade.liquidation_penalty`의 라이브 계상 규칙(이중 차감 금지).** net 등식의 다른 항과 달리 이 항은 통지
필드로 직접 오지 않으므로 분류 규칙으로 확정한다. 거래소가 청산 체결에 보고하는 수수료성 금액 중 **일반 체결
수수료(taker)는 `Fill.fee`(→ `Trade.total_fee`)로만** 계상하고, 거래소가 **별도 항목으로 구분해 보고하는 청산
정리 수수료만** `liquidation_penalty`로 계상한다(그 항목 명세는 (a)의 식별자 집합과 함께 구현 시 거래소 명세로
확정). 구분 보고가 없으면 penalty는 **0**이다 — 강제청산의 손실 자체는 실측 체결가의 gross와 수수료에 이미
담기며, 0은 "언제나 0 이상" 제약과 정합한다. **같은 금액을 `total_fee`와 `liquidation_penalty` 두 곳에 넣는
것은 금지한다** — net 등식에서 같은 비용이 두 번 차감된다.
...
생성한다. 조회 창의 기준점은 **마지막으로 정상 처리한 주문 이벤트의 거래소 시각**(그 이후부터 현재까지)이다.
...
zeroing + CRITICAL 알림 경로로 수렴한다(수동 개입). **주기 대조** — 300초 주기 포지션 대조(`balance_sync_service.py`)가 "거래소에 없는 로컬 포지션"을
발견하면, 현행처럼 곧바로 zeroing하지 않고 **먼저** 같은 주문 이력 조회로 원인 청산 주문을 찾는다. 찾으면 (b)로
정상 회계 처리하고, 못 찾으면 현행대로 zeroing + CRITICAL 알림으로 남긴다(수동 개입 — 원인을 모른 채 추정으로
장부를 채우지 않는다).
...
| `est_inputs` | 추정 입력 — `entry`·`leverage`·`mmr` |
...
2. **차이 전건이 표준 수식으로 설명 가능해야 한다.** 옛 기대값과 새 값의 차이는 net 등식
   ...
   설명 불가 차이는 기대값 갱신이 아니라 **버그 후보**로 조사한다.
3. **최종 기준은 거래소 실측이고, 대사 기준은 다음과 같다.** 재수립된 골든의 수용 기준은 옛 코드와의 일치가
   ...
   - **실측 경로(라이브 기록 — 실측 체결가·수수료·펀딩 정산·실현 손익)** — 거래소 보고값을 같은 자릿수로
     반올림한 값과 **차이 0**. 모델 추정(슬리피지·청산가)은 회계 입력이 아니므로 이 대사에 끼지 않는다.
...
- **수치 게이트** — 각 지표의 `min_history` 워밍업 구간을 제외한 뒤, `|old| > 1e-8`인 점에서 상대 오차
  `|new − old| / |old| ≤ 1e-4`, 그 밖의 점에서 절대 오차 `≤ 1e-8`. 이 시작값은 보수 가정이며(옛 구현의 seed
  규약이 다를 수 있어 0이 아닌 여유를 둔다), 채택 시 실측 seed 감쇠를 보고 이 절의 값을 좁혀 확정·기록한다.
...
2. **절체 연속성** — 절체 시각 전후로 OHLCV는 1분당 1행, 펀딩 요율은 정산 간격당 1행이 유지되어야 한다
   (갭 0·중복 0).
3. **복귀 경로** — 게이트 실패 시 구 수집기로 되돌린다. 구 수집기는 절체와 동시에 폐기하지 않고 병행 창
   통과 후에만 은퇴시킨다.
...
5. **선-조회** — 주기 대조가 "거래소에 없는 로컬 포지션"을 발견했을 때, 원인 청산 주문이 있으면 정상 회계로,
...
**완료 확인 목록(채택 기록).** 채택은 다음이 전부 기록으로 남았을 때 완료다 — ① wallet 전수 회귀 통과(교정
표면 골든 재수립 내역 포함), ② 활성 전략 analyze 골든 통과 목록, ③ 지표 동등성 결과(첫 대상 `adx_14`·확정
허용오차), ④ 강제청산 수신 배선과 다섯 수용 케이스 통과(부록.3의 (a)~(d) + 위 수신 게이트), ⑤ `fill_timing` 전환 시각·커밋·재검증 결과,
⑥ 수집기 내부화 — 절체 게이트(병행 적재 일치·절체 연속성) 통과와 적재 불변식 확인, ⑦ 재복제 가드 통과, ⑧ 실거래 전환 시 core-lib 고정 버전과
`core_lib_version` 메타 일치.
...
| 부록.1·부록.2 | 프로덕션 접촉은 채택 절차뿐(설계 단계 미변경) · 채택은 무중단 re-export shim(로직 잔존 금지, 계약 변경 자리는 호출부 수정) · 표면별 게이트(보존=동등성·교정=실측 골든 재수립·트레일링 무접촉) · 실거래 전환은 core-lib 버전 고정 산출물(§1.2 거버넌스와 정합) |
| 부록.2 국면 2 | 라이브 지표 인소싱(사전계산 테이블 폐지·`core_lib.indicators` 증분 직접 계산·동등성 게이트) · 수집기 내부화(적재만·확정 캔들마다 1행 불변식) · 체결 시점 통일(`fill_timing` 기본값 전환으로 `decision_ts < execution_ts`) |
| 부록.3 | 강제청산 출처·수렴 계약 준수(거래소가 집행·우리는 수신 정리만, `Fill(exit_reason=LIQUIDATION)` 동일 모양·단일 회계 경로·`Trade.liquidated`·종결 상태 이벤트 1회 생성·부분 체결-종료 처리) · `liquidation_penalty` 라이브 계상 규칙(구분 보고만 계상·이중 차감 금지) · 누락 복구(재접속 catch-up·주기 대조 확장, 영속 워터마크·조회 창 기준·멱등) · 실측 대사(청산 근사의 보수성 실측 확인 — 구↔신 대사 제외와 별개, 기록은 독립 라이브 Evidence SQLite로 연구 데이터·운영 DB 분리 준수) · Decimal 단일 변환 관문(거래소 문자열 수치) |
| 부록.4 | 회귀는 전수 계약(wallet 약 1,279건) · 교정 표면 골든 재수립 규약(검증 로직 보존·차이 전건 표준 수식 설명·실측 대사 시작 기준은 자릿수 반올림 안 차이 0) · 지표 동등성 게이트(첫 대상 `adx_14`·수치+판단 두 겹) · 강제청산 수신 게이트(다섯 수용 케이스 — 신설 회계 경로의 검증) · 수집기 절체 게이트(OHLCV·펀딩 요율 병행 적재 전 컬럼 차이 0·절체 연속성·복귀 경로) · `fill_timing` 전환 재검증 트리거·기록 · 재복제 가드 |
| 부록.5 | 구↔신 백테스트 수치 대사 제외의 명문화(수용 기준 아님 — 세 사유) · 대체 자체 검증 기준선(결정성·look-ahead 무결성·골든·수식 골든·회계 실측 정확성·세 모드 동등성·통계 검증) |
