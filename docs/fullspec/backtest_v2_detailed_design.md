...
전략은 레지스트리 등록만 하고 Adaptee 구현을 뒤로 미룰 수 있다.
...
| signal | 전략 구동 | `StrategyFactory.create_from_db`(`application/scheduler/strategy_executor.py` 464행)·`registry.py` 수동 등록(`main.py` 84~99행, 17개 직수입) | `Adapter Manager.create`(생성) + `StrategyConfig.resolve`(파라미터 해석) + `StrategyRegistry`(live) 어댑터 — 전략 목록은 `signal_db`의 재정의된 레지스트리 테이블(§5.1.3) | 보존 — 활성 전략 analyze 골든 |
...
| wallet `application.services.paper_trading_service`·`sl_tp_monitor_service`(임포터 각 3곳·2곳) | 위임 shim | `core_lib.execution.matcher`·트리거 판정 |
...
거래에는 `Trade.liquidated = TRUE`·`liquidation_penalty ≥ 0`이 남는다(§4.1.1). **멱등성** — 같은 청산 주문
id를 두 번 처리하지 않도록 기록 전 중복 검사를 둔다(현행 `exists_by_exit_order_id` 패턴, `binance_order_event_handler.py`
521행의 계승).
...
생성한다. **주기 대조** — 300초 주기 포지션 대조(`balance_sync_service.py`)가 "거래소에 없는 로컬 포지션"을
...
last-price 근사, mmr 기본 0.4%). 기록처는 라이브 Evidence(연구 피드백 경로 — §1.2)다. 용도는 백테스트 청산
근사가 **보수 방향**(청산 과대 검출)임을 실측으로 계속 확인하고, mmr 티어·모델 보정의 입력을 쌓는 것이다.
이 대사는 **거래소 실측과의 대조**이며, 부록.5가 제외하는 구↔신 백테스트 수치 대사와는 별개다 — 제외되지 않는다.
...
3. **최종 기준은 거래소 실측이다.** 재수립된 골든의 수용 기준은 옛 코드와의 일치가 아니라 거래소 실측 대비
   정확성(정의된 허용 오차 내 대사)이다 — 라이브 손익 불일치의 재발을 여기서 막는다.
...
바꾸는 것은 채택 전체에서 유일한 **의도된 프로덕션 동작 변경**이다(`decision_ts < execution_ts` 통일). 규약:
...
flowchart TD
    ...
    F1 --> A
    G1 -->|통과| B["국면 1 — signal 전략 구동 치환<br/>(선행: 활성 전략 Adaptee + analyze 골든)"]
...
허용오차), ④ 강제청산 수신 절차 배선 확인(부록.3의 (a)~(d)), ⑤ `fill_timing` 전환 시각·커밋·재검증 결과,
...
| 회계 실측 정확성 | 재구현 회계·손익·체결이 현실과 맞는다 | 거래소 실측 대비 정의된 허용 오차 내 대사(부록.4 골든 재수립의 최종 기준) |
...
| 부록.3 | 강제청산 출처·수렴 계약 준수(거래소가 집행·우리는 수신 정리만, `Fill(exit_reason=LIQUIDATION)` 동일 모양·단일 회계 경로·`Trade.liquidated`) · 누락 복구(재접속 catch-up·주기 대조 확장, 멱등) · 실측 대사(청산 근사의 보수성 실측 확인 — 구↔신 대사 제외와 별개) · Decimal 단일 변환 관문(거래소 문자열 수치) |
| 부록.4 | 회귀는 전수 계약(wallet 약 1,279건) · 교정 표면 골든 재수립 규약(검증 로직 보존·차이 전건 표준 수식 설명·최종 기준은 거래소 실측) · 지표 동등성 게이트(첫 대상 `adx_14`·수치+판단 두 겹) · `fill_timing` 전환 재검증 트리거·기록 · 재복제 가드 |
