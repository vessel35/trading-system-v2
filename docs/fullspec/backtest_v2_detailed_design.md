...
**실행 경로의 명시적 설계 차이.** 벡터화 방식을 백테스트 실행의 기본으로 두었던 원안과 달리, 실행은 증분
방식만 쓴다. 전 구간 벡터화를 실행에 넣으면 시각별로 `close_time ≤ 판단 시각`을 지키는 일이 입력 구조 자체가
아니라 호출자가 미래 구간을 잘라 주는 규율에 의존한다. 증분 상태는 확정 캔들을 한 칸씩만 받아 미래 캔들을
구조적으로 볼 수 없으므로 look-ahead 배제 수준이 더 강하다. 벡터화 방식은 없애지 않고 연구용 일괄 계산과
증분 방식의 독립 일치 검증 기준으로 유지한다.
