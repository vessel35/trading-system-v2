...
- **책임** — 주문 하나의 신원(무엇을·얼마나·어느 방향으로)과 그 주문이 지금 어느 단계에 있는지를 소유한다.
  상태를 바꾸는 유일한 통로가 되어, 허용되지 않는 전이를 거부한다. 상태 값 자체의 뜻은 아래 `OrderStatus`가
  정의한다.
- **왜 State Machine으로 구현하는가** — 주문의 생애는 되돌릴 수 없는데, 그것을 강제할 장치가 없으면 장부가 조용히
  오염되기 때문이다. 체결 사건은 중복되거나 순서가 뒤바뀐 채 도착할 수 있다 — 라이브에서는 거래소 이벤트가 재전송·
  재접속 재생으로 같은 체결을 두 번 보낼 수 있다. 상태를 아무렇게나 바꿀 수 있게 두면 이미 끝난 주문에 체결이 한 번
  더 반영되고, 그 순간 회계 항등식(`cash + position = equity`)이 깨지고 비용이 두 번 차감되며 손익이 틀어진다.
  게다가 이런 오류는 예외를 던지지 않아 **눈에 띄지 않는다.** 허용 전이를 표로 못박아 두면 불가능한 전이가 조용한
  오염이 아니라 **명시적 거부**가 되고, 같은 사건 열은 언제나 같은 종료 상태를 낸다(결정성). 백테스트의 매처가 만든
  체결이든 라이브의 거래소 이벤트든 같은 표를 지나므로, 두 환경에서 주문의 생애가 똑같이 흐른다.
- **`VALID_TRANSITIONS`의 단일 소유** — 이 표는 `types.Order`만 갖는다. `execution.OrderLifecycle`은 표를 복제하지
  않고 이를 읽어 쓴다(§4.3.1). 두 곳에 두면 한쪽만 고쳐져 전이 규칙이 갈린다.
- **메서드 의미**
...
체결 사실을 명시하는 타입. 현행에 대응 타입이 없어 새로 만든다.
...
체결 완료된 거래 한 건. 현행 `ClosedTrade`가 갖고 있던 필드를 그대로 가져오고, 최초 위험 `r0`를 새 필드로
더한다(상속이 아니라 필드를 옮겨 온 것이다).
