...
한 지표(또는 DMI/ADX처럼 여러 값을 한 묶음으로 내는 지표 시스템 하나)를 **어떻게 계산하고 언제부터 유효한지
선언하는 명세**다. `registry`에 등록되는 단위가 바로 이 명세이며, 등록된 지표 하나에 `IndicatorSpec` 하나가
대응한다. 계산에 필요한 모든 것 — 식별자, 고정할 구현, 워밍업 요건, 두 계산 방식의 진입점 — 을 담되 **실행 중
상태는 갖지 않는다.** 캔들이 흐르며 변하는 값(직전 EMA, 워밍업 완료 여부 등)은 `IndicatorState`가 따로 보유한다.
즉 명세는 "이 지표를 무엇으로·어떻게 계산하는가"를 적은 불변 서술이고, 실제 계산과 상태 전진은 명세가 내주는
벡터화 함수와 `IndicatorState`가 맡는다.

- **책임** — `name`·`params`로 지표를 식별하고, 벡터화·증분 두 계산 방식의 진입점을 제공하며, 유효 시작 시점
  (`min_history`)과 채택 구현(`version`·`pinned_impl`)을 고정한다.
- **필드**
