...
전략 작성과 자금관리 변경의 규범은 `docs/strategy-authoring-contract.md`가 단독으로 소유한다. 이 문서는 그
규칙을 다시 정의하지 않고, 현재 코드의 클래스 관계·책임 배치·실행면별 차이만 as-built로 보인다.
...
10. **생존 사이징.** 거래당 위험은 계좌의 1% 이하(`1R ≤ 1%`)이며, 엣지는 진입 신호에서 온다(손절·익절 배치로
    ...
    표시한다.
11. **결정성.** 같은 설정 입력·같은 원천 데이터·같은 seed는 언제나 같은 Evidence를 낸다. 결정성 검증 해시는
    SQLite 파일 바이트가 아니라 정렬된 행의 정규화 직렬화(wall-clock 제외)로 산출하며, 설정이 같아도 원천 데이터
    해시가 다르면 재현 대조 대상에서 제외한다.
...
현재 두 실행 능력은 예약 상태다. `core_lib.strategy.trailing`에는 docstring뿐이고 matcher는 trailing 주문을
`NotImplementedError`로 거부한다. `trigger_feed` 열거형에는 `m1_subcandle`이 있으나 `RunConfig`와 Engine이 이를
`NotImplementedError`로 거부한다. 따라서 현재 집행 판정은 전략 timeframe 캔들 수준의 보수 판정이며 손절·익절
동시 도달 시 손절 우선이다.
...
| §4 | 클래스 다이어그램 + 정의서 (컴포넌트별) | §4.1 타입·지표·패턴·계열 해석 · §4.2 전략(+config 시퀀스)과 자금관리(+실행면별 조합) · §4.3 실행·평가(+판정 플로우) · §4.4 Engine(+캔들 루프·집행 시퀀스) · §4.5 출력(+run 저장 시퀀스) | 이 문서 · as-built |
...
| 부록 | 채택·회귀 절차와 개정 이력 | 채택 지점·이행 순서·shim · 라이브 강제청산 수신 절차 · 회귀 범위·게이트 · 검증 기준선(구↔신 수치 대사 제외) · 비밀 저장 방식 변경 · 전략 플랫폼 as-built 갱신 이력 | 이 문서 · 확정 |
...
그래서 §3.1은 core-lib 컴포넌트 열세 종을, §3.2는 Engine·ConfigLayer·Harness에 일곱 포트 어댑터를 더해 그린다.
...
services/core-lib/
  ...
  core_lib/
    ...
    patterns/                        # [컴포넌트] TA-Lib 캔들 패턴 명세·증분 상태·별도 레지스트리
      ...
      talib_single_candle.py         #   단일 캔들 패턴 포트
      talib_two_candle.py            #   두 캔들 패턴 포트
      talib_three_candle.py          #   세 캔들 패턴 포트
      talib_multi_candle.py          #   복수 캔들 패턴 포트
...
flowchart TD
    ...
    IND["indicators"]
    PAT["patterns"]
    subgraph SERIES["series 계열 선택 컴포넌트"]
        SER["series<br/>공통 Protocol"]
...
flowchart TD
    subgraph CORELIB["core-lib (설치형 공유 패키지 · import core_lib)"]
        ...
        PAT["patterns<br/>캔들 패턴·별도 레지스트리"]
        subgraph SERIES["series 계열 선택 컴포넌트"]
            SER["series<br/>공통 소비 Protocol"]
            RES["series_resolution<br/>레지스트리 선택·실행 열쇠"]
        end
...
| ------------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ | --------------------------- |
| DataFeed 구현         | `DataFeed`         | 과거 확정 OHLCV·funding·mark_price를 timeframe별로 공급하고 `up_to` 경계 이후 캔들을 노출하지 않는다. Engine은 전략 TF, funding 근거 1분봉, turtle 정책의 별도 일봉을 읽지만 캔들 내 trigger에는 현재 전략 TF만 사용한다. | `ports.DataFeed`·`types.Candle` | `crypto_data` 읽기(백테스트 미기록) |
...
- **유보·기타** — wall-clock 즉시 체결(`filled_at = now()`)은 `Clock` 포트·`fill_timing`으로 대체. 구 import 경로엔
  re-export shim이 있다. 트레일링 열거형은 남아 있지만 계산 구현이 없고 matcher가 거부한다.
...
값 타입의 단일 정의처다. 다이어그램은 목표 판단 `DecisionIntent`, legacy 및 내부 구체화 경계 `TradingSignal`을
포함한 값 타입 여덟(`Candle`·`DecisionIntent`·`TradingSignal`·`Order`·`OrderRequest`·`Fill`·`Position`·`Trade`),
금액 정밀도 유틸리티 `money`, ENUM 아홉과 그 관계를 담는다.

**타입마다 수치 정밀도가 갈리는 이유.** 값이 어느 경로에 놓이느냐가 그 타입의 정밀도를 정한다.

- **판단 경로는 `float`** — 지표를 계산하고 전략이 판단하는 길이다. 값이 많고 자주 계산되며 끝자리 오차가 판단을
  뒤집지 않으므로 빠른 `float`를 쓴다. `Candle`·`DecisionIntent`·`TradingSignal`과, 체결 관문 이전의 float 주문 요청
...
classDiagram
    ...
    class TradingSignal {
        <<legacy and internal materialization>>
        +str symbol
        +datetime timestamp
        +float confidence
        +float price
...
- **개요** — 전략이 무엇을 할지 명시적으로 반환하는 목표 판단 타입.
- **책임** — `ENTER_LONG`·`ENTER_SHORT`·`EXIT`·`HOLD` 행동, 판단 기준 가격·시각·근거와 메타데이터만 담는다.
  `stop_loss`·`take_profit`·`quantity`·`leverage`·`market_type`·계좌 상태는 담지 않는다.
- **필드** — `timestamp`는 timezone-aware여야 하고 `reference_price`는 유한한 양의 float, `confidence`는 유한한
  0 이상 1 이하의 float다. `metadata`는 생성 시 읽기 전용 매핑으로 동결된다.
- **불변식** — 방향은 보호가격의 상대 위치가 아니라 `DecisionAction`으로 명시된다. 현재 `VesselReference`가 이
  타입을 반환하며, 진입 결정은 자금관리 정책의 입력이 된다.
...
- **책임** — 판단 근거와 이미 정해진 보호 수준·시장 종류·요청 leverage를 담는다. 주문 방향(`OrderSide`)·수량
  필드는 갖지 않으며, 방향과 행동은 legacy 호환 경계에서 실행 드라이버가 아래 규칙으로 도출한다.
    - **도출 입력** — core-lib의 `TradingSignal`에는 `signal_type`이 없다. signal-service는 보호 수준의 유무·기하와
      `analyze`에 넘긴 `current_position` 문맥으로 `SignalIntent`·`PositionSide`를 도출한 뒤, 서비스 지속 값인
      `PersistedSignal.signal_type`을 정한다. `DecisionIntent`에서 구체화한 신호에는 원래 행동을
      `metadata.decision_action`으로 함께 남긴다. 아래 세 갈래는 서로 겹치지 않는다.
        - **관망(HOLD)** — `analyze()`가 `None` 또는 `DecisionAction.HOLD`를 반환하면 아무 행동도 하지 않는다.
...
- **상속관계** — 없음(독립 값 타입).
- **필드** — `price`는 실행가가 아니라 판단 기준가이며 실제 체결가는 다음 캔들 시가에서 정해진다.
  `stop_loss`·`take_profit`은 legacy 전략이 제안했거나 자금관리 정책이 만든 보호가격으로 NULL을 허용하고,
  `leverage`는 legacy 입력 또는 정책 요청값이며 `confidence`는 0~1이다.
...
- `TRAILING_STOP_MARKET` — 열거형 값은 남아 있지만 matcher가 `NotImplementedError`로 거부하는 예약 주문이다.
...
- `STOP_LOSS` — 손절가 도달.
- `TAKE_PROFIT` — 목표가 도달.
- `TRAILING_STOP` — 열거형 값은 존재하지만 현재 matcher가 trailing 주문을 거부하므로 생성되지 않는 예약 사유다.
...
일치해야 하며, 이를 일치 테스트로 못박는다.

**실행 경로의 명시적 설계 차이(발주자 확정 2026-07-24).** 벡터화 방식을 백테스트 실행의 기본으로 두었던
원안과 달리, 실행은 증분 방식만 쓴다. 전 구간 벡터화를 실행에 넣으면 시각별로 `close_time ≤ 판단 시각`을
지키는 일이 입력 구조 자체가 아니라 호출자가 미래 구간을 잘라 주는 규율에 의존한다. 증분 상태는 확정 캔들을
한 칸씩만 받아 미래 캔들을 구조적으로 볼 수 없으므로 look-ahead 배제 수준이 더 강하다. 벡터화 방식은 없애지
않고 연구용 일괄 계산과 증분 방식의 독립 일치 검증 기준으로 유지한다. 이 설계 차이는 look-ahead 구조적 배제
...
classDiagram
    ...
    class IndicatorRegistry {
        ...
        +resolve_specs(mode, declared, explicit) list~IndicatorSpec~
    }
...
classDiagram
    ...
    class PatternRegistry {
        ...
        +list() list~PatternSpec~
        +names() set~str~
        +specs_for(enabled_set: Collection) list~PatternSpec~
        +specs_from_descriptors(descriptors: Collection) list~PatternSpec~
        +resolve_enabled(mode: str, declared: Collection, explicit: Collection) set~str~
        +resolve_specs(mode: str, declared: Collection, explicit: Collection) list~PatternSpec~
    ...
    class SeriesResolution["core_lib.series_resolution"] {
        ...
        +series_specs_from_descriptors(descriptors, indicators, patterns) list~SeriesSpec~
        +resolve_series_specs(mode, declared, explicit, indicators, patterns) list~SeriesSpec~
    }
...
- **메서드**
    - `compute_vectorized` : 확정된 전 구간을 한 번에 계산해 시계열을 돌려준다. 연구용 일괄 계산과 증분 방식의
      독립 일치 검증에 쓰며 실행 캔들 루프에는 넣지 않는다.
    - `make_state` : 이 지표의 증분 상태 객체(`IndicatorState`)를 만들어 준다. 백테스트·라이브·페이퍼가 캔들마다
      O(1)로 갱신하는 증분 방식의 진입점이다.
...
- **책임** — 등록된 명세를 조회하고, 한 run에서 실제로 계산할 지표 집합을 한 곳에서 확정하며, 연구·일치 검증용
  벡터화 계산을 제공한다. `auto`·`explicit`·`all`의 해석과 외부 이름·파라미터를 등록 명세로 바꾸는 일은 이
  레지스트리만 맡고, `Engine`은 다시 해석하지 않는다.
- **상속관계** — 없음.
...
- **메서드**
    ...
    - `resolve_specs(mode, declared, explicit)` : 위 모드 해석과 이름·파라미터 조회, 중복 제거, 결정적 정렬을 한 번에
      수행해 실행할 `IndicatorSpec` 목록을 돌려준다. `Engine`이 지표 대상을 얻는 유일한 진입점이다.
- **불변식** — 계산 전에 세 가지를 검증한다: 요청 지표·파라미터가 등록돼 있는가, 워밍업(`min_history`)이 확보됐는가,
  시장폭 지표라면 별도 입력 채널이 있는가.

#### `IndicatorState`

- **개요** — 백테스트·라이브·페이퍼가 실제로 도는 증분 계산 방식(캔들 하나당 O(1) 갱신).
- **책임** — 세 실행 환경의 확정 캔들을 같은 방식으로 처리한다. 벡터화 방식은 이 상태 구현과 같은 값을
  내는지를 독립적으로 대조하는 검증 기준이다.
- **상속관계** — 없음.
...
#### `PatternSpec`·`PatternState`·`PatternRegistry`
...
두 레지스트리의 이름은 원래 이름과 실행 열쇠 정규화 결과 모두에서 서로소여야 한다. `series_key`는 이름을
소문자로 만들고 영숫자가 아닌 연속 문자를 밑줄로 바꾼 뒤 정렬한 `key=value` 파라미터를 붙인다. 예를 들어
`EMA`, `period=9`의 실행 열쇠는 `ema:period=9`다. 이 열쇠는 대문자 이름과 `repr` 파라미터를 쓰는
`IndicatorSpec.identifier`와 용도가 다르다.

`auto`는 전략과 strategy-timeframe 정책이 선언한 지표·패턴만, `explicit`은 명시 목록만 먼저 해석한다. 백테스트
Engine은 explicit 결과에 모든 필수 조합이 있는지 다시 확인한다. `all`은 등록 지표 전체와 전략이 선언한 패턴만
계산하며 패턴 전체를 자동 활성화하지 않는다. signal-service는 현재 `auto`만 사용한다.
...
- **시장폭 지표의 조건부 활성** — 켜는 데 별도 조건이 붙는 지표다. McClellan Oscillator·Summation·TRIN 세 지표는
  ...
  지표를 건너뛴다.
- **현재 참조 전략과 정책 요구** — `VesselReference`는 EMA 9와 EMA 21을 선언한다. manual 정책은 strategy
  timeframe ATR 14를 별도로 선언하고, turtle 정책은 전략 입력과 분리된 일봉 `TURTLE_N`을 선언한다.
...
끝낸 뒤에야 신호를 받아들인다. 둘째, 벡터화 방식과 증분 방식이 같은 지표에서 같은 값을 내야 하는데(일치 테스트),
재귀형 지표는 시작 상태가 다르면 초반 값이 갈린다. 두 방식이 같은 규칙으로 seed해야 첫
...
- **preload** — 실행 드라이버는 평가 구간 시작 전에 `max(전략 min_history, 해석된 SeriesSpec의 최장 워밍업)`
  캔들을 별도 preload한다. strategy-timeframe 정책 요구도 이 목록에 합쳐진다. 별도 일봉 정책 요구는 달력 구간
  계산과 전용 정책 원천에서 처리한다.

#### 지표 계산 플로우

한 run의 입력으로 지표 값을 만드는 방식은 두 가지이고, 어느 쪽이든 같은 지표 구현·같은 프리미티브를 거쳐 값이
서로 같아야 한다(일치 테스트로 못박는다). 아래는 증분 실행의 look-ahead 관문과 벡터화 검증의 관계를 보인다.

```mermaid
flowchart TD
    CFG["run 설정: 계산 대상 모드<br/>auto · explicit · all"] --> RESOLVE["series_resolution.resolve_series_specs<br/>지표·패턴 명세 확정"]
    RESOLVE --> INC["SeriesState.seed 워밍업<br/>확정 캔들마다 update<br/>백테스트·signal-service 실행"]
    INC --> GATE["contracts.assert_finalized<br/>close_time ≤ 판단 시각 T"]
    GATE --> OUT["시각 t 지표 값<br/>전략 판단에 입력"]
    RESOLVE --> VEC["각 레지스트리 compute_batch<br/>확정 전 구간 일괄 계산<br/>연구·일치 검증 전용"]
    VEC -. 독립 일치 테스트 .-> INC
```
...
classDiagram
    ...
    class StrategyBase {
        ...
        +series_value(market_data: Mapping~str,object~, spec: SeriesSpec) object
    }
    ...
    class StrategyMetadata {
        ...
        +MoneyManagementSupport money_management
    }
    class MoneyManagementSupport {
        +tuple~str~ supported
        +str|None default
        +bool supports_external_stop
        +bool supports_external_take_profit
        +bool supports_signal_exit
        +bool supports_pyramiding
    }
    class ParameterSchema {
        +dict~str_FieldSpec~ fields
        +bool extra_forbidden
        +tuple cross_validators
    }
    ...
    class AdapterManager["Adapter Manager"] {
        +create(strategy_id: str, raw_config: Mapping) StrategyAdapter
        +create_runtime(strategy_id: str, raw_config: Mapping, money_management_config: Mapping) StrategyRuntime
    ...
    class InProcessStrategyRegistry {
        +register(strategy_id: str, adaptee_class: AdapterClass) None
        +get(strategy_id: str) AdapterClass
        +list() list~str~
        +unregister(strategy_id: str) None
    }
    class StrategyReconciliationModule["strategy.reconciliation"] {
        <<module>>
        +reconcile_strategy_registries(catalog_registry: StrategyRegistry, adapter_registry: InProcessStrategyRegistry) tuple~StrategyReconciliation,...~
    }
    class StrategyReconciliationState {
        <<enumeration>>
        CATALOG_ONLY
        ALLOWLIST_ONLY
        IDENTITY_MISMATCH
    ...
    class StrategyImplementationIdentity {
        <<frozen>>
        +str|None class_name
        +str|None module_path
    }
    class StrategyReconciliation {
        <<frozen>>
        +str strategy_id
        +StrategyReconciliationState state
        +StrategyImplementationIdentity|None catalog_identity
        +StrategyImplementationIdentity|None allowlist_identity
        +bool|None is_active
        +bool|None is_deprecated
    }
    ...
    class StrategyRuntime {
        ...
        +MoneyManagementPolicy|None money_management
    }
    ...
    AdapterManager --> InProcessStrategyRegistry : injected
    AdapterManager ..> StrategyReconciliationModule : reconcile_catalog
    AdapterManager --> StrategyRuntime
    StrategyReconciliationModule ..> StrategyRegistry : reads catalog
    StrategyReconciliationModule ..> InProcessStrategyRegistry : reads allowlist registry
    StrategyReconciliationModule --> StrategyReconciliation : returns findings
    StrategyReconciliation *-- StrategyReconciliationState
    StrategyReconciliation *-- StrategyImplementationIdentity
...
- **불변식**
    ...
    - **runtime 검사 범위** — `@runtime_checkable`의 `isinstance(instance, StrategyAdapter)`는 필요한 메서드 이름의
      ...
      않은 하위 클래스를 인스턴스화하는 시점에 막는다. 추상 메서드도 오버라이드의 시그니처 일치 자체를 런타임에
      검증하지는 않는다.

#### `StrategyBase`

- **개요** — `StrategyAdapter`를 만족하는 전략을 쓸 때 선택할 수 있는 stateless 편의 기반 클래스다. 규범은
  `StrategyAdapter` Protocol이고 이 클래스의 상속은 의무가 아니다.
- **책임** — 전략 작성자가 채울 `get_metadata`·`get_parameter_schema`·`analyze` 추상 메서드 셋과, Engine이
  전달한 `market_data["indicators"]`에서 `SeriesSpec`의 공통 실행 열쇠로 값을 꺼내는 읽기 전용
  `series_value`만 제공한다.
- **상속관계** — `ABC`를 상속하며 구조적으로 `StrategyAdapter`를 실현한다.
- **필드** — `__slots__ = ()`로 기반 클래스 자체에 인스턴스 상태 저장 자리를 두지 않는다.
- **메서드**
...
- **개요** — 코드에는 `Adaptee`라는 필수 기반 클래스가 없다. 등록 클래스가 `StrategyAdapter`의 구조를 만족하면
  Adaptee 역할을 하며, 현재 기본 실행 조립에 등록되는 참조 구현은 `VesselReference` 2.0.0이다.
- **책임** — EMA 9와 EMA 21의 상대 위치로 진입·청산 edge만 판단한다. 무포지션에서는 명시적인
  `ENTER_LONG`·`ENTER_SHORT`, 보유 중 regime가 뒤집히면 `EXIT`, 그 밖에는 `None`을 반환한다. ATR 손절·목표·수량·
  leverage는 만들지 않는다.
...
- **개요** — `reconcile_strategy_registries(catalog_registry, adapter_registry)`가 외부 운영 카탈로그 전체와 배포된
  in-process 레지스트리 전체를 읽고, 실행 불가 또는 불일치 항목을 `StrategyReconciliation`으로 반환한다.
- **책임** — 전략 id를 정렬해 각 id에 아래 상태를 최대 하나만 부여한다.

| `StrategyReconciliationState` | 뜻 |
|---|---|
| `CATALOG_ONLY` (`catalog_only`) | 외부 카탈로그에는 있지만 허용 목록에는 없다. 이 서비스가 그 전략 구현을 품지 않는다는 뜻이다. |
| `ALLOWLIST_ONLY` (`allowlist_only`) | 허용 목록에는 구현이 있지만 외부 카탈로그에는 없다. 코드가 배포되었으나 운영 카탈로그에 등록되지 않았다는 뜻이다. `CATALOG_ONLY`와 원인이 정반대다. |
| `IDENTITY_MISMATCH` (`identity_mismatch`) | 양쪽에 같은 전략 id가 있지만 카탈로그의 `class_name`·`module_path`와 허용 목록 클래스 신원이 다르다. |
| `INACTIVE` (`inactive`) | 양쪽 신원이 일치하지만 카탈로그의 `is_active`가 거짓이고 deprecated 상태는 아니다. |
| `DEPRECATED` (`deprecated`) | 양쪽 신원이 일치하지만 카탈로그의 `is_deprecated`가 참이다. 동시에 inactive여도 이 상태가 우선한다. |

- **상속관계** — `StrategyReconciliationState`는 `StrEnum`이고 두 결과 값 타입은 frozen dataclass다.
- **필드** — 각 결과는 양쪽에서 확보한 클래스 신원과 카탈로그의 `is_active`·`is_deprecated` 값을 함께 보존한다.
- **메서드**
    - `reconcile_strategy_registries(catalog_registry, adapter_registry)` : 다섯 상태의 결정적 tuple을 반환한다.
    - `AdapterManager.reconcile_catalog()` : 매니저에 주입된 두 레지스트리로 위 함수를 호출한다.
- **불변식** — 소속 차이를 행 flag보다 먼저, 신원 차이를 lifecycle flag보다 먼저 판정하고 deprecated를 inactive보다
  먼저 판정한다. 카탈로그와 허용 목록에 모두 있고 신원이 일치하며 활성·비폐기인 정상 항목은 결과에서 빠진다.
  맞춰 보기 결과는 현재 코드 표면으로만 제공되며 화면은 아직 이 다섯 상태를 사용하지 않는다.
...
- **상속관계** — 없음(독립 값 타입).
- **필드**
    - `required_indicators` — 이 전략이 요구하는 등록 계열 목록이다. 각 항목은 이름과 파라미터의 쌍 `{name, params}`
      이며, `series_resolution`이 지표 또는 패턴 소유 레지스트리로 보낸 뒤 실행면이 계산해
      `analyze`에 넘긴다.
    ...
    - `money_management` — 허용 정책 id, 기본 정책, 외부 보호가격·신호 청산·피라미딩 capability를 담은
      `MoneyManagementSupport`다. 현재 `VesselReference`는 manual과 turtle을 허용하고 기본은 manual이며,
      외부 stop·take-profit과 signal exit는 참, pyramiding은 거짓이다.
- **메서드** — 없음(순수 값).
- **불변식** — 없음.
...
- **메서드**
    ...
    - `create_runtime(strategy_id, raw_config, money_management_config)` : 위 생성 뒤 메타데이터가 정책을 지원하면
      `MoneyManagementFactory`로 엄격히 해석한 정책을 결합해 `StrategyRuntime`을 반환한다. 정책 id가 지원 목록에
      없으면 거부하고 turtle은 `supports_signal_exit`가 참일 때만 허용한다.
    - `activate` : Adaptee를 활성화한다.
    - `deactivate` : Adaptee를 비활성화한다.
    - `list_registered` : signal_db 레지스트리의 등록 목록을 조회한다.
    - `register` : 같은 id의 in-process 클래스가 있는지 확인한 뒤 외부 포트에 위임한다. 현재 backtest와 signal
      서비스의 포트 구현은 읽기 전용이므로 쓰기는 `PermissionError`로 거부된다.
- **불변식** — 레지스트리 DB 접근은 주입된 `StrategyRegistry` 포트로만 한다. 외부 카탈로그와 클래스의
  `class_name`·`module_path`·`min_history`·`supported_timeframes`·`required_indicators_json`을 대조하고 어긋나면
  실행을 거부한다. 전략 판·파라미터 기본값·프로파일·정책 capability는 현재 이 대조 범위에 없다.
...
sequenceDiagram
    ...
    participant CR as StrategyRegistry (외부 카탈로그)
    participant IR as InProcessStrategyRegistry
    participant AD as 등록된 Adaptee 클래스
    ...
    CR-->>AM: catalog_entry
    AM->>AM: active·deprecated·외부 id 검사
    AM->>IR: get(strategy_id)
    IR-->>AM: adaptee_class
    AM->>AM: 클래스 신원과 등록 이력 선언 대조
    ...
    AM->>AF: create(adaptee_class, ResolvedConfig)
    AF-->>AM: StrategyAdapter
    AM->>AD: instance.get_metadata()
    AD-->>AM: MoneyManagementSupport
    alt supported가 비어 있음
        AM-->>E: StrategyRuntime(strategy, None)
    else 정책 지원 선언이 있음
        AM->>MF: create(money_management_config)
        MF-->>AM: MoneyManagementPolicy
        AM->>AM: 정책 id와 turtle signal-exit capability 검사
        AM-->>E: StrategyRuntime(strategy, policy)
    end
```

외부 카탈로그는 실행 가능 여부와 운영자에게 보이는 선언을 제공하고, in-process 레지스트리는 실제 클래스를
제공한다. 두 값이 어긋나면 어느 쪽이 낡았는지 추정하지 않고 거부한다. 그다음 스키마 선언·설정 해석·인스턴스 생성·
정책 runtime 조합을 순서대로 수행한다. `StrategyConfig`는 스키마 값만 받아 Adaptee를 되짚지 않는다.
...
`strategy/trailing/trailing_stop.py`에는 예약 위치를 알리는 모듈 docstring만 있다. `TrailingStopCalculator`나
`TrailingState` 클래스 및 계산 함수는 현재 코드에 없다. `OrderType.TRAILING_STOP_MARKET`과
`ExitReason.TRAILING_STOP` 값은 존재하지만 matcher는 해당 주문을 `NotImplementedError`로 거부한다. 따라서 이
문서에는 트레일링 계산 클래스나 실행 가능한 트레일링 경로를 그리지 않는다.

### §4.2.2 자금관리 계층 — 결정을 한계 지어진 계획으로 변환
...
`PolicyIndicatorRequirement.timeframe`은 현재 `strategy`와 `1d`만 허용한다. `strategy` 요구는 전략 선언과 합쳐
공통 `series_resolution`을 통과하고, `1d` 요구는 전략 입력과 분리된 정책 전용 일봉 경로에서 처리된다.
`RiskLimits.risk_per_trade`는 0보다 크고 1퍼센트 이하여야 하며 정책이 확대할 수 없다.

`MoneyManagementPlan`은 요청 계획이지 주문이나 체결 결과가 아니다. Engine과 matcher는 실제 체결 가격에 최초
...
sequenceDiagram
    ...
    M->>M: 방향별 stop_loss와 take_profit 계산
    M-->>E: MoneyManagementPlan
```

manual은 strategy timeframe의 ATR 14를 요구한다. 손절거리는 ATR과 `atr_stop_multiple`의 곱이고 목표가격은 그
거리에 `reward_risk`를 적용한다. 요청 수량은 `equity × risk_per_trade ÷ 손절거리`다. futures에서는 설정
leverage, spot에서는 1을 요청하며 전역 최대 leverage를 넘으면 거부한다.

이 정책이 legacy Vessel의 ATR 손절, 고정 reward-risk 목표와 leverage 결과를 보존한다. 백테스트 `RunConfig`에서
`money_management`가 없는 `vessel-reference` 설정은 과거 평면 `leverage`·`reward_risk`·`atr_stop_multiple` 값을
manual 설정으로 정규화한다. `RunConfig.money_management`와 `VesselReference` 메타데이터의 기본값도 manual이다.

#### `TurtleMoneyManagement`와 별도 일봉
...
sequenceDiagram
    ...
    F-->>E: 일봉 목록
    E->>N: turtle_n_series(확정 일봉, period)
    N-->>E: close_time별 N 값
    Note over E: 판단 시각 이하의 가장 최근 N만 선택한다.
    E->>T: plan_entry(decision, MarketSnapshot(N), AccountRiskSnapshot, RiskLimits)
    T->>T: 손절거리·위험예산·요청수량 계산
    T->>T: 최소 정수 leverage와 청산 안전성 검사
    T-->>E: take_profit이 없는 MoneyManagementPlan
```

turtle은 `TURTLE_N` 하나를 일봉에서 요구한다. Engine은 전략 timeframe 이력을 재표본화하지 않고 같은 실행에서
`DataFeed.candles(symbol, "1d", end)`를 별도로 호출한다. `turtle_n_series`는 True Range의 첫 기간 평균 뒤 Wilder
방식으로 N을 갱신한다. 판단 시점에는 그 시각까지 완전히 닫힌 가장 최근 일봉의 N만 사용한다.

turtle은 `N × stop_n_multiple`을 손절거리로 쓰고 manual과 같은 전역 위험 상한으로 수량을 계산한다. 요청
leverage는 요청 notional을 가용 현금으로 나눈 값의 올림이며 최소 1이다. futures에서는 정책 cap과 전역 cap 중
작은 값을 넘으면 거부하고, spot에서는 leverage 1로 감당하지 못하면 거부한다. 예상 청산가격이 손절보다 먼저
도달하면 거부하고 고정 take-profit은 만들지 않는다.

현재 turtle 정책은 피라미딩이나 단계별 익절 계획을 만들지 않으며 전체 역사적 Turtle 시스템을 구현하지 않는다.
진입과 청산 edge는 계속 전략 소유다. `core_lib.sizing.turtle_unit`의 유닛 및 0.5N 계산 함수는 존재하지만 현재
`TurtleMoneyManagement`나 Engine이 이를 호출하지 않는다.
...
sequenceDiagram
    ...
    AM-->>E: StrategyRuntime(strategy, policy)
    E->>P: 진입 DecisionIntent마다 plan_entry(...)
```

백테스트 `RunConfig`는 manual과 turtle을 `mode`로 구분하는 설정 union을 가진다. 설정이 없으면 manual이고,
turtle은 `sizing_method="risk_based"`일 때만 허용된다. Engine은 실제 equity·가용 현금·시장·전역 위험 한도로 계획을
만들며 정책이 `MoneyManagementError`로 거부하면 주문 없이 `money_management_rejected` 후보 Evidence를 기록한다.
...
classDiagram
    ...
    class Matcher {
        <<module>>
        +match(order: Order, candle: Candle, history: list~Candle~, cost_model: CostModel, fill_timing: str, risk_budget: Decimal|None, available_margin: Decimal|None, leverage: int) Fill
        +resolve_triggers(position: Position, candles: list~Candle~, cost_model: CostModel, stop_price: Decimal|None, take_profit_price: Decimal|None, entry_time: datetime|None) Fill|None
        +recompute_qty_and_stop(order: Order, fill_price: Decimal, risk_budget: Decimal|None, available_margin: Decimal|None, leverage: int, cost_model: CostModel|None) tuple
    }
    class PositionBook {
        +apply(fill: Fill, leverage: int|None, margin_type: MarginType|None, market_type: MarketType, liquidation_price: Decimal) None
        +weighted_average(Fill) Decimal
        +reduce(Fill) Decimal
        +apply_funding(Position, Decimal, Decimal) FundingMarginResult
...
- **메서드**
    - `match(order, candle, history, cost_model, fill_timing)` : 주문을 `t+1` 시가에 슬리피지를 얹어 체결한다
      ...
      처리한다(리버설 순서. 마진 가용성은 청산 정산 후 기준). next-bar 체결 시각은
      `max(다음 캔들 open_time, 결정 캔들 close_time + 1ms)`이고, 다음 캔들의 여는 시각이 결정 캔들의 닫는 시각과
      이어지지 않으면 그 주문은 이 메서드로 넘기지 않는다. 결측 구간을 건너 먼 미래 캔들에서 체결하는 것은 허용하지
      않는다.
    - `resolve_triggers` : 손절·익절·강제청산의 캔들 내 발동을 판정한다. 현재 Engine이 허용하는 입력은 전략 TF
      캔들 하나뿐이며, `m1_subcandle`은 `RunConfig`와 Engine이 `NotImplementedError`로 거부한다. 어떤 포지션도
      자기 체결 시각 이하 캔들로 소급 검사하지 않는다. trailing 주문은 `_trigger_reference`에서
      `NotImplementedError("trailing-stop matching is reserved")`로 거부된다.
    - `recompute_qty_and_stop` : 수량과 최초 보호 스탑을 신호 캔들 종가가 아니라 실제 체결가 기준으로 재산정한다.
      ...
      보호 스탑)` tuple이다. 진입 가능 수량은 진입 마진·진입 수수료·강제청산 시 한 번 부과할 청산 수수료까지
      감당하는 범위로 제한한다. 청산 수수료는 이때 즉시 차감하는 비용이 아니라 지급 능력 예약이며, 미래 펀딩 비용은
      예약하지 않는다. 아직 관측하지 않은 정산 경계를 준비금으로 잡으면 같은 전략과 원천이라도 평가 창 길이에 따라
      수량이 달라지기 때문이다.
- **불변식**
    - **체결은 결정보다 나중(next-bar)** — 신호는 캔들 `t` 마감에 나고 체결은 `t+1` 시가라 `decision_ts <
      execution_ts`를 만족한다. 결정 캔들 종가로 체결하지 않는다.
    - **체결 지연 상한** — 대응 판단이 있는 체결은 `execution_ts − decision_ts`가 전략 주기 한 개를 넘지 않는다.
      이 상한은 결측 구간을 건넌 지연 체결을 시점의 대소 관계만으로 놓치지 않게 하며, Evidence의 시점 순서 검사가
      다시 확인한다.
    ...
    - **트레일링 미구현** — `OrderType.TRAILING_STOP_MARKET`과 `ExitReason.TRAILING_STOP`은 열거형에만 있다.
      matcher는 trailing 주문을 거부하고 `resolve_triggers`에는 trailing 가격 인자가 없다.
...
- **메서드**
    ...
    - `reduce` : `reduce_only` Fill의 수량만큼 감량하고 마진을 반환한다. 전량이면 장부에서 제거하며 임의의 더 작은
      수량이면 남은 포지션을 유지한다.
    - `apply_funding` : 선물 격리 포지션의 이론 펀딩 비용을 포지션 마진에 적용하고, 이론 비용·실제 적용 비용·마진
      소진 여부를 `FundingMarginResult`로 돌려준다. 지급 비용은 남은 격리 마진을 넘지 못하고, 펀딩 수취는 마진을
      늘린다. 적용 뒤 남은 마진으로 강제청산가를 다시 계산한다.
    - `check_liquidation` : Isolated를 우선해 청산을 판정한다 — 발동 비교 자체는 `costs.Liquidation.is_triggered`에
      위임하고(수식·비교 복제 방지) 이 메서드는 장부 차원의 적용만 한다.
- **불변식** — 이 임의 수량 감량 능력은 저수준 체결 회계다. 현재 `MoneyManagementPlan`과 Engine에는 목표가격이
  하나뿐이고 matcher의 목표 trigger는 포지션 전량 exit Fill을 만들므로 단계별 부분 익절 계획은 존재하지 않는다.
  회계 항등식 `cash + position = equity`는 `Accounting`이 강제한다.
...
- **책임** — UTC 정산 경계를 지나는 포지션에 펀딩을 부과한다. 이 모듈은 수식만 소유하고, rate는 과거 실측을
  `DataFeed`로 주입받으며 실측이 없을 때만 `CostModel`의 fallback rate `0.0001`(0.01%)을 쓴다. 계산한 이론
  비용은 현금이 아니라 `PositionBook.apply_funding`을 거쳐 격리 포지션 마진에 적용한다.
- **상속관계** — 없음(모듈).
- **필드** — 없음(모듈. rate 값은 주입).
- **메서드**
    - `settle(position, rate, price)` : UTC `0/8/16`시 경계를 보유 상태로 지나는 포지션의 이론 펀딩 비용을
      `notional × rate`로 계산한다. 경계를 지나지 않으면 실제 펀딩은 0이다. 정산가는 정산 경계를 포함하는 최소
      가용 주기 캔들의 시가로 고정하고, 그 캔들이 없으면 직전 확정 캔들 종가를 쓴다. 경계 직전 보유분에만 부과하고,
      동시각 신규 체결은 그 정산을 물지 않는다.
    - `is_boundary(t)` : 주어진 시각이 UTC `0/8/16`시 정산 경계인지 판정한다.
- **불변식**
    - 이산 정산이 표준이다. `×보유시간/8h` pro-rata 식은 사전 추정용 참고로만 쓰고 실제 부과에는 쓰지 않는다.
    - 지급 방향의 실제 적용 비용은 `min(이론 비용, 정산 직전 남은 격리 마진)`이다. 이론 비용과 실제 적용 비용을
      모두 Evidence에 남긴다. 실제액이 이론액보다 작으면 한도 적용으로 남은 마진이 0임을 뜻하고, 이론액과 정확히
      같더라도 적용 뒤 마진이 0이면 소진이다. 두 경우 모두 그 거래는 같은 경계의 `boundary + 1ms`에
      `Fill(exit_reason=LIQUIDATION)`로 끝난다. 이 체결에는 일반 taker 청산 수수료를 부과해 거래의
      `total_fee`에 정확히 한 번 합산하고 `liquidation_penalty`는 0으로 둔다. 별도 청산 손실을 다시 빼면 같은
      사건을 이중 계상하므로 금지한다.
    - 펀딩은 가용 현금에서 직접 빼지 않는다. 포지션 마진과 누적 펀딩만 바뀌며, 어떤 경로에서도 현금 잔고를 음수로
      만들 수 없다.
...
classDiagram
    ...
    class RiskMoney {
        <<module>>
        +size(risk_per_trade: float, equity: float, stop_distance: float) float
        +one_r(entry: float, stop: float, quantity: float) float
        +equity(cash: float, used_margin: float, unrealized: float) float
    }
    class TurtleUnit {
        <<module>>
        +unit_size(risk_per_trade: float, equity: float, n: float) float
        +pyramid_step() float
        +int unit_limit$
    }
    class WalletPct {
    ...
    class MoneyManagementPolicy {
        <<Protocol>>
        +plan_entry(...) MoneyManagementPlan
    }
    class MoneyManagementPlan {
        +requested_quantity float
        +initial_risk_amount float
...
- **책임** — 거래당 위험을 계좌 비율(`risk_per_trade`)로 고정해 수량을 산정한다. 손절거리와 Equity로 수량을
  정한다. 이 모듈 자체는 leverage를 계산하지 않는다.
- **상속관계** — 없음(모듈).
...
- **메서드**
    - `size(risk_per_trade, equity, stop_distance)` : `수량 = (risk_per_trade × Equity) / 손절거리`. 여기서
      ...
      시장일수록 수량이 작아져 모든 포지션의 위험기여도가 균등화된다. 요청 leverage는 자금관리 정책이, 최종
      증거금 허용은 matcher가 처리한다.
    - `one_r(entry, stop, quantity)` : `1R = |체결가 − 최초 보호 스탑| × 수량`으로 최초 위험을 계산한다. 현재
      자금관리 계획은 모두 최초 stop을 제공하며 트레일링 계산기는 구현되어 있지 않다. 최초 stop을 정의할 수 없는
      legacy 거래는 `r0`를 NULL로 두고 R 기반 지표에서 제외한다.
...
- **개요** — 변동성 단위 `N`의 저수준 계산기. `TurtleMoneyManagement`와 이름은 관련되지만 현재 runtime에서 직접
  호출되지 않는다.
- **책임** — `unit_size`로 N 기준 한 유닛 크기를 계산하고 0.5N 간격과 네 유닛 상수를 노출한다. 실제 포지션을
  증량하거나 계획을 생성하지 않는다.
- **상속관계** — 없음(모듈. `RiskMoney` 수식을 조합해 쓴다).
- **필드** — `unit_limit`는 단일 시장의 유닛 상한을 담은 상수다(원조 4유닛).
- **메서드**
    - `unit_size(risk_per_trade, equity, n)` : 변동성 단위 `N`으로 한 유닛의 크기를 잡는다.
    - `pyramid_step()` : 0.5N마다 증량한다.
- **불변식** — 현재 Engine은 피라미딩을 실행하지 않고 `VesselReference.supports_pyramiding`도 거짓이다. 이
  모듈의 존재를 피라미딩 계획이 구현됐다는 뜻으로 읽지 않는다.
...
- **책임** — 판정 전에 여섯 가지를 검산하며, 하나라도 실패하면 `diagnostic_only`로 **멈춰서** 데이터·기록을 고쳐
  ...
  일치 테스트가 담당하므로 이 여섯 검사에는 넣지 않는다. `check`는 입력 mapping에 `trailing_parity`가 있을 때만
  선택적인 일곱 번째 검사로 처리하지만, 현재 backtest Evidence sink는 여섯 표준 키만 만들며 Engine도 trailing과
  `m1_subcandle`을 거부한다. 따라서 현행 run에서 선택 검사가 실행되는 경로는 없다.
- **상속관계** — 없음(모듈).
- **필드** — 없음(모듈).
- **메서드**
    - `check(evidence)` : 아래 여섯 가지를 검산해 `IntegrityResult`를 낸다.
        - **회계 항등식** — `cash + position = equity`가 모든 시점에 성립하는가.
        - **시점 순서** — `feature_ts ≤ decision_ts < execution_ts`를 어긴 거래가 없는가. 대응 판단이 있는 체결은
          `execution_ts − decision_ts`가 전략 주기 한 개를 넘지 않아야 하고, 판단 없는 체결은 `END_OF_DATA`만
          허용한다(look-ahead와 결측 횡단 체결의 사후 검증).
        - **비용 1회 차감** — 각 비용이 중복 차감되지 않았는가.
        - **net-of-cost** — 기록된 손익이 net인가("비용 0 가정"의 잔재가 없는가).
        - **결정성** — 같은 `config_hash`와 같은 `source_data_hash`를 가진 비교 대상이 같은 정규화 Evidence 해시를
...
## §4.4 backtest-service 클래스 — Engine·포트 어댑터·설정·Harness (+ 캔들 루프 시퀀스)
...
- **캔들 루프 순서 규칙** — `Engine`의 캔들 루프(`step_open`·`step_close`·`walk_triggers`, §4.4.1)가 지키는 캔들
  ...
  (§4.3.1)가 소유한다.
    ...
    - **결정 < 체결(next-bar)** — 신호는 캔들 `t` 종가에 만들지만 체결은 `t+1` 시가에만 일어난다(`decision_ts <
      ...
      순서를 강제한다.
    - **캔들 내 트리거는 전략 TF 캔들로 판정** — 현재 실행 가능한 `trigger_feed`는 `tf_candle`뿐이다.
      `m1_subcandle`은 `RunConfig` 검증과 `Engine.run` 및 `walk_triggers`가 `NotImplementedError`로 거부한다.
      손절·익절·강제청산을 전략 TF 캔들의 고저로 보수 판정하며 trailing은 matcher가 거부한다.
    - **동시 도달 손절 우선(보수적 최악 경로)** — 같은 전략 TF 캔들에서 손절과 익절이 모두 범위 안이면 손절 우선이다.
...
classDiagram
    ...
    class Engine {
        ...
        -StrategyAdapter|None _strategy
        -MoneyManagementPolicy|None _money_management
        -list~SeriesSpec~ _indicator_specs
    ...
    Engine ..> AdapterManager : StrategyRuntime 생성
    Engine --> StrategyRuntime
    Engine --> SeriesSpec
    Engine --> MoneyManagementPolicy
...
- **책임** — 사전등록·run_id 발급·워밍업 preload·피드 push·전략 판단·사이징·체결·2계층 저장·finalize·판정 호출을
  한 흐름으로 엮는다. `AdapterManager.create_runtime`으로 전략과 정책을 조합하고, 공통 계열 요구를 해석하며,
  `DecisionIntent`를 정책 적용 뒤 내부 `TradingSignal` 형식으로 구체화한다. 전략·정책·지표·사이징·비용·체결·평가
  수식은 `core-lib` 구현을 호출한다. 데이터·체결·시각·기록·전략 목록은 주입 포트로만 접근한다.
...
- **필드**
    ...
    - `manager` — `Adapter Manager`(§4.2). `strategy_id`·전략 파라미터·자금관리 설정으로 `StrategyRuntime`을
      생성한다. 내부적으로 주입된
      `StrategyRegistry` 어댑터(§4.4.2)를 통해 전략 목록에 접근한다.
    - `config` — 검증을 마친 `RunConfig`(§4.4.3).
    - `engine_version`·`core_lib_version` — 설치된 `backtest-service`와 `core-lib` 패키지 메타데이터에서 읽은
      실행 코드 버전이다. 생성자가 명시적으로 덮어쓰는 검증 경로를 제외하면 사람이 별도 상수를 갱신하지 않으며,
      같은 값이 카탈로그와 Evidence에 기록된다.
    - `_strategy`·`_money_management` — 현재 runtime에서 분리해 보관한 판단 전략과 선택 정책이다. 정책은
      `DecisionIntent` 진입에만 적용하며 legacy `TradingSignal`에는 적용하지 않는다.
    - `_indicator_specs` — 전략 선언과 strategy-timeframe 정책 요구를 `resolve_series_specs`로 해석한
      `IndicatorSpec | PatternSpec` 목록이다. turtle의 일봉 N은 이 목록과 별도다.
    - `pending` — 직전 캔들 종가에 산출해 다음 캔들 시가에 체결할 `_PendingOrder` 목록. 보통 0개나 1개이고,
      ...
      직전 캔들의 닫는 시각과 다음 캔들의 여는 시각이 같을 때만 주문을 만들 수 있으며, 그 사이에 결측이 있으면
      판단은 `next_candle_gap` 사유의 건너뜀으로 기록한다. 방어적으로 남은 주문이 결측 경계를 건너면 실행을 거부한다.
- **메서드**
    - `run(config)` : run 하나를 처음부터 끝까지 구동해 `RunResult`를 낸다. 설정을 재검증하고
      `create_runtime`으로 전략과 정책을 조합하며, 전략과 정책 계열을 해석한다. 평가 시작 전
      `max(전략 min_history, strategy-timeframe SeriesSpec 최장 min_history)` 캔들을 내부 `_preload`로 선택해
      상태를 seed하고, turtle이면 별도 일봉 N 원천도 준비한 뒤 캔들 루프와 finalize를 진행한다. `preload()`라는
      공개 메서드는 없으며 제한 조회가 부족하면 하한 제한을 풀고 다시 읽는다.
    - `step_open(t)` : 캔들 `t`가 열리는 순간의 일. `Clock`이 `t` 시가에 있을 때 호출된다. `pending`의 주문을 담긴
      ...
      그다음 전략 TF 캔들 하나에 대해 `walk_triggers`로 기존 포지션의 손절·익절·강제청산을
      판정하고(발동하면 신호 없는 판단으로 Decision(`signal_id` 비움)과 Execution·Trade를 기록한다), UTC 정산 경계를
      지나면 `costs.funding.settle`로 펀딩을 부과한다. 펀딩으로 격리 마진이 소진되면 그 정산 경계의 1밀리초 뒤에
      `LIQUIDATION` 체결을 만들어 해당 포지션을 닫는다.
    - `step_close(t)` : 캔들 `t`가 닫히는 순간의 일. `Clock`이 `t` 종가로 전진한 뒤 호출된다. `DataFeed`로 `up_to =
      t.close`까지만 받아 모든 `SeriesState`를 갱신하고 Indicator Snapshot과 캔들 종가로 마킹한 Position·Portfolio/PnL
      ...
      현재 캔들의 닫는 시각과 다음 캔들의 여는 시각이 이어지지 않으면 주문을 예약하지 않는다. 열린 포지션은 마지막
      확정 가격으로 결측 직전에 `DATA_GAP` 청산하고 일반 청산과 같은 수수료·슬리피지를 적용한다. 그 뒤 결측이 삼킨
      펀딩 경계는 도달 불가 경계로 세어 Source Data Snapshot과 run 요약에 남기므로, 펀딩 정산 완성도가 당시 포지션
      보유 여부에 우연히 좌우되지 않는다.
    - `walk_triggers(position, subcandles)` : 이름은 `subcandles`를 받지만 현재 `trigger_feed != "tf_candle"`이면
      즉시 `NotImplementedError`를 발생시킨다. Engine은 `[candle]` 하나를 넘겨 `resolve_triggers`의 손절·익절·
      강제청산 보수 판정을 실행한다. `entry_time` 이하 캔들은 건너뛰고 같은 캔들의 손절·익절 동시 도달은 손절
      우선이다. trailing 가격 인자는 없고 1분 하위 캔들 walk도 실행되지 않는다.
    - `finalize()` : 캔들 루프 종료 후 열린 포지션을 마지막 확정 캔들 종가로 강제 정리해
      ...
      `RunResult`다. 판정 입력에는 관측 캔들 수·원천 부재 수·부분 버킷 수·커버리지·최장 연속 결측·도달 불가
      펀딩 경계 수·`DATA_GAP` 청산 수가 포함된다. 커버리지가 `0.95`보다 낮거나 최장 연속 결측이 `86,400`초를
      넘으면 무결성이 통과했더라도 Hard Gate A를 `not_promotable`로 만들고 최종 경로를 `retest`로 둔다. 두 기준은
      판정 입력으로서 Evidence의 `data_quality_criteria_json`에 반드시 기록하고 결정성 해시에 포함한다. 사전등록
      선언 사본인 `prereg_json`과 섞지 않으므로, 사전등록 해시 제외 원칙과 판정 입력의 재현성을 함께 지킨다.
- **불변식**
    ...
    - **`decision_ts < execution_ts`** — 신호는 `step_close`에서, 체결은 다음 `step_open`에서 일어난다. `pending`의
      한 칸 지연이 이 순서를 강제한다.
    - **결측은 미지의 값으로 남긴다** — 결측 구간을 보간하거나 미래 캔들로 주문을 이월하지 않는다. 결측 직전의
      미체결 주문은 만들지 않고 열린 포지션은 `DATA_GAP`으로 닫아, 미관측 구간의 가격과 펀딩을 성과로 가정하지
      않는다.
    - **워밍업 preload 구간 신호 discard** — preload 구간의 신호·주문은 성과에 반영하지 않는다.
    - **결정성** — 시각은 `Clock`만 공급하고 wall-clock·무제어 난수를 쓰지 않는다. 같은 설정 입력·같은 원천
      데이터·같은 seed는 같은 정규화 Evidence 해시를 낸다.
...
sequenceDiagram
    ...
    participant SER as SeriesState (core-lib)
    participant AD as StrategyAdapter (core-lib)
    participant MM as MoneyManagementPolicy (core-lib)
    participant EX as execution (core-lib)
    participant CST as costs (core-lib)
    participant SIZ as sizing·risk (core-lib)
    ...
    ENG->>EX: matcher.resolve_triggers(position, [t], cost_model, stop_price, take_profit_price, entry_time)
    EX-->>ENG: Fill 또는 None
    ...
    ENG->>SER: update(t)
    SER-->>ENG: series_key별 값
    ENG->>EV: record(Indicator Snapshot·Position·Portfolio/PnL — 캔들당 1회 격자)
    ENG->>AD: analyze(market_data[t], current_position)
    alt 진입 DecisionIntent
        AD-->>ENG: DecisionIntent
        ENG->>MM: plan_entry(decision, market, account, limits)
        MM-->>ENG: MoneyManagementPlan
        ENG->>ENG: 내부 TradingSignal로 구체화
    else 청산 DecisionIntent
        AD-->>ENG: DecisionIntent
        ENG->>ENG: 정책 호출 없이 청산 TradingSignal로 구체화
    else legacy TradingSignal
        AD-->>ENG: TradingSignal
        Note over ENG,MM: legacy 보호가격·leverage를 사용하고 정책은 호출하지 않는다.
    else None 또는 HOLD
        AD-->>ENG: 관망
    end
    opt 구체적인 TradingSignal
        ENG->>EV: record(Signal·Decision)
        ENG->>SIZ: 정책 요청 수량 또는 legacy risk sizing·노출 제한
        SIZ-->>ENG: 허용 quantity
...
`walk_triggers`의 현재 실행 가능 경로와 예약 경계를 보인다.

```mermaid
flowchart TD
    START["step_open(t): 포지션 보유 중"] --> FEED{"trigger_feed"}
    FEED -->|"tf_candle"| TF["resolve_triggers(position, [t], …)<br/>손절·익절·강제청산 보수 판정"]
    FEED -->|"m1_subcandle"| REJECT["NotImplementedError<br/>RunConfig와 Engine이 거부"]
    TF --> FIRE{"발동?"}
    FIRE -->|"손절·익절 동시 → 손절 우선"| OUT["전체 포지션 exit Fill"]
    FIRE -->|"없음"| NONE["None"]
    TRAIL["TRAILING_STOP_MARKET 주문"] --> TFAIL["matcher가 NotImplementedError로 거부"]
```

현재 트리거 판정은 전략 TF 캔들 하나의 고가·저가만 사용한다. 손절·익절·강제청산의 판정은
`execution.matcher.resolve_triggers`가 소유하고 Engine은 그 함수를 한 번 호출한다. `m1_subcandle`과 trailing은
설정 또는 matcher 경계에서 명시적으로 거부되므로 이 문서에 실행 시퀀스나 파리티 수치를 두지 않는다.
...
classDiagram
    ...
    class BacktestClock {
        ...
        +from_candles(list~Candle~) BacktestClock
        +now() datetime
        +advance() None
    }
...
- **책임** — `crypto_data`에서 과거 확정 OHLCV·펀딩 실측·마크 가격을 읽어 전략 TF 캔들로 공급하고, `up_to` 이후
  캔들은 절대 내주지 않는다(look-ahead 구조 배제). 같은 메서드는 `tf = "1m"` 원천·펀딩 근거와 turtle 정책의
  별도 `1d` N 계산에도 쓰이지만, Engine의 캔들 내 trigger는 전략 TF 캔들만 소비한다. `m1_subcandle`은 설정과 Engine
  경계에서 거부된다. backtest는 읽기만 하고 `crypto_data`에 쓰지 않는다.
  상위 주기 리샘플링은 해당 버킷의 1분 원천이 모두 있을 때만 캔들을 만들며, 일부만 있는 버킷은 정상 원천 부재와
  구별해 부분 버킷으로 기록한다. 어느 경우에도 값을 채우거나 보간하지 않는다.
- **상속관계** — `DataFeed`(§4.3.4 ABC)를 실현한다.
- **필드** — `crypto_data`는 과거 OHLCV·펀딩을 읽는 읽기 전용 연결이다(쓰기 없음).
- **메서드**
    - `candles(symbol, tf, up_to)` : `up_to` 이후 캔들을 반환하지 않는다. 전략 판단과 정책 계산에 요청된 TF의 확정
      캔들을 내며, 완전하지 않은 상위 주기 버킷은 반환하지 않는다. `"1m"` 결과는 원천 검증·펀딩 근거에 쓰일 수
      있지만 현재 trigger walk에는 쓰이지 않는다.
...
- **메서드**
    - `from_candles(candles)` : 실제 공급된 모든 캔들의 여는 시각과 닫는 시각의 합집합을 정렬해 `schedule`을 만든다.
...
백테스트 run 전용 설정의 스키마·검증이다. 전략 파라미터 스키마·검증은 소유하지 않고(그건 `core-lib`의
`StrategyConfig` §4.2 소관) 선택값만 담아 넘긴다. signal-service는 별도 `SignalGenerationConfig`를 쓰지만 전략
파라미터는 두 실행면 모두 같은 `StrategyConfig` 경로에서 검증된다.
...
classDiagram
    ...
    class RunConfig {
        ...
        +str fill_timing
        +str profile_ref
        +MoneyManagementConfig money_management
        +Optional~dict~ sweep
        +revalidate() None
        +selection() dict
    }
    class ManualMoneyManagementConfig {
        <<pydantic, extra forbid>>
        +Literal~manual~ mode
        +int leverage
        +float reward_risk
        +float atr_stop_multiple
    }
    class TurtleMoneyManagementConfig {
        <<pydantic, extra forbid>>
        +Literal~turtle~ mode
        +int n_period
        +Literal~1d~ n_timeframe
        +float stop_n_multiple
        +int leverage_cap
    }
    class MoneyManagementConfig {
        <<discriminated union by mode>>
    }
    RunConfig *-- MoneyManagementConfig
    MoneyManagementConfig <|-- ManualMoneyManagementConfig
    MoneyManagementConfig <|-- TurtleMoneyManagementConfig
```

#### `RunConfig`

- **개요** — 백테스트 run 하나의 설정을 담는 검증 스키마(pydantic).
- **책임** — OHLCV·펀딩 소스와 구간, `CostModel` 값, 거래소 규칙, 실행·리스크 설정, 파라미터 스윕, 계열 계산 모드,
  자금관리 mode, 트리거 세밀도, 체결 시점, 프로파일 선택을 검증한 뒤 `Engine`에 주입한다. 전략 파라미터의 스키마 선언·해석·검증은
...
- **필드**
    ...
    - `indicator_mode` — 지표와 패턴 계산 대상의 해석 방식. `{auto, explicit, all}` 중이며 기본은 `auto`다. 필드명은
      호환 때문에 indicator로 남아 있지만 `series_resolution`이 두 레지스트리를 함께 해석한다.
    - `explicit_indicators` — `indicator_mode = explicit`일 때 계산할 지표 또는 패턴을 이름·파라미터 쌍의 목록으로 지정한다.
      explicit 모드에서는 비어 있을 수 없고 그 밖의 모드에서는 비어 있어야 하며, 각 항목은 `name`과 `params` 둘만
      담는다.
    - `trigger_feed` — 스키마 값은 `{tf_candle, m1_subcandle}`이지만 현재 허용되는 실행값은 `tf_candle`뿐이다.
      `m1_subcandle`은 validator가 `NotImplementedError`로 거부한다.
    - `fill_timing` — 스키마 열거에는 `{immediate, next_bar}`가 남아 있지만 현재 `RunConfig` 검증은 `next_bar`만
      허용한다(`decision_ts < execution_ts`).
    - `profile_ref` — 전략 프로파일(§4.2 `StrategyProfile`) 선택.
    - `money_management` — `mode` discriminator로 manual 또는 turtle 설정을 고른다. 기본은 manual이다. manual은
      `leverage`·`reward_risk`·`atr_stop_multiple`, turtle은 `n_period`·고정 `n_timeframe="1d"`·
      `stop_n_multiple`·`leverage_cap`만 허용하며 잉여 키를 거부한다. `money_management`가 없는
      `vessel-reference` legacy 입력은 평면 전략 파라미터의 세 값을 manual 설정으로 정규화한다.
    - `sweep` — 파라미터 스윕 설정. 없으면 단일 run이다.
- **메서드**
    - `revalidate()` : 이미 만들어진 인스턴스에 대해 필드 타입·범위·상호 정합을 다시 검증한다. `start < end`,
      `fill_timing="next_bar"`, 실행 가능한 `trigger_feed="tf_candle"`를 강제한다. `risk_based`는
      `risk_per_trade`만, `pct`는 `position_size_pct`만 허용하고 turtle은 `risk_based`와만 조합된다. 전략 파라미터
      값 검증은 `StrategyConfig`에 위임한다.
...
- **결정성 Evidence 해시** — Evidence 해시는 SQLite 파일 바이트가 아니라 정렬된 행의 정규화 직렬화(wall-clock 제외)로
  낸다. 같은 설정 해시·같은 원천 데이터 해시·같은 seed의 두 run이 같은 해시를 내야 한다. 설정이 같아도 원천
  데이터 해시가 다르면 결정성 비교 대상이 아니다.
...
classDiagram
    ...
    class BacktestEvidenceSink {
        ...
        +audit(require_eval_decision: bool) dict~str,bool~
    ...
    class BacktestCatalogStore {
        ...
        +save_prereg(object) None
        +register(object) str
        +determinism_reference(run_id: str, config_hash: str, source_data_hash: str, evidence_schema_version: str) DeterminismReference
        +upsert_summary(object) None
        +reconcile_orphaned() int
    }
...
- **책임** — run 하나의 무거운 시점별 상세를 run별 SQLite 파일 하나에 적어 파일만으로 자기완결이게 하고,
  ...
  갈린다(기록 주체·시점 규약은 데이터베이스 설계 §5 소유).
    - **실행 중(사실)** — Backtest Run 로컬 사본·Source Data Snapshot·Feature/Indicator Definition·
      ...
      `BACKTEST_RUN_LOCAL`은 `submitted_money_management_json`, 해석된 `money_management_json`, 전략과 정책의
      합성 계열 목록을 보관하고, `params_json._money_management`에도 해석된 정책 신원을 포함한다. 정책이 진입마다
      계산한 변동성·요청 수량·요청 leverage·최초 위험액은 `SIGNAL.metadata_json.money_management`에 기록한다.
...
- **메서드**
    ...
    - `audit(require_eval_decision=False)` : FK·시점·비용·격자·정의 완전성을 검사해 check 이름별 bool을 돌려준다.
    - `source_data_hash()` : 원천 snapshot만으로 결정적 비교 지문을 낸다.
...
- **불변식**
    - **결정성 해시** — 해시는 정렬된 행의 정규화 직렬화로 내며 파일 바이트가 아니고 wall-clock을 제외해 결정적이다.
      ...
      `submitted_money_management_json`은 제출 축약 형태가 달라도 해석된 설정이 같으면 논리 해시가 같도록 해시에서
      제외된다. 해석된 정책 id·판·설정은 해시에 포함된다.
...
- **메서드**
    ...
    - `register(run_meta)` : `run_id`를 `backtest_db` 시퀀스로 단독 발급하고 run 인덱스 헤더를 연다. 반환은
      ...
      레지스트리 조회), `StrategyConfig`가 해석을 마친 확정 파라미터(`params_json`)·해석된 지표 목록
      (`resolved_indicators_json`)과 스키마 버전, 실행 코드
    ...
    - `determinism_reference(run_id, config_hash, source_data_hash, evidence_schema_version)` : 현재 run에 원천 데이터
      해시를 저장하고, 설정·원천·Evidence 스키마 판이 비교 가능한 가장 최근 run을 결정성 대조 대상으로 돌려준다.
      같은 설정이라도 원천 해시가 다르면 `source_changed`, 같은 설정·원천이라도 스키마 판이 다르면
      `evidence_schema_changed`로 구분한다. 같은 조건에서 Evidence 해시가 다르면 `mismatched`로 기록하고 결정성
      검사를 실패시킨다.
...
설계서 본문(§1~§4)과 데이터베이스 설계(§5)가 **무엇을 만들지**를 확정했다면, 이 부록은 그 결과물을 유지
서비스 — `signal-service`(신호 생성)·`wallet-service`(체결·리스크) — 가 **어떻게 받아들이는지**를 확정한다.
채택 지점과 이행 순서, 무중단을 위한 shim 배치, 라이브 강제청산 수신 절차(§4.3.2가 이 부록에 명시적으로 남긴
인계 항목), 회귀 범위와 게이트, 새 백테스트의 검증 기준선(구 백테스트와의 수치 대사 제외 포함), 그리고 비밀
저장 방식 변경까지가 범위다. 프로덕션을 손대는 절차는 이 문서 전체에서 이 부록이 정의하는 것뿐이며, 실제 수행은
구현 단계의 일이다 — 여기서는 절차를 설계한다.

## 부록.1 제약사항·방향

- **프로덕션 접촉은 이 부록의 절차뿐이다.** 설계 단계는 기존 signal/wallet을 읽기 전용으로만 참조하며 어떤
  파일도 바꾸지 않는다. 이 부록이 정의한 절차를 구현 단계가 수행할 때 비로소 프로덕션 코드가 바뀐다.
- **채택의 효과는 표면마다 다르고, 게이트도 표면마다 다르다(§3.3).** 지표·전략 판단(`analyze`) 표면은 계산
  결과가 그대로인 **동작 보존**이라 옛 값과 새 값의 동등성 검사로 닫고, 회계·손익·체결 표면은 새 구현이 계산을
  바로잡는 **정확도 교정**이라 거래소 실측과 맞춘 골든 기준선을 다시 세워 닫는다. 트레일링 표면은 유보 스코프라
  채택이 **건드리지 않는다**(현행 코드 그대로, 기존 테스트 그대로 통과). "기존 회귀가 그대로 통과 = 동작 불변"
  이라는 등식은 보존·무접촉 표면에서만 성립한다.
- **무중단 채택.** 구 import 경로에는 re-export shim(옛 경로가 새 위치의 심볼을 다시 내보내는 얇은 호환 층)을
  남겨, 치환 커밋 하나가 서비스 전체 임포터를 동시에 고치지 않아도 되게 한다. shim에는 로직이 없다 — 로직
  사본은 재복제 가드(§1.2)의 검출 대상이다.
- **라이브 인프라는 wallet·signal 소유로 남는다.** 큐·폴링·HTTP·WebSocket·상태 복구는 채택 후에도 각 서비스
  안에 있다. 채택이 바꾸는 것은 계산·판단·회계의 **구현처**(자체 구현 → `core_lib` import)이지 인프라가 아니다.
- **구 백테스트와의 수치 대사는 수용 기준이 아니다.** 그 제외의 명문화와 대체 검증 기준선은 부록.5가 소유한다.
- **용어.** 이 부록에서 "비밀 저장 방식 변경"은 비밀번호를 **커밋 파일 하드코딩에서 환경 파일 주입으로 바꾸는
  것**을 뜻하며, 비밀 **값 자체의 교체는 포함하지 않는다**(기존 값 유지 — 부록.6).
- **현행 코드 인용의 성격.** 이 부록의 `파일(줄)` 인용은 채택 절차가 손댈 자리를 특정하기 위한 현행 리포 실측
  스냅샷이다. 줄 번호는 리포가 움직이면 밀리므로, 구현 시에는 심볼 이름 기준으로 재확인한다.
...
```mermaid
sequenceDiagram
    participant OPS as 채택 작업(구현 단계)
    participant WS as wallet-service
    participant SS as signal-service
    participant CL as core-lib
    participant G as 게이트(회귀·동등성·골든)
    Note over OPS,CL: 준비 — 백테스트로 검증을 마친 core-lib 위에서 시작한다(§1.2 빌드 순서).
    OPS->>WS: monorepo 이관 + core-lib 의존성 추가(editable)
    OPS->>SS: monorepo 이관 + core-lib 의존성 추가(editable)
    Note over OPS,G: 국면 1 — 내부 구현 치환(표면별 게이트)
    OPS->>WS: 체결·회계·비용·사이징을 core_lib 호출로 치환 + 구 경로 shim
    WS->>G: 전수 회귀(보존·무접촉 표면 그대로 통과) + 회계 골든 재수립(실측 정확성)
    G-->>OPS: 통과 — 차이 전건이 표준 수식으로 설명됨(부록.4)
    OPS->>WS: 강제청산 수신 배선(부록.3) — 판별 분기·catch-up 확장·주기 대조 선-조회
    WS->>G: 강제청산 다섯 수용 케이스(부록.4)
    G-->>OPS: 통과
    OPS->>SS: 전략 구동을 Adapter Manager·StrategyConfig로 치환(레지스트리는 signal_db) + shim
    SS->>G: 활성 전략 analyze 골든(동일 입력 → 동일 신호) + 트리거 변경 확인
    G-->>OPS: 통과
    Note over OPS,G: 국면 2 — 라이브 정합(공급 경로·체결 시점)
    OPS->>SS: 지표 인소싱 — 사전계산 테이블 읽기 폐지, core_lib.indicators 증분 직접 계산
    SS->>G: 지표 동등성(첫 대상 adx_14, 부록.4 허용오차)
    G-->>OPS: 통과
    OPS->>OPS: 수집기 내부화 — OHLCV 적재만(확정 캔들마다 1행·무조건), 지표 역할 폐지
    OPS->>G: 수집기 절체 게이트(부록.4 — 병행 적재 일치·절체 연속성)
    G-->>OPS: 통과
    OPS->>WS: fill_timing 기본값 전환 immediate → next_bar(별도 배포)
    WS->>G: 전수 회귀 재검증 + 전환 기록(부록.4)
    G-->>OPS: 통과
    Note over OPS,CL: 실거래 전환 — core-lib 설치를 editable에서 버전 고정 산출물(wheel)로 바꾼다(§1.2).
```

읽는 법: wallet을 signal보다 먼저 치환한다. wallet의 교정 표면(회계·손익·체결)이 채택 전체에서 가장 무거운
게이트(골든 재수립)를 갖고 있어, 이 게이트를 먼저 통과시켜야 이후 단계의 실패 원인이 좁혀진다. signal의 전략
구동 치환은 활성 전략의 Adaptee가 준비된 뒤에만 가능하다(아래 선행 조건). 국면 2의 각 단계는 독립 배포 단위다 —
특히 `fill_timing` 전환은 체결 시점이라는 프로덕션 동작을 바꾸는 유일한 단계라 반드시 저 홀로 배포하고 저 홀로
재검증한다(부록.4).

**단계 통제(공통).** 위 시퀀스의 각 단계는 시작 전에 커밋으로 기준점을 남기고, 완료 시 커밋과 **코드 리뷰
게이트**(리뷰어 또는 발주자)를 통과한 뒤에만 다음 단계로 넘어간다. 게이트 통과 판정과 이 부록이 "채택 시
확정"으로 미뤄 둔 기준값의 확정은 **발주자가 최종 승인**하며, 판정·승인 결과는 채택 기록(부록.4)에 남긴다.
국면 1의 각 치환 배포가 실패하면 복귀 수단은 **이전 배포본으로의 롤백**이다(치환은 커밋 단위라 되돌릴 수
있다) — shim은 경로 호환 장치이지 복귀 장치가 아니다. 국면 2의 복귀는 각 게이트의 복귀 규정(수집기 복귀·
`fill_timing` 기본값 복원)을 따른다.

**국면 1 선행 조건(전략 구동 치환).** 전략 구동을 `Adapter Manager`로 바꾸려면 그 시점의 **활성 전략 전부가
Adaptee로 구현되어 있고** 각각의 analyze 골든(현행 전략과 동일 입력에서 동일 신호)이 통과해야 한다. 판단 내부는
전략 작성자 소유이므로 이 재구현은 작성자 몫이고, 채택 절차는 골든 통과를 확인만 한다. 활성 전략 중 트레일링을
쓰는 전략이 있으면 트레일링 유보를 먼저 해제해야 한다(유보 상태의 채택은 트레일링 무접촉이 전제다). 비활성
전략은 레지스트리 등록만 하고 Adaptee 구현을 뒤로 미룰 수 있다. 레지스트리 적재 자체 — 현행 코드 상주
목록(`main.py`의 전략 클래스 16개 직수입)을 `signal_db` 레지스트리 행으로 등록하는 작업(테이블 구조는
§5.1.3) — 는 이 치환 단계에서 채택 작업이 수행한다.

**치환 지점.** 표의 현행 위치는 실측 스냅샷이다(부록.1 인용 규약).

| 서비스 | 표면 | 현행(치환 전) | 채택 후 | 효과·게이트 |
|---|---|---|---|---|
| signal | 지표 로딩 | `application/scheduler/strategy_executor.py`의 `IndicatorLoader.load_latest` 지연 import(943행)·생성(952행) → `technical_indicators` 테이블 읽기 | 국면 1에서는 유지, **국면 2**에서 `core_lib.indicators` 증분 직접 계산으로 치환 | 보존 — 지표 동등성(부록.4) |
| signal | 전략 구동 | `StrategyFactory.create_from_db`(`application/scheduler/strategy_executor.py` 464행)·`registry.py` 수동 등록(`main.py` 84~99행, 전략 클래스 16개 직수입) | `Adapter Manager.create`(생성) + `StrategyConfig.resolve`(파라미터 해석) + `StrategyRegistry`(live) 어댑터 — 전략 목록은 `signal_db`의 재정의된 레지스트리 테이블(§5.1.3) | 보존 — 활성 전략 analyze 골든 |
| signal | 판단 트리거 | 분당 폴링 게이트 `check_interval_minutes`(`strategy_executor.py` 477행) | 전략 타임프레임 캔들 마감마다 판단(폴링 게이트 제거) | 구동 방식 변경 — 판단 입력·산출은 캔들 마감 기준으로 동일함을 골든이 함께 확인 |
| wallet | 사이징 | `Wallet.calculate_available_for_investment`(`domain/entities/wallet.py` 123행; 호출 `futures_trade_service.py` 208행 등 3곳) | `core_lib.sizing`의 pct 호환 경로(현행과 같은 방식·같은 값 — §4.3.3 `WalletPct`) | 보존 — pct 경로는 1R≤1% 미보장 시 비준수 플래그를 남김. `RiskMoney` 전환은 채택 범위 밖(별도 운영 결정) |
...
것은 여전히 `fill_timing` 전환 하나뿐이다.

트리거의 정의는 **벽시계가 아니라 데이터 가용**이다 — 마감 시각 T에 발화하는 것이 아니라, **T 캔들(그리고
상위 타임프레임이면 그 집계 행)이 저장소에 적재 확인된 뒤** 판단한다. 수집기 적재가 몇 초~수십 초 늦거나 집계
...
**국면 2 보충 — 지표 인소싱의 재기동 워밍업.** 증분 계산 상태는 메모리에 있으므로 signal-service가 재기동하면
사라진다. 기동 시 이력 preload로 seed한 뒤 워밍업 구간의 판단을 버린다(백테스트의 preload·discard와 동형).
지표 동등성 게이트는 정상 상태만 검증하므로, 이 규약이 없으면 재기동 직후의 판단이 미성숙 지표 위에서 샌다.

**배선 주의(실측 발견).** `application/services/futures_paper_trading_service.py`의
`FuturesPaperTradingService`는 현행 트리에서 **임포터가 0건**이다(테스트 포함 어디서도 참조 없음 — 미배선).
치환·shim 대상이 아니며, 폐기 목록의 폐지 분류와 정합함만 확인한다.

**shim 배치 규약.** shim은 두 형태만 허용한다.
...
|---|---|---|
| signal `domain.indicators.technical`(`TechnicalIndicators`; 임포터 `domain/indicators/__init__.py` 6행·`domain/strategies/base.py` 18행) | 순수 재노출 또는 위임 | `core_lib.indicators` 동형 심볼 |
| signal `domain.indicators.extended`(개별 함수 직수입; 임포터 `alex_sr_strategy.py` 21행 등 전략 다수) | 순수 재노출 | `core_lib.indicators` 동형 함수 |
| wallet `domain.services.futures_calculator`(임포터 8곳) | 위임 shim | `core_lib.execution.position_book`·`core_lib.costs` |
| wallet `domain.services.slippage_calculator`(임포터 3곳) | 위임 shim | `core_lib.costs.slippage` |
| wallet `application.services.paper_trading_service`·`sl_tp_monitor_service`(임포터 각 3곳·1곳, 테스트 제외 기준) | 위임 shim | `core_lib.execution.matcher`·트리거 판정. 단 `sl_tp_monitor_service`가 수행하는 트레일링 상태 정리는 무접촉 표면(트레일링 유보)이라 위임하지 않고 서비스에 잔류한다 |

**shim 제거.** 제거 시점의 기준은 둘이다 — 국면 1·국면 2의 게이트가 모두 통과했고(부록.4), 내부 임포터의 새
경로 이전이 끝나 옛 경로의 임포터가 0건임이 확인(전역 grep 또는 import 규칙 검사)됐을 때. 두 조건이 차면
**별도 커밋**으로 shim을 제거한다. shim이 남아 있는 동안에도 재복제 가드는
로직 사본만 검출하므로 shim 자체는 걸리지 않는다.

**의존성·배포.** 두 서비스의 core-lib 의존은 페이퍼 운영까지 editable(최신 소스 추종)로 두고, **실거래 전환
시점에 태그에서 빌드한 버전 고정 산출물(wheel)로 바꾼다**(§1.2 거버넌스). 이후 어떤 core-lib 수정도 새 버전을
일부러 배포하기 전에는 실거래 경로에 닿지 않으며, `core_lib_version` 메타가 "백테스트가 검증한 버전 = 실거래가
도는 버전"의 대조 근거다.
...
Stream을 WebSocket으로 이미 수신한다: `infrastructure/external/binance_websocket_client.py`가 연결·listenKey
관리(`_start_user_data_stream` 240행, 30분 주기 갱신)와 이벤트 라우팅(`_parse_message` 135~169행 —
`ORDER_TRADE_UPDATE`·`ACCOUNT_UPDATE`·`MARGIN_CALL`)을 갖고, LIVE 모드에서 `main.py`(526~541행)가 주문·계좌
콜백을 배선한다. REST 폴링 폴백(`order_polling_service.py`, 5초 주기)도 있다. 그러나 강제청산은 세 곳 모두에서
끊긴다.

1. **이벤트 드롭** — 주문 이벤트 처리기 `application/services/binance_order_event_handler.py`의
   `_process_sl_tp_fill`(292행)은 로컬 DB에서 주문을 찾지 못하면 건너뛴다(367~381행). 거래소가 개시한 강제청산
   주문은 우리가 낸 주문이 아니라 로컬에 없으므로 **전부 드롭된다**(청산 식별 분기 자체가 전역에 없음).
2. **LIVE 미구현** — 청산 감시 서비스 `application/services/liquidation_guard_service.py`의
   `_handle_liquidation`(458~493행)은 PAPER면 시뮬 청산을 수행하지만 LIVE면 오류 로그와 수동 개입 알림만 남긴다.
3. **회계 없는 정리** — 주기 대조 서비스 `application/services/balance_sync_service.py`(300초 주기 `sync`,
...
```mermaid
sequenceDiagram
    participant EX as 거래소(Binance)
    participant WSC as WS 클라이언트<br/>(binance_websocket_client)
    participant H as 주문 이벤트 처리기<br/>(강제청산 분기 신설)
    participant EXE as core_lib.execution<br/>(position_book·accounting)
    participant DB as wallet_db
    participant EV as 라이브 Evidence(연구 피드백)
    Note over EX,H: (a) 정상 수신 — 거래소가 집행하고 우리는 받는다
    EX->>WSC: ORDER_TRADE_UPDATE(청산 주문 체결)
    WSC->>H: handle_order_event(event)
    H->>H: 강제청산 판별(로컬 미발주 + reduce-only 체결 + 거래소 청산 식별자)
    H->>H: 멱등 검사(청산 주문 id 중복 처리 방지)
    H->>EX: (b) 계좌 수입 이력 REST 조회 — 청산 정리 수수료
    EX-->>H: 정리 수수료 항목(지정 채널 조회 후 없으면 0)
    H->>EXE: Fill(exit_reason=LIQUIDATION, …) 전달 — (b) 변환
    EXE->>EXE: 포지션 축소·회계 반영(백테스트와 동일 소비 경로)
    EXE->>DB: Trade(liquidated=TRUE, liquidation_penalty≥0) 기록
    H->>EV: (d) 실측 대사 — 실측 청산가·수수료 vs costs.Liquidation 추정 차이 기록
    Note over EX,DB: (c) 누락 복구 — 다운타임·재접속으로 놓친 청산을 뒤늦게 반영
    EX--xWSC: (연결 단절 구간의 청산 통지 유실)
    WSC->>H: 재접속·기동 catch-up 트리거
    H->>EX: 열린 포지션 심볼의 강제 주문 전용 조회(창 분할 반복)
    EX-->>H: 청산 주문 발견
    H->>EXE: 같은 판별·멱등 검사 후 Fill 소급 생성 — (b)와 동일 경로
    Note over H,DB: 조회로도 원인 불명이면 현행대로 포지션 zeroing + CRITICAL 알림(수동 개입) — 추정으로 장부를 채우지 않는다
```

**(a) 수신 경로.** 새 수신기를 만들지 않는다 — 이미 있는 `ORDER_TRADE_UPDATE` 콜백 체인(`handle_order_event`,
187행)에 **강제청산 분기를 신설**한다. 판별 규칙은 세 조건의 동시 성립이다: ① 이벤트의 주문이 로컬 DB에 없고
(우리가 낸 주문이 아님), ② **체결 방향이 보유 포지션을 줄이는 방향**이며 — 이벤트의 reduce-only 플래그
리터럴에 의존하지 않는다(거래소가 자동 청산 주문에 그 플래그를 세우지 않아도 판별이 성립해야 한다; `Fill`의
`reduce_only = TRUE`는 판별 결과로 우리가 세팅하는 값이다) — ③ 거래소가 청산 주문에 부여하는
식별자와 일치한다. ③의 정확한 식별자 집합(Binance 선물이 자동 청산 주문에 부여하는 client order id 규칙)은
구현 시 거래소 명세로 확정하며, 판별이 확신에 이르지 못하면 자동 회계 대신 수동 개입 알림으로 보수 처리한다.
식별자 확정 범위에는 **자동 감축(ADL — 거래소가 반대편 파산을 메우려 상대 포지션을 강제로 줄이는 사건)의
식별자도 포함**한다 — 단 첫 스코프에서 ADL은 강제청산으로 수렴시키지 않고 "ADL로 식별됨"이라는 구분된 수동
개입 경보로 보낸다(판별 불확신과 다른 분류). 청산과 회계 성격이 달라(파산가 아님·페널티 구조 다름) 자동
수렴은 별도 결정으로 미루는 보수 처리이며, 이것이 의도다. 경보 후 사람이 조치할 때까지 장부는 거래소와
어긋난 상태로 남는다 — 주기 대조는 포지션이 통째로 사라진 경우만 검출하므로, ADL의 부분 감축 불일치는 이
경보가 유일한 신호다.
`MARGIN_CALL` 이벤트는 청산 **예고**로만 소비한다(경보 — 회계 아님; 현행은 소비자 미배선). 5초 주기 청산 감시
...
| `Fill` 필드 | 라이브 강제청산에서의 값 |
|---|---|
| `order_id` | 거래소가 발행한 청산 주문 id(비어 있지 않음 — §4.1.1) |
| `symbol`·`side`·`position_side`·`quantity`·`timestamp` | 통지의 실측값(시각은 거래소 체결 시각 — wall-clock 아님) |
| `price` | 실측 평균 체결가 |
| `reference_price` | 실측 평균 체결가와 동일 — 라이브 실측이 이미 최종가라 모델 슬리피지를 얹지 않으며, 따라서 구성 슬리피지(체결가와 기준가 차이 × 수량 — §4.1.1의 검산식)는 0이다. 추정 청산가를 여기 넣지 않는다(모델 추정은 (d) 대사에서 실측과 **대조**하는 값이지 회계 입력이 아니다) |
| `fee` | 거래소 통지의 실측 수수료. **수수료 자산이 정산 통화(USDT)가 아니면**(예: BNB 수수료 차감 계정) 자동 회계를 만들지 않고 수동 개입으로 보낸다 — 자산 환산을 회계 경로에 끼워 넣지 않는다 |
| `slippage` | 0(위 `reference_price` 규약의 귀결) |
| `liquidity` | `taker`(강제청산은 시장가 집행 — §4.1.1) |
| `exit_reason` | `LIQUIDATION` |
| `reduce_only` | TRUE(§4.1.1 — 강제청산은 항상 포지션 축소) |
| `gap_filled`·`qty_truncated` | FALSE(기본값 — 갭·절삭은 백테스트 `Matcher`만 아는 사실, §4.1.1) |

**`Trade.liquidation_penalty`의 라이브 계상 규칙(수송 채널 지정·이중 차감 금지).** net 등식의 다른 항과 달리
이 항은 주문 이벤트 필드로 직접 오지 않는다. 더 중요하게, **청산 정리 명목의 금액(청산 수수료·보험기금으로
가는 몫)은 주문 이벤트가 아니라 다른 채널로 보고될 수 있다** — Binance에서는 계좌 수입 이력의 별도 수입
유형(보험기금 정산)으로 남는 것으로 알려져 있다. 그래서 계상 규칙은 채널까지 지정한다.

- **일반 체결 수수료(taker)** — 주문 이벤트의 수수료 필드에서 받아 `Fill.fee`(→ `Trade.total_fee`)로만 계상한다.
- **청산 정리 수수료(penalty)** — 청산 `Fill`을 처리할 때 **계좌 수입 이력 REST 조회**로 해당 청산의 정리 수수료
  항목을 찾아 `liquidation_penalty`로 계상한다(심볼·시각 근접으로 대응 — 주문 id 연결 키가 명세에 없으면 시간창
  매칭; 정확한 수입 유형 이름·연결 키는 구현 시 거래소 명세로 확정, 부록.4 확정 목록). **지정 채널을 조회한
  뒤에도 해당 항목이 없을 때에만 0**으로 둔다 — 0은 "언제나 0 이상" 제약과 정합하지만, 채널 조회 없이 0으로
  두는 구현은 이 규칙 위반이다(penalty가 항상 0으로 새면 장부 잔고가 거래소 실제보다 체계적으로 커지고,
  부록.4 실측 대사의 차이 0 게이트가 실패한다).
- **이중 차감 금지** — 같은 금액을 `total_fee`와 `liquidation_penalty` 두 곳에 넣는 것은 금지한다(net 등식에서
  같은 비용이 두 번 차감된다). 수입 이력의 정리 수수료와 주문 이벤트의 체결 수수료가 같은 금액의 다른 보고인지
  별개 항목인지도 구현 시 명세·실측으로 확정한다.

수치는 거래소가 문자열로 주므로 `Decimal(str(x))` 관문을 자연 통과한다(§4.3.1 normalizer 규약과 정합). 결과
거래에는 `Trade.liquidated = TRUE`·`liquidation_penalty ≥ 0`이 남는다(§4.1.1). **생성 시점 — 종결 상태
이벤트에서만.** 거래소 주문 이벤트는 부분 체결마다 올 수 있으므로, `Fill`은 주문이 종결 상태(전량 체결 —
누적 체결 수량과 평균 체결가가 확정된 이벤트)에 이르렀을 때 **한 번만** 만들고 중간 부분 체결 이벤트에서는
만들지 않는다. 그래야 아래 주문 id 멱등과 정합한다(첫 부분 체결에서 멱등 처리되어 잔여 체결이 유실되거나
중복 회계되는 여지를 없앤다). 주문이 일부만 체결된 채 다른 종료 상태(취소·만료 등)로 끝나면, 종료 상태 도달
시점의 누적 체결 수량이 0보다 클 때 그 누적 수량·평균 체결가로 1회 생성하고 0이면 생성하지 않는다 — 부분
체결이 유실되지도, 전량 체결을 기다리며 영영 미생성되지도 않는다. **멱등성** — 같은 청산 주문 id를 두 번 처리하지 않도록 기록 전 중복 검사를
둔다(현행 `exists_by_exit_order_id` 패턴, `binance_order_event_handler.py` 521행의 계승).

**`wallet_db` 스키마 인계(데이터베이스 설계 §5.1.4의 위임 수령).** 데이터베이스 설계는 `wallet_db`를 무변경으로
선언하되 "공유 라이브러리 채택으로 생길 수 있는 스키마 변경은 채택 단계에서 다룬다"고 이 부록에 위임했다. 그
변경 목록을 여기서 확정한다 — 거래 기록 테이블(`closed_trades`)에 두 컬럼을 추가한다: `liquidation_penalty`
(기존 금액 컬럼과 같은 타입 규약, NOT NULL, 기본 0 — 이 절의 계상 규칙이 채우는 값)와 `r0`(같은 타입 규약,
NULL 허용 — 최초 위험, NULL이면 R 기반 지표에서 제외라는 §4.1.1 규약 그대로). 현행 스키마에는 `liquidated`
불리언만 있고 이 두 컬럼이 없다(실측: `init-scripts/02-init-wallet-db.sql` 401행 전후). 마이그레이션은 기존
규약대로 `init-scripts/wallet-service/`의 날짜 하위 디렉터리에 둔다. 그 밖의 값 타입 변화가 추가 컬럼을
요구하면 같은 위임 아래 채택 시 확정해 채택 기록에 남긴다.

**(c) 누락 복구.** 두 층으로 복구한다. **기동·재접속 catch-up** — 현행 catch-up(`catch_up_pending_orders`,
704~777행)은 로컬에 있는 손절·익절 주문만 조회하므로 강제청산을 못 본다. 여기에 **열린 포지션 심볼의 주문 이력
REST 조회**를 더해, 단절 구간에 거래소가 낸 청산 주문을 찾아 (a)와 같은 판별·멱등 검사를 거쳐 `Fill`을 소급
생성한다. 조회 엔드포인트는 **강제 주문 전용 조회**를 쓴다 — 거래소에 따라 자동 청산·자동 감축 주문이 일반
주문 이력에 나타나지 않을 수 있어(Binance에는 강제 주문 전용 조회가 따로 있고 청산과 자동 감축을 구분해
돌려준다), 일반 주문 이력만 조회하는 구현은 단절 구간의 청산을 영영 못 찾고 매번 수동 개입으로 떨어진다.
전용 조회에는 요청당 시간창 제한이 있으므로, 워터마크부터 현재까지를 **엔드포인트의 최대 창 단위로 쪼개 반복
조회**한다(정확한 엔드포인트·창 크기·보관 한계는 구현 시 거래소 명세로 확정, 부록.4 확정 목록. 단일 심볼이라
레이트리밋은 문제가 아니다). 조회 창의 기준점은 **마지막으로 정상 처리한 주문 이벤트의 거래소 시각**(그 이후부터 현재까지)이다.
이 워터마크는 메모리가 아니라 영속 기록에서 도출한다 — `wallet_db`에 남은 주문·체결 기록의 거래소 시각
최댓값을 쓰고, 기록이 없으면 열린 포지션의 진입 시각으로 창을 연다(재기동·크래시 후에도 창이 열린다).
거래소 이력 조회의 보관 한계를 넘는 구간(장기 다운타임)은 이 층이 복구할 수 없으므로, 아래 주기 대조의
zeroing + CRITICAL 알림 경로로 수렴한다(수동 개입). **주기 대조** — 300초 주기 포지션 대조(`balance_sync_service.py`)의
대조 범위를 "포지션 존재"에서 **수량 일치**로 넓힌다 — 포지션이 통째로 사라진 경우만 보면 자동 감축의 부분
감축과, 소켓이 죽은 줄 모르는 구간에 놓친 부분 청산이 같은 구멍으로 빠진다(재접속 catch-up이 결국 복구하더라도
감지 트리거가 재접속뿐이면 드리프트 구간이 길어진다). 존재 소멸이든 수량 불일치든 발견하면, 현행처럼 곧바로
zeroing하지 않고 **먼저** 같은 강제 주문 조회로 원인 청산 주문을 찾는다. 찾으면 (b)로 정상 회계 처리하고,
못 찾으면 현행대로 zeroing + CRITICAL 알림으로 남긴다(수동 개입 — 원인을 모른 채 추정으로 장부를 채우지 않는다).

**수동 개입의 책임·기록.** 이 절의 경보 — 판별 불확신((a)), ADL 식별((a)), zeroing + CRITICAL((c)) — 와
부록.4의 상시 경보(펀딩 간격 이탈 등)는 현행 운영 알림 채널로 보내며, 대응 책임은 운영자(발주자)에게 있다. 조치 내용은 채택 기록(부록.4)에 남긴다.
**장부-거래소 불일치가 해소되기 전에는 해당 심볼의 신규 진입을 중지한다** — 어긋난 장부 위에서 사이징·리스크
계산을 굴리지 않기 위해서다.

**(d) 실측 대사.** 청산 `Fill`을 처리할 때마다(소급 포함) 실측값과 모델 추정의 차이를 기록한다 — 실측 청산
체결가·수수료 대 `costs.Liquidation.price(entry, leverage, mmr)` 추정(§4.3.2: `Entry×(1−1/lev+mmr)`,
last-price 근사, mmr 기본 0.4%). 용도는 백테스트 청산 근사가 **보수 방향**(청산 과대 검출)임을 실측으로 계속
확인하고, mmr 티어·모델 보정의 입력을 쌓는 것이다. 이 대사는 **거래소 실측과의 대조**이며, 부록.5가 제외하는
구↔신 백테스트 수치 대사와는 별개다 — 제외되지 않는다.

기록의 실체는 다음과 같이 확정한다. **기록처는 wallet-service가 소유하는 독립 라이브 Evidence SQLite 파일**
(append-only, 경로는 wallet 설정으로 주입)이다 — 연구 피드백 성격의 데이터라 운영 DB(`wallet_db`)에 넣지
않는다(연구 데이터·운영 DB 분리 불변식). 이것이 라이브 쪽 `EvidenceSink` 구현(§1.2가 채택 설계로 위임한
live/paper 포트 구현 중 하나)의 최소 시작 범위다 — 첫 스코프에서는 이 청산 대사 레코드 하나만 적고, 라이브
Evidence의 확장은 별도 결정으로 미룬다(과설계 방지). 레코드 필드:

| 필드 | 내용 |
|---|---|
| `symbol` · `position_side` | 청산된 포지션의 심볼·방향 |
| `liq_order_id` | 거래소가 발행한 청산 주문 id(멱등 키 — (b)와 동일) |
| `exchange_ts` | 거래소 체결 시각 |
| `measured_price` · `measured_fee` | 실측 평균 체결가·실측 수수료 |
| `est_liq_price` | `costs.Liquidation.price` 추정가 |
| `est_inputs` | 추정 입력 — `entry`·`leverage`·`mmr`(주입 기본값과, 확인되면 그 시점 심볼의 실제 티어 값을 병기 — 기본값이 실제 티어보다 낮으면 대사가 반보수 방향으로 읽히는 오해를 막는다) |
| `price_diff_abs` · `price_diff_rel` | 실측 대 추정의 절대·상대 차이 |
| `backfilled` | (c) 누락 복구로 소급 생성된 건이면 TRUE |

컬럼의 저장 표기는 Evidence SQLite의 공통 저장 규약(§5.3.1)을 준용한다 — 금액·가격(실측 체결가·수수료·추정
청산가)은 스케일 정수, 시각은 epoch 밀리초 정수, 참·거짓은 0/1 정수, 상대 차이 같은 비율은 배정밀도
부동소수점(REAL). 파일은 삭제·덮어쓰기 없이 보존하고(연구 피드백의 원천), 운영 백업 대상에 포함한다.
...
| 표면 | 범위 | 게이트 | 통과 기준 |
|---|---|---|---|
| 보존 — signal 지표·`analyze` | 지표 값, 활성 전략의 판단 산출 | 동등성·골든 | 옛 값 = 새 값(아래 허용오차), 동일 입력 → 동일 신호 |
| 교정 — wallet 회계·손익·체결·비용 | 실행·비용·사이징 커버 약 262건 중 유보 제외 분 | 골든 재수립 | 거래소 실측 대비 정확성(아래 재수립 규약) |
| 무접촉 — 트레일링·15분 폴링 | 약 45건 | 기존 테스트 그대로 | 무수정 통과(채택이 이 표면을 건드리지 않음) |
| 인프라·기타(큐·상태기계·API 등) | 나머지 전수 | 기존 테스트 그대로 | 무수정 통과 |

**교정 표면의 골든 재수립 규약.** `core_lib`가 회계를 바로잡으므로 옛 기대값을 단언하던 테스트는 실패하는 것이
정상이다. 재수립은 다음 규칙을 따른다.

1. **검증 로직은 보존하고 기대값만 갱신한다.** 테스트가 검사하는 성질(항등식·경로·상태 전이)은 그대로 두고,
   수치 기대값만 새 표준 수식의 값으로 바꾼다.
2. **차이 전건이 표준 수식으로 설명 가능해야 한다.** 옛 기대값과 새 값의 차이는 net 등식
   (`x_net = x_gross − fee_entry − fee_exit − slippage − funding − liquidation_penalty`), 비용 1회 차감, 이산
   펀딩(UTC 0/8/16 경계·과거 실측 rate), 회계 항등식(`cash + position = equity`) 중 하나로 설명되어야 한다.
   설명 불가 차이는 기대값 갱신이 아니라 **버그 후보**로 조사한다. 설명은 말이 아니라 기록이다 — 갱신된
   기대값마다 "어느 수식이 차이를 만드는지"를 적은 **차이 설명 대장**을 채택 기록의 일부로 남겨, 제3자가
   갱신의 정당성을 사후에 판정할 수 있게 한다.
3. **최종 기준은 거래소 실측이고, 대사 기준은 다음과 같다.** 재수립된 골든의 수용 기준은 옛 코드와의 일치가
   아니라 실측 대비 정확성이며, 회계가 Decimal 전용(이진 오차 없음)이므로 기준을 느슨하게 둘 이유가 없다.
   - **결정 경로(페이퍼 시뮬 체결·수수료·펀딩·손익 수식)** — 표준 수식으로 재계산한 기대값과 금액 자릿수
     (소수 8자리, 짝수 반올림 — `money` 규약)에서 **차이 0**.
   - **실측 경로(라이브 기록)** — 항목의 성격에 따라 둘로 가른다. **보고값을 그대로 기록하는 항**(실측
     체결가·수수료·펀딩 정산액)은 거래소 보고값을 같은 자릿수로 반올림한 값과 **차이 0**이 항등적으로
     성립해야 한다. **우리가 재계산하는 항**(실현 손익 — 평균단가·순차 체결 처리를 우리 수식으로 계산)은
     시작 기준은 같은 차이 0이되, 거래소 내부 반올림 규약과의 차이가 실측으로 확인되면 그 항에 한해 채택 시
     기준을 확정·기록한다(아래 조정 조항을 처음부터 이 항에 밟을 수 있음을 예고해 둔다). 모델 추정(슬리피지·
     청산가)은 회계 입력이 아니므로 이 대사에 끼지 않는다.
   - 차이 0을 만족하지 못하는 항목은 허용오차를 넓혀 통과시키지 않고 규칙 2의 버그 후보로 조사한다. 거래소
     보고 지연·정밀도 차이 같은 실무 사유로 기준을 조정해야 하면, 조정값과 사유를 채택 시 이 절의 값으로
     확정·기록한다(지표 동등성 게이트와 같은 처리) — 라이브 손익 불일치의 재발을 여기서 막는다.

**지표 동등성 게이트(국면 2).** 동일 확정 OHLCV 구간에서 옛 사전계산 테이블(`technical_indicators`)의 값과
`core_lib.indicators` 증분 계산 값을 대조한다. **첫 대상은 `adx_14`**(재귀 평활이라 seed 차이가 가장 잘
드러나는 대표)이고, 이후 활성 전략의 `required_indicators` 전부로 넓힌다. 게이트는 두 겹이다.

- **수치 게이트** — 비교 시작점은 각 지표 **기간의 10배**(최소 `min_history`) 이후 구간이다 — 옛 값은 수집
  개시 이래 전 이력으로 평활된 값이라 재귀 평활(Wilder 이중 평활 등)의 seed 감쇠가 느리고, `min_history`
  직후 구간의 차이는 구현 차이가 아니라 seed 잔향이기 때문이다(불필요한 조사 루프 방지). 그 이후 구간에서
  `|old| > 1e-8`인 점은 상대 오차 `|new − old| / |old| ≤ 1e-4`, 그 밖의 점은 절대 오차 `≤ 1e-8`. 이 시작값은
  보수 가정이며(옛 구현의 seed 규약이 다를 수 있어 0이 아닌 여유를 둔다), 채택 시 실측 seed 감쇠를 보고 이
  절의 값을 좁혀 확정·기록한다.
- **판단 게이트(우선)** — 같은 구간에서 활성 전략의 신호 산출이 동일해야 한다. 수치 게이트를 통과해도 신호가
  갈리면 실패이고, 수치 차이가 있어도 신호가 전 구간 동일하고 차이가 seed 감쇠로 설명되면 통과다.

**수집기 절체 게이트(국면 2 — 외부 수집기 → 내부 수집기).** "수집기를 바꿔도 값은 같다"는 가정이 아니라 검증
대상이다 — 적재 불변식(확정 캔들마다 1행·무조건)은 연속성 검사일 뿐 값 동등성을 보지 않고, 지표 동등성
게이트는 절체 **전에** 이미 닫히므로, 내부 수집기가 미묘하게 다른 OHLCV를 적재하면 어떤 게이트도 잡지 못한다.
절체는 셋으로 닫는다.

1. **병행 적재 일치** — 구·신 수집기를 같은 구간에 동시 적재하고(병행 창 시작값 **최소 24시간 = 1분봉
   1,440행**; 채택 시 확정·기록), 동일 구간 행이 **전 컬럼 차이 0**으로 일치해야 한다 — 확정 캔들은 거래소가
   확정한 값이라 반올림 여지가 없다. 비교 범위는 OHLCV 1분봉만이 아니라 수집기가 적재하는 **펀딩 실측 요율
   행도 포함**한다(8시간 간격이라 병행 창 24시간에 3행 수준 — 펀딩 적재가 조용히 죽으면 net 손익의 실측 요율
   주입이 무너진다). 병행 창 동안 신 수집기는 비교 전용 위치에 적재한다 — 병행 창이 끝나면 버리는 일시
   산출물이며(저장소의 "스키마 변경 없음" 선언과 충돌하지 않는다), 운영 테이블 이중 적재는 "확정 캔들마다
   1행" 불변식과 충돌하므로 하지 않는다.
2. **절체 연속성** — 절체 시각 전후로 OHLCV는 1분당 1행, 펀딩 요율은 정산 간격당 1행이 유지되어야 한다
...
3. **자가 복구 시연** — 병행 창은 정상 경로만 보므로, 병행 창 중 신 수집기를 일부러 짧게(예: 10분) 중단했다
   ...
   캔들마다 1행" 불변식이 깨진다.
4. **복귀 경로** — 게이트 실패 시 구 수집기로 되돌린다. 구 수집기는 절체와 동시에 폐기하지 않고 병행 창
   통과 후에만 은퇴시킨다.

적재된 확정 캔들은 **불변으로 간주한다** — 거래소 정정에 따른 재수집·수정 경로는 두지 않는다("확정 캔들마다
1행·무조건"과 저장소 무변경 선언의 귀결이며, 이것이 의도다). 병행 창 중 미세 불일치가 나면 정정 경로를 만들
일이 아니라 게이트 실패로 다루고 원인을 조사한다.

**`fill_timing` 전환 게이트(국면 2 마지막).** 체결 시점 기본값을 즉시(immediate)에서 다음 캔들 시가(next_bar)로
바꾸는 것은 채택에서 유일하게 **프로덕션의 체결 동작을 의도적으로 바꾸는** 단계다(`decision_ts < execution_ts`
통일. 판단 트리거 변경은 판단 산출을 보존하되 방출 시각이 앞당겨지는 수용된 실행-시점 변화이고(부록.2),
강제청산 수신은 없던 회계 기록을 새로 더하는 것이라 성격이 다르다). 규약:

- **전환 조건** — 국면 1·국면 2의 앞선 게이트가 모두 통과한 뒤에만, **저 홀로** 배포한다(다른 변경과 섞지 않는다).
- **재검증 트리거** — `fill_timing` 기본값을 바꾸는 모든 커밋은 wallet 전수 회귀 재실행을 의무로 갖는다. 통과
  없이 배포 금지. 이 규정을 규정으로만 두지 않는다 — 자동 빌드 절차가 없는 환경이므로, **기본값이 `next_bar`인데
  채택 기록에 전환 항목이 없으면 실패하는 저비용 가드 테스트**를 wallet 테스트에 둔다(재복제 가드와 같은 로컬
  가드 방식).
- **기록** — 서비스는 기동 시 유효 `fill_timing`과 `core_lib_version`을 기동 로그(INFO)에 남기고, 전환 수행
  시각·전환 커밋·재검증 결과는 채택 기록(아래 완료 확인 목록)에 남긴다. 전환 이후의 거래는 체결 시각이 결정
  시각보다 뒤라는 사실 자체로 전환 전 거래와 구분된다. 전환 전후의 페이퍼 손익 시계열은 체결 의미가 달라
  비교 불가능한 불연속이 생기므로, **전환 경계를 넘는 성과 비교 금지** 주석을 채택 기록의 전환 항목에 함께
  남긴다.

**강제청산 수신 게이트(국면 1 — 부록.3의 배선을 닫는다).** 강제청산 수신은 채택에서 유일하게 **새로 만드는**
프로덕션 회계 경로이므로, 배선 확인이 아니라 수용 케이스 통과로 닫는다. 다섯 케이스가 전부 통과해야 한다.

1. **정상 변환** — 합성 청산 통지(체결 종결 상태)를 주입하면 `Fill(exit_reason=LIQUIDATION)`이 부록.3 (b)의
   필드 대응 그대로 만들어지고, 회계에 `Trade.liquidated = TRUE`가 남으며, `liquidation_penalty`는 (b)의 계상
   규칙을 **두 갈래로** 검증한다 — 수입 이력 stub에 정리 수수료 항목이 **있는** 갈래에서는 그 금액이 penalty로
   계상되고 `total_fee`와 중복 계상되지 않으며, 항목이 **없는** 갈래에서는 지정 채널 조회가 실제로 일어난 뒤
   0이 된다(조회를 배선하지 않은 구현은 이 갈래에서 걸린다). 회계 항등식(`cash + position = equity`)이 유지된다.
2. **소급 생성** — 단절 구간에 발생한 청산을 재접속 catch-up이 주문 이력 조회로 찾아 같은 `Fill`을 소급
   생성하고, 대사 레코드에 `backfilled = TRUE`가 남는다(`backfilled`는 `Fill`이 아니라 부록.3 (d) 대사
   레코드의 필드다).
3. **보수 처리** — 판별이 확신에 이르지 못하는 통지(식별자 불일치·정보 부족)는 자동 회계를 만들지 않고 수동
   개입 알림으로만 남는다.
4. **멱등** — 같은 청산 주문 id의 통지를 두 번(수신 + 소급, 또는 재전송) 넣어도 회계는 정확히 한 번만 반영된다.
5. **선-조회** — 주기 대조가 포지션 소멸 또는 수량 불일치를 발견했을 때, 원인 청산 주문이 있으면 정상 회계로,
   없으면 zeroing + CRITICAL 알림으로 갈라진다.

다섯 케이스의 합성 픽스처는 가능하면 **거래소 테스트넷에서 실물 청산 1회를 일으켜 채집한 실측 payload**로
만든다 — 최소 증거금·최대 레버리지 포지션을 테스트넷에서 일부러 청산시키면 실제 이벤트 시퀀스(부분 체결 형태·
필드 이름·식별자)를 저비용으로 얻는다. 이 배선이 딛고 선 "구현 시 확정" 항목들(식별자 집합·penalty 수입 유형·
전용 조회 창)을 실측으로 닫는 가장 값싼 방법이며, 드릴 결과는 채택 기록에 남긴다.

**재복제 가드.** 치환 뒤 `core_lib` 밖에 표준 모듈의 로직 사본이 남지 않았는지 가드 테스트(§1.2 — 파일·심볼
스캔 또는 import 규칙 검사)로 확인한다. 위임 shim은 로직이 없어 걸리지 않는다.

```mermaid
flowchart TD
    A["국면 1 — wallet 치환"] --> G1{"전수 회귀<br/>보존·무접촉·인프라 무수정 통과<br/>+ 교정 표면 골든 재수립"}
    G1 -->|"설명 불가 차이"| F1["버그 후보 조사<br/>(기대값 갱신 금지)"]
    F1 --> A
    G1 -->|통과| A2["국면 1 — 강제청산 수신 배선<br/>판별 분기·catch-up 확장·선-조회"]
    A2 --> G1b{"강제청산 수용 케이스<br/>정상 변환·소급·보수 처리·멱등·선-조회"}
    G1b -->|실패| F1b["배선·판별 규칙 수정"]
    F1b --> A2
    G1b -->|통과| B["국면 1 — signal 전략 구동 치환<br/>(선행: 활성 전략 Adaptee + analyze 골든)"]
    B --> G2{"analyze 골든<br/>동일 입력 → 동일 신호"}
    G2 -->|실패| F2["Adaptee 재구현 수정(작성자)"]
    F2 --> B
    G2 -->|통과| C["국면 2 — 지표 인소싱"]
    C --> G3{"지표 동등성<br/>첫 대상 adx_14<br/>수치 게이트 + 판단 게이트(우선)"}
    G3 -->|실패| F3["seed·구현 차이 조사"]
    F3 --> C
    G3 -->|통과| D["국면 2 — 수집기 내부화<br/>(적재 불변식: 확정 캔들마다 1행·무조건)"]
    D --> G3b{"수집기 절체 게이트<br/>병행 적재 전 컬럼 차이 0 · 절체 연속성"}
    G3b -->|실패| F3b["구 수집기 복귀 후 조사"]
    F3b --> D
    G3b -->|통과| E["국면 2 — fill_timing 전환<br/>immediate → next_bar (단독 배포)"]
    E --> G4{"전수 회귀 재검증<br/>+ 전환 기록(기동 로그·채택 기록)"}
    G4 -->|실패| F4["전환 되돌림(기본값 복원) 후 조사"]
    F4 --> E
    G4 -->|통과| Z["실거래 전환 — core-lib 버전 고정 wheel<br/>+ 재복제 가드 상시"]
```

**채택 기록(기록물의 실체).** 이 부록의 모든 게이트·전환·확정·수동 조치가 수렴하는 기록처는 **monorepo 안
고정 경로의 채택 기록 문서**(마크다운, git 추적 — 배포 루트와 형제인 `docs/adoption/` 아래)다. 항목의 최소
형식은 — 무엇을(게이트·전환·확정의 이름), 결과(통과/실패/확정값), 언제(실행 시각), 어디서(대상 커밋 해시),
누가(수행자·승인자 — 승인은 발주자, 부록.2 단계 통제) — 다섯이며, 골든 재수립의 차이 설명 대장과 테스트넷
드릴 결과·수동 조치 내역도 여기 붙는다. git 추적 문서이므로 보존은 저장소와 함께 영구이고, 기록의 변경
이력도 커밋으로 남는다.

**채택·구현 시 확정 목록(한 곳으로 모음).** 이 부록이 시작값·유보로 두고 채택 또는 구현 시 확정하기로 한
항목의 전체 목록이다 — 흩어져 있으면 누락되므로 여기 모으고, 각 확정은 발주자 승인과 함께 채택 기록에 남긴다.

1. 거래소 청산 주문 식별자 집합(자동 감축 식별자 포함) — 부록.3 (a).
2. 청산 정리 수수료의 수입 유형 이름·주문과의 연결 키(수입 이력 조회) — 부록.3 (b).
3. 강제 주문 전용 조회의 엔드포인트·요청당 시간창·보관 한계 — 부록.3 (c).
4. 지표 동등성 수치 게이트의 최종 허용오차(시작값 상대 1e-4·절대 1e-8에서 좁힘) — 부록.4.
5. 회계 실측 대사 중 재계산 항(실현 손익)의 최종 기준(시작값 차이 0에서 조정 시) — 부록.4.
6. 병행 적재 창 길이(시작값 24시간에서 조정 시) — 부록.4.
7. 실제 티어 유지증거금률(대사 레코드 `est_inputs` 병기값) — 부록.3 (d).
8. 채택으로 추가 확인되는 `wallet_db` 컬럼(기본 둘 외 추가분이 있으면) — 부록.3.

**완료 확인 목록.** 채택은 다음이 전부 채택 기록으로 남았을 때 완료다.

1. wallet 전수 회귀 통과 — 교정 표면 골든 재수립 내역과 차이 설명 대장 포함.
2. 활성 전략 analyze 골든 통과 목록.
3. 지표 동등성 결과 — 첫 대상 `adx_14`와 활성 전략 `required_indicators` **전체**로 넓힌 결과, 확정 허용오차.
4. 강제청산 수신 배선과 다섯 수용 케이스 통과(부록.3의 (a)~(d) + 위 수신 게이트) — 테스트넷 드릴 결과,
   드릴을 수행하지 못했으면 그 사유.
5. `fill_timing` 전환 시각·커밋·재검증 결과 — 전환 경계 성과 비교 금지 주석 포함.
6. 수집기 내부화 — 절체 게이트(병행 적재 일치 결과 요약 포함 — 병행 원자료는 폐기하므로 비교 결과를 폐기
   **전에** 기록으로 확정한다·절체 연속성·자가 복구 시연) 통과와 적재 불변식 확인.
7. 판단 트리거 치환 후 모니터링 결과 — 신호 방출 시각 이동 폭과 마감 후 행 가용 지연 분포(부록.2).
8. 재복제 가드 통과와 shim 제거 확인(옛 경로 임포터 0건·별도 커밋 — 제거를 뒤로 미뤘으면 그 사실).
9. 비밀 점검 결과 — 커밋되는 파일에 새 비밀 리터럴 없음(부록.6 절차 3).
10. 실거래 전환 시 core-lib 고정 버전과 `core_lib_version` 메타 일치.

## 부록.5 새 백테스트의 검증 기준선 — 구↔신 수치 대사 제외

**수용 기준(명시).** **구 백테스트와 새 백테스트가 같은 수치를 내는지 대조(대사)하는 것은 새 백테스트의 수용
기준이 아니다.** 두 결과의 수치 일치를 확인하는 기준선을 두지 않으며, 그 일치·불일치는 새 빌드의 합격·불합격에
어떤 영향도 주지 않는다. 이 제외는 다음 세 사유로 확정되었다(폐기·재구축 인벤토리의 결정을 이 부록이 수용
기준 문구로 다시 못박는 것이다 — downstream에서 이 게이트 변경이 유실되지 않게).

1. **폐기 대상을 참조로 되살리지 않는다.** 구 백테스트는 전면 폐기 대상이다. 수치 대사를 만들려면 그것을
   실행하거나 되살려야 하는데, 이는 폐기 결정과 모순된다.
2. **결함 기준선에 정렬하지 않는다.** 구 백테스트는 즉시 체결의 look-ahead와 gross 회계라는 알려진 편향을
   안고 있었다. 그 수치에 맞추는 것은 정확성이 아니라 결함에 검증하는 것이다.
3. **동작이 의도적으로 다르다.** 새 빌드는 체결을 다음 캔들 시가로, 회계를 net-of-cost로 바꿨다. 수치 일치는
   애초에 목표가 아니다.

**제외가 "검증 없음"이 아니다 — 대체 기준선.** 새 백테스트의 수용은 아래 자체 기준선으로 판정한다.

| 기준선 | 내용 | 확인 방법 |
|---|---|---|
| 결정성 | 같은 설정 입력·같은 원천 데이터·같은 seed는 언제나 같은 Evidence를 낸다 | 동일 `config_hash`·동일 `source_data_hash` run 2회 → 정규화 Evidence 해시(정렬 행 직렬화·wall-clock 제외) 동일 |
| look-ahead 무결성 | 미래 데이터가 과거 판단에 스며들지 않는다 | 시계열 뒤에 미래 캔들을 덧붙여도 과거 신호·지표 값 불변 |
| 지표·전략 골든 | 계산이 표준과 일치한다 | 지표별 1차 출처 교차대조 골든값, 벡터화↔증분 일치, 활성 전략 analyze 골든 |
| 수식 골든 | 성과 지표 계산이 표준 수치를 재현한다 | 표준 재현 수치와 일치 — Sortino 2.4910·PF 3.4118·SQN 1.6647·MDD −3.50%·Calmar 3.2102. 입력 거래 집합은 성과 지표 표준 참조 `50_metrics_reference.md`의 재현 예제다(구현까지 유지되는 표준 참조 파일) |
| 회계 실측 정확성 | 재구현 회계·손익·체결이 현실과 맞는다 | 거래소 실측 대비 대사 — 시작 기준은 금액 자릿수 반올림 안 차이 0(부록.4 골든 재수립 규칙 3) |
| 실데이터 회귀 | 픽스처가 놓치는 데이터 범위·방향·펀딩·마진 경로가 실제 원천에서도 완주한다 | 통합 테스트의 명시적 회귀 매트릭스가 3일·10일·30일·45일·135일·180일·330일 창, 롱·숏, 양·음 펀딩 요율, 1배·3배·39배 레버리지를 분담해 검사한다. 짧은 등급은 일반 체결과 리버설을 상시 확인하고, 긴 등급은 요율 부호·펀딩 소진 청산·결측·창 길이 불변성을 확인한다. 음수 요율과 고레버리지의 직교 조합은 비용이 큰 전수 조합에서 의도적으로 빼되, 부호 산술은 1배에서 격리 마진 한도는 39배에서 각각 독립 검증한다 |
| 세 모드 동등성 | 백테스트=페이퍼=라이브가 같은 판단을 낸다 | 동일 입력에서 세 실행 모드의 지표·신호 산출 일치(공유 코어 단일 구현의 검증) |
| 통계 검증 | 과최적화를 걸러낸다 | Harness(§4.4.4)의 표본 내/외 분리(성능 저하 50% 미만)·워크포워드·몬테카를로·확률적 샤프(PSR ≥ 0.95) |

**구분을 못박는다.** 제외 대상은 "구 백테스트 수치와의 일치를 기준으로 삼는 것" 하나뿐이다. 위 대체 기준선의
검증들, 그리고 거래소 **실측**과의 대사(부록.3 (d)·부록.4 회계 골든)는 제외 대상이 아니며 그대로 수행한다.

**수용하는 잔여 위험(명시).** 이 제외로 구·신 두 구현을 맞대는 교차 검증의 기회는 사라진다 — 만약 어떤 결함이
구와 신 양쪽에 같은 모양으로 존재한다면(예: 두 구현이 공유하는 잘못된 가정), 대체 기준선의 골든·실측 대사가
그 지점을 직접 덮지 않는 한 교차 대조로 드러날 기회가 없다. 이 위험은 결함 기준선에 정렬하지 않는 이득과
맞바꾼 것으로 수용하며, 완화 장치는 대체 기준선의 1차 출처 골든(구 구현이 아니라 원 출처와 대조)과 거래소
실측 대사다.

## 부록.6 비밀 저장 방식 변경

**결정(확정).** 두 갈래다 — ① **기존에 커밋된 비밀번호의 값은 바꾸지 않고 계속 쓴다.** 대상이 로컬 전용 개발
데이터베이스(사설 저장소·폐기 가능 데이터)라 git 노출의 실무 위험이 낮다는 판단이며, 값 교체를 강제하는 일반
규약에서 벗어나는 의도적 조정이다. ② **저장 방식은 바꾼다** — 앞으로 새 비밀은 커밋되는 파일에 하드코딩하지
않고 환경 파일(`.env`, gitignore) 주입으로만 둔다. 이미 환경 파일 방식인 것들은 손대지 않는다.

**현황(실측 목록).** 값은 마스킹 상태로 식별만 한다.

| 위치 | 성격 | 처리 |
|---|---|---|
| `init-scripts/01-init-databases.sql` 26~28행 — `wallet_writer`·`signal_writer`·`backtest_writer` 비밀번호 | 커밋됨 · 라이브 생성 | 값 유지(결정 ①) |
| `init-scripts/02-init-wallet-db.sql` 597행 — `report_reader` 비밀번호 | 커밋됨 · 약함 | 값 유지 |
| `init-scripts/archive/07-init-backtest-db.sql` 16행 — 구 `backtest_writer` 비밀번호 | 커밋됨 · legacy(대체 완료) | 참조하지 않음(아카이브 그대로) |
| `wallet-service/docker-compose.e2e.yml` 10·52~53행 — 테스트 DSN | 커밋됨 · 테스트 전용 | 값 유지(테스트 성격) |
| `wallet-service/core/config.py` 37행 — `secret_key` 기본값 | 커밋됨 · 플레이스홀더 | 실비밀 아님 — 실값은 환경 파일로만 |
| signal·wallet `.env`·`.env.live`·`.env.dev` — 실운영 DSN | 미추적(디스크) · 이미 환경 파일 방식 | 손대지 않음 |
| crypto-data-hub `.env`의 외부 API 키(`NEWS_API_KEY`) | 미추적 · 외부 유료 서비스 | 유출 정황이 확인되면 그때 값을 바꾼다 |

**절차(신규 작업에 적용).**

1. **`backtest_reader`(신설 읽기 전용 역할 — §5.1.1)는 처음부터 환경 파일 주입만 쓴다.** 신규 초기화
   SQL(`init-scripts/NN-init-backtest-db.sql` — §2.3)에는 역할 생성과 `GRANT SELECT` 권한만 담고 **비밀번호
   리터럴을 넣지 않는다.** 비밀번호는 배포 시 환경 파일 값으로 별도 설정한다(psql 변수 주입 또는 생성 직후
   `ALTER ROLE … PASSWORD`를 환경 파일 값으로 실행). 계승하는 `backtest_writer`는 기존 값 그대로다(결정 ①).
2. **신규 서비스의 접속 정보는 전부 환경 변수 주입이다.** `backtest-service`의 접속 문자열은 코드·커밋 파일에서
   조립하지 않고 완성된 URL을 환경 변수로 받는다(§5.1.1과 동일 규약 — 기존 두 서비스의 현행 방식과 같다).
3. **확인.** 채택·구축 기간의 커밋 리뷰에서 "커밋되는 파일에 새 비밀 리터럴 없음"을 점검 항목으로 둔다(간단한
   비밀 패턴 grep이면 충분하다 — 이 확인은 새 유입을 막는 것이고, 기존 커밋 값은 결정 ①의 대상이다).

## 부록.7 전략 플랫폼 as-built 갱신 이력
...
2026년 8월 7일에 별도 조사 문서였던 `strategy_platform_asbuilt_design.md`의 확인 내용을 이 원본에 통합하고 그
파일을 제거했다. 상세 행위 규칙은 복제하지 않았으며 `docs/strategy-authoring-contract.md`가 계속 단독으로
소유한다. 이 절은 원본이 왜 바뀌었는지와 어느 부분을 코드에 맞췄는지만 기록한다.

| 갱신한 절 | 낡았던 원인 | 반영한 현재 구현 |
|---|---|---|
| §2.1 코드 트리 | `money_management/`·`patterns/`·`series/contracts.py`·`series_resolution.py`가 없었다 | 실제 패키지와 `VesselReference`·예약 trailing 모듈까지 트리에 추가했다 |
| §3.1 core-lib 컴포넌트 | 지표와 전략만 보여 패턴·공통 계열 해석·자금관리 의존을 설명하지 못했다 | 별도 패턴 레지스트리, `SeriesSpec` Protocol과 해석 모듈, 자금관리 정책 및 runtime 조합을 추가했다 |
| §4.1.1 타입 | `TradingSignal`만 전략 판단 타입으로 설명했다 | 목표 `DecisionIntent`, legacy 및 내부 구체화 `TradingSignal`, signal-service의 `PersistedSignal.signal_type` 경계를 분리했다 |
| §4.1.2 지표·패턴·계열 | 패턴 레지스트리와 두 실행면의 계열 선택 경로가 없었다 | `PatternSpec`·`PatternState`·`PatternRegistry`, 네 출력, `resolve_series_specs`의 auto·explicit·all 의미를 추가했다 |
| §4.2.1 전략 | `analyze`가 `TradingSignal`만 반환하고 전략이 보호가격까지 정하는 것으로 적혀 있었다 | Protocol의 과도기 합집합과 `VesselReference`의 `DecisionIntent | None`, `MoneyManagementSupport`, `StrategyRuntime`을 반영했다 |
| §4.2.2 자금관리 | 계층 전체가 원본에 없었다 | 전략과 실행 사이에 현재 위치한 manual·turtle 정책, 값 타입, Factory, 정책 전용 일봉, 실행면별 조합 차이를 새 하위 절로 세웠다 |
| §4.3.1·§4.3.3 실행·사이징 | 단계별 부분 익절과 trailing 계산이 구현된 것처럼 읽힐 여지가 있었고 자금관리와 sizing의 관계가 없었다 | 단계별 익절 계획은 없지만 `PositionBook`이 임의 축소 체결을 처리하는 경계, trailing matcher 거부, 정책 계획과 기존 sizing·체결가 상한의 층위를 분리했다 |
| §4.4 Engine·설정·피드 | 판단 시퀀스가 `TradingSignal` 전용이고 1분 trigger walk가 실행 가능한 것으로 적혀 있었다 | `DecisionIntent` 정책 적용과 legacy 우회, manual·turtle 설정, 전략 TF trigger만 실행하며 `m1_subcandle`을 거부하는 현재 흐름을 반영했다 |
| §4.5 Evidence | 스키마 판과 지표 정의 필드가 구판이었다 | 스키마 `1.5.0`, 기본 열네 테이블과 확장 일곱 테이블, `INDICATOR_DEFINITION.series_kind`·`category`·`impl_note`, 자금관리 제출·해석 기록 경계를 반영했다 |

2026년 8월 7일 커밋 `e1fd478`에서 추가된 전략 작성 기반·배포 허용 목록·카탈로그 맞춰 보기 경계를 다시 코드와
대조해 같은 원본에 반영했다.
...
이 문서(§1~§4.5·부록)가 어떤 표준 요구를 충족하는지를 이름으로 적는다.
...
| 제약사항·방향(확정된 범위 조정), §2.1 `strategy/trailing/` | 트레일링은 열거형만 존재하고 matcher가 거부하며, `m1_subcandle`은 설정과 Engine이 거부하는 현재 예약 경계를 구조에 반영 |
...
| §3.1 컴포넌트·다이어그램 화살표 | 단일 표준 구현(열세 컴포넌트를 한 곳에 정의, 세 모드 공유) · 의존은 한 방향(내부 types 바닥, 역참조 없음) |
...
| §4.4 `Engine`·캔들 루프 | 캔들 두 순간의 고정 순서가 look-ahead 방지(시가=지난 캔들 정산·전략 TF trigger, 종가=이번 캔들 판단) · 시점 순서 `decision_ts < execution_ts`(종가 신호·다음 시가 체결, `pending` 한 칸 지연) · `DecisionIntent` 진입에 정책 적용·legacy 신호 우회 · 결측 직전 주문 건너뜀과 `DATA_GAP` 청산 · 동시 도달 손절 우선·소급 금지 · `m1_subcandle`과 trailing 명시적 거부 · 워밍업 preload 후 신호 discard · 결정성(Clock 주입·wall-clock 금지·동일 설정·원천·seed→동일 Evidence) |
...
> 이 문서는 core-lib 클래스 뷰(§4.1~§4.3)와 backtest-service 클래스 뷰(§4.4~§4.5), 그리고 부록(채택·회귀
> 절차 — 채택 지점·shim, 라이브 강제청산 수신, 회귀 게이트, 검증 기준선과 수치 대사 제외, 비밀 저장 방식
> 변경)까지 확정했다. 데이터베이스 설계(§5)는 짝이 되는 별도 문서가 각 DB의 ERD·필드를 확정한다. 이로써 상세
> 설계는 §1~§5와 부록 전부가 확정 상태다 — 구현 단계는 이 두 문서만으로 짓는다.
