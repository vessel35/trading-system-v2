...
**shim 제거.** 채택이 안정된 뒤 내부 임포터를 새 경로로 점진 이전하고, 옛 경로의 임포터가 0건임을 확인(전역
grep 또는 import 규칙 검사)한 뒤 **별도 커밋**으로 shim을 제거한다. shim이 남아 있는 동안에도 재복제 가드는
...
- **청산 정리 수수료(penalty)** — 청산 `Fill` 처리 직후 **계좌 수입 이력 REST 조회**로 해당 청산의 정리 수수료
...
**수동 개입의 책임·기록.** 이 절의 경보 — 판별 불확신((a)), ADL 식별((a)), zeroing + CRITICAL((c)) — 는 현행
운영 알림 채널로 보내며, 대응 책임은 운영자(발주자)에게 있다. 조치 내용은 채택 기록(부록.4)에 남긴다.
...
1. **정상 변환** — 합성 청산 통지(체결 종결 상태)를 주입하면 `Fill(exit_reason=LIQUIDATION)`이 부록.3 (b)의
   ...
   규칙대로다 — 구분 보고된 청산 정리 수수료만 계상(구분 보고가 없으면 0)하고 같은 금액이 `total_fee`에 중복
   계상되지 않는다. 회계 항등식(`cash + position = equity`)이 유지된다.
...
4. 강제청산 수신 배선과 다섯 수용 케이스 통과(부록.3의 (a)~(d) + 위 수신 게이트), 테스트넷 드릴 결과 포함.
