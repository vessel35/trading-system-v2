...
sequenceDiagram
    ...
    WS->>G: 강제청산 수용 케이스(부록.4 — 변환·소급·보수 처리·멱등)
    G-->>OPS: 통과
...
수렴은 별도 결정으로 미루는 보수 처리이며, 이것이 의도다.
...
중복 회계되는 여지를 없앤다). **멱등성** — 같은 청산 주문 id를 두 번 처리하지 않도록 기록 전 중복 검사를
...
통일. 판단 트리거 변경은 구동 방식 변경으로 판단 산출을 보존하고, 강제청산 수신은 없던 회계 기록을 새로 더하는
것이라 성격이 다르다). 규약:
...
1. **정상 변환** — 합성 청산 통지(체결 종결 상태)를 주입하면 `Fill(exit_reason=LIQUIDATION)`이 부록.3 (b)의
   필드 대응 그대로 만들어지고, 회계에 `Trade.liquidated = TRUE`·`liquidation_penalty ≥ 0`이 남으며 회계
   항등식(`cash + position = equity`)이 유지된다.
...
flowchart TD
    ...
    D --> E["국면 2 — fill_timing 전환<br/>immediate → next_bar (단독 배포)"]
...
⑥ 수집기 내부화·적재 불변식 확인, ⑦ 재복제 가드 통과, ⑧ 실거래 전환 시 core-lib 고정 버전과
...
| 수식 골든 | 성과 지표 계산이 표준 수치를 재현한다 | 표준 재현 수치와 일치 — Sortino 2.4910·PF 3.4118·SQN 1.6647·MDD −3.50%·Calmar 3.2102 |
...
| 부록.3 | 강제청산 출처·수렴 계약 준수(거래소가 집행·우리는 수신 정리만, `Fill(exit_reason=LIQUIDATION)` 동일 모양·단일 회계 경로·`Trade.liquidated`·종결 상태 이벤트 1회 생성) · 누락 복구(재접속 catch-up·주기 대조 확장, 조회 창 기준·멱등) · 실측 대사(청산 근사의 보수성 실측 확인 — 구↔신 대사 제외와 별개, 기록은 독립 라이브 Evidence SQLite로 연구 데이터·운영 DB 분리 준수) · Decimal 단일 변환 관문(거래소 문자열 수치) |
| 부록.4 | 회귀는 전수 계약(wallet 약 1,279건) · 교정 표면 골든 재수립 규약(검증 로직 보존·차이 전건 표준 수식 설명·실측 대사 시작 기준은 자릿수 반올림 안 차이 0) · 지표 동등성 게이트(첫 대상 `adx_14`·수치+판단 두 겹) · 강제청산 수신 게이트(다섯 수용 케이스 — 신설 회계 경로의 검증) · `fill_timing` 전환 재검증 트리거·기록 · 재복제 가드 |
