...
- **지표 무생성.** `OHLCV 수집기`는 지표를 만들지 않는다 — 지표는 signal-service·backtest가 공유 코어로 직접
  계산한다(코드 + 호출 계약 = 확정 캔들 트리거·매 캔들·OHLCV 순수 함수, 세 실행 모드 동일).
...
세 가지의 코드·계약은 Binance 선물 적재 경로에 이미 있으니 그대로 가져온다(Chapter 2.1 참고). 다만 순수 적재
...
- **호출 계약을 라이브·backtest가 통일한다.** 확정 캔들마다(매 캔들 무조건), 그 시점까지의 확정 OHLCV만 입력으로
  받는 순수 함수로 계산한다. 코드가 같은 것(DRY)만으로는 부족하고 이 **트리거·입력 계약**까지 같아야 세 실행
...
| 2.2, 3.2 | 지표 계산 능력은 공유 코어가 새로 담당(폐지되는 것은 collector의 지표 역할뿐); 대체 계산 방향(호출 계약 통일·증분·인프로세스) |
