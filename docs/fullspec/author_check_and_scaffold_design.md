# 전략 작성의 배포 관문을 명령 하나로 판정하고 상용구를 플랫폼이 소유한다
...
가장 크게 바로잡은 것은 이 관문의 이름이다. 공통 규범 검사 묶음이 없는 동안 이 명령은
**배포 관문**(파일을 두고 등록해도 되는가)이지 **완료 관문**(전략이 문서대로 다 됐는가)이
아니며, 완료는 이 관문에 검산과 사람의 확인을 더한 것이다.
...
3. **플랫폼이 가져야 할 상용구를 작성자가 쓴다.** `market_data`에서 캔들·시간대·지표를 꺼내
   형을 확인하는 코드, `DecisionIntent`를 만드는 코드, 시험에서 실행 key를 적는 코드를
   전략마다 다시 썼다. 인수 작업의 QA 단계에서 그 상용구에서 mypy 오류 셋과 시험의 key 오기가
   났고 커밋 전에 고쳤다. 인수 기록은 결과만 적고 이 과정을 적지 않았으므로 여기 남긴다.
4. **원문과의 차이가 산문에만 남는다.** 빈 값을 정한 것과 규칙을 근사한 것이 인수 기록 4장의
   문단으로만 있어서, 무엇이 승인된 이탈이고 무엇이 검산 대상인지 프로그램이 알 수 없다.
...
- **능력 목록에는 series 이력 깊이가 없다.** `run.strategy_inputs`는 전략이 받는 여섯 key를
...
- **Engine을 데이터베이스 없이 돌리는 조립은 시험 파일 안에만 있고, 전략 셋을 고정한다.**
  `services/backtest-service/tests/test_document_sourced_strategies_engine.py`가 합성
  1시간봉 feed(320봉 warm-up과 240봉 평가 구간, 고정 seed), 메모리 카탈로그(결정성 참조를
...
- **저장소 뿌리는 QA 밖이다.** 뿌리에 `pyproject.toml`·`ruff.toml`·`mypy.ini`가 없고 각
  ...
  `scripts/` 아래 파일은 ruff·mypy·pytest 어느 관문도 지나지 않는다.

## 3. 무엇을 만드는가

여섯 덩어리다. 처음 넷이 둘째 책임(오류 없는 코드)을, 다섯째가 상용구를, 여섯째가 첫째 책임
(정확한 이식)을 맡는다.

### 3.1 식별자 규칙을 잎 모듈 한 곳에 두고 네 자리에서 쓴다

`core_lib/identifiers.py`(아무것도 import하지 않는 잎 모듈)에
...
### 3.4 `author_check`: 배포 관문을 명령 하나로 판정한다
...
**mode별 대표 설정.** 단계 3과 6은 지원하는 mode마다 정책 설정 하나가 필요하다. 그 정책의
설정에 모두 기본값이 있으면 기본값을 쓰고, 기본값 없는 설정이 하나라도 있으면
`--mode-settings` 파일에서 그 mode의 설정을 읽으며, 거기에도 없으면 단계 3이
`policy-settings-required`로 실패한다. 이것은 공통 규범 검사 설계 3절의 방식 사례표가
...
| 6. 합성 경로 실행 | **지원하는 mode마다** 대표 설정으로, 고정된 합성 캔들 경로 위에서 실제 Engine을 끝까지 돌려 예외가 없고 Evidence 무결성이 통과하며 두 번 돌린 hash가 같은가. Engine이 보고한 `warmup_candles`를 함께 낸다. 거래가 0건인 mode는 "경고"로 보고한다(규칙이 그 경로에서 서지 않았다는 뜻이며 오류는 아니다) | `backtest_service.diagnostics.synthetic_dry_run` (3.4.1) |
...
**이 명령이 증명하는 것과 증명하지 않는 것.** 이 명령은 전략이 발견·등록·조합·실행되는
**배포 관문**이다. 문서의 규칙대로 진입하고 청산하는지(방향별 진입·청산 증거)와 결정성·정책
독립성·시간 무결성·금지 의존·선언과 접근 일치·유효한 청산은 증명하지 않는다. 그것은 공통
규범 검사 설계가 방식 사례표로 정한 일이며, 그 묶음이 구현되면 단계 6과 7 사이에 "10. 공통
규범 검사(그 전략 하나에 대해)"로 들어가고 그때 이 명령이 완료 관문이 된다.
...
관문이다"를 다음으로 바꾼다. "구현이 끝났다고 말하려면 셋이 필요하다. 첫째,
`python -m backtest_service.author_check <id>`가 exit 0이고 단계 9가 건너뜀이 아니다. 둘째,
`verify-strategy` 절차가 이탈 장부(3.6)의 행마다 확인을 마쳤다. 셋째, 프로그램이 못 보는
...
`test_document_sourced_strategies_engine.py`의 합성 feed, 메모리 카탈로그, 등록 행 fixture,
실행 설정 생성을 옮기고 시험은 그 모듈을 import한다. **등록 행 fixture는 세 전략의 고정 표가
아니라 발견 결과에서 id로 만든다.** 그래야 `vessel-reference`를 포함한 배포 전략 전부와 앞으로
더해질 전략에 같은 명령이 닿는다.

**배포 패키지에 시험 대역이 들어간다.** 메모리 카탈로그는 결정성 참조를 "이전 실행 없음"으로
꾸며 내고 등록 행 fixture는 배포된 클래스에서 행을 지어낸다. 받아들이는 이유는 이 대역들이
...
시험의 `DataFeed` 대역은 그대로 두고, 이 인수 시험 하나만 진단 모듈을 쓰도록 옮긴다.
...
changeset이며, 판단이 바뀌지 않음을 기존 시험과 합성 경로의 Evidence hash로 확인한다.

**뼈대 생성기를 둔다.** `python -m trading_plugins.scaffold <입력 JSON>`이 JSON 하나(전략
...
`TradingSignal` 방식의 뼈대는 만들지 않는다. 정책 뼈대도 이 단계에서 만들지 않으므로 OpenAPI
재생성은 생성기의 범위 밖이며, 정책을 배포할 때 `npm run generate:api`가 필요하다는 사실은
규범 6.4절에 적는다. 생성 직후의 뼈대는 판단 없이도 `author_check` 단계 1부터 5와 7을 통과해야
한다. 그래야 작성자가 채우는 것이 판단 로직뿐이다.

생성기는 파일을 만드는 도구이므로 사실 모듈에 두지 않고 별도 모듈로 둔다. 존재하는 파일은
덮어쓰지 않고 거부한다.

### 3.6 이탈 장부: 원문과 구현의 차이를 구조로 남긴다
...
"이탈 장부"이고 열은 넷이다. 원문 규칙, 플랫폼 표현, 이탈 종류, 근거. 이탈 종류는 셋 중
하나다.
...
`author-strategy` skill 4장과 5장의 "이탈을 모아 한 번 묻는다"를 이 표로 바꾼다. 장부의 각
행은 전략 모듈 docstring에 같은 표로 옮겨 적고, `verify-strategy` 절차는 장부의 행마다
그 이탈이 실제로 그렇게 구현됐는지를 확인 항목으로 삼는다(예: 상태 규칙으로 읽었다면 손절 뒤
재진입이 몇 번 있었는지 세어 보인다).

"질문하지 말고 진행하라"는 지시가 있을 때 첫째와 셋째 종류에서도 멈출지는 설계가 정할 일이
아니라 사용자가 정할 일이므로 8장에 둔다.

이 단계에서는 장부를 프로그램이 검사하지 않는다. 장부를 구조화된 파일로 두고 검사하는 것은
둘째 단계(중간 언어)의 일이다.
...
1. **식별자 규칙과 능력 항목 둘.** `core_lib/identifiers.py`, `run_config`의 교체,
   ...
   `test_engine_and_harness.py`의 이력 깊이 시험. 이 changeset만으로 능력 목록 시험이 초록이다.
...
5. **`core_lib` 보조 다섯과 뼈대 생성기.** 그 뒤 기존 전략 넷을 보조로 다시 맞추는 changeset을
   따로 둔다.
6. **이탈 장부.** 전략 기술 문서 형식, 두 skill의 문구, 새 전략 셋의 문서에 장부를 소급해
   적는다.

1과 2가 인수 시험에서 난 실패 둘을 직접 막고, 4가 배포 관문을 세운다. 5와 6은 그 뒤에
...
- 규범의 편집 다섯(4.1절 예시의 클래스 선언과 `decision_contract` 선언, 두 표시 주석, 6.4절
  문장, 6.5절 문장)과 skill의 편집 셋(3.2의 절차 규칙, 3.4의 완료 정의, 3.6의 이탈 장부),
...
- 뼈대 생성기가 만든 전략이 손대지 않은 채 `author_check` 단계 1부터 5와 7을 통과하고, 만든
...
내리고, 뼈대 생성기의 출력 형식이 변환기의 출력 형식이 되며, 이탈 장부는 중간 언어의 한 절이
된다. 언어의 범위는 이 단계에서 쌓인 전략들이 실제로 쓴 규칙 모양(series 값 비교, 상태
정렬, 밴드 이탈, 문턱 교차)에서 정한다.

## 8. 열려 있는 결정

- "질문하지 말고 진행하라"는 지시가 있을 때, 이탈 장부의 첫째와 셋째 종류(능력 부재로 강제됨,
...
- 뼈대 생성기가 `StrategyProfile` 열두 값을 입력으로 요구할지, 표시 자리만 두고 `provisional`
  기본값을 채울지. 규범 4.1절이 일곱 값의 기준을 아직 정하지 않았다.
- 기존 전략 넷을 보조 메서드로 다시 맞추는 일을 5번 changeset에 붙일지 따로 둘지.
- 합성 경로에서 거래가 0건인 mode를 경고로 둘지 실패로 둘지. 합성 경로는 전략마다 다르게
...
- **합성 경로 길이를 전략마다 바꾸면 기존 시험의 판정이 흔들린다.** 320봉과 240봉 고정 경로로
  못박고 시험이 자기 설정을 명시적으로 넘긴다고 적었다.
- **`synthetic_dry_run`은 시험 대역을 배포 패키지에 들이는 일이다.** 순환은 없지만 그 사실과
  받아들이는 근거, `conftest.py` 대역과의 관계를 3.4.1에 적었다.
- **changeset 1이 홀로 빨갛게 끝날 순서였고 정규식이 네 번째로 복사될 참이었다.**
  `verified_by`를 changeset별로 나누고, 상수를 잎 모듈에 두어 `capabilities.py`가 import하게
  했다.
- **저장소 뿌리의 `scripts/`는 어떤 QA 관문도 지나지 않는다.** 판정 코드를 서비스 모듈로 옮기고
...
- **단계 8의 기준은 fixture 문자열로도 만족됐고, 단계 9는 등록 MCP 설계 6장의 결정을 도구로
  ...
  통합 시험을 부른다"로 바꿨다.
- **폐쇄 기준에 규칙 이름 어휘, 규범과 skill의 편집, turn 상한, 뼈대 생성기의 06 포함, 건너뜀
...
- **합성 시험은 "배포된 전략"이 아니라 새 전략 셋만 돌린다.** `_STRATEGIES`가 셋으로 고정되어
  `vessel-reference`가 빠져 있었고, 그대로 옮기면 넷에 대한 폐쇄 기준이 성립하지 않았다. 2장의
...
- **이 관문은 지원하는 모든 정책 경로와 방향별 진입·청산 증거를 증명하지 못하므로 완료
  관문이 아니다.** 단계 6을 지원 mode 전부로 넓히고, 이름을 배포 관문으로 바꾸고, 완료의
  정의를 관문·검산·사람의 확인 셋으로 다시 썼다(문서 제목, 3.4, skill 문구).
- **통합 시험의 이름에 전략 id가 없어 `-k <id>`가 아무것도 고르지 못한다.** 시험을 배포된
  전략과 정책으로 매개변수화하고, 명령이 수집 건수를 확인하게 했다(3.4, 5장 4번).
- **뼈대가 `decision_contract`를 내지 않으면 기본값 `TradingSignal` 때문에 단계 3에서
