...
이 검사는 전략의 수익성을 평가하지 않는다. 신규 전략과 기존 전략이 플랫폼 계약을 계속
지키는지를 CI에서 검증한다.

## 2. 계약 상태와 적용 경계

현재 허용 목록의 `VesselReference`는 목표 계약인 `DecisionIntent`를 반환하고 Engine은
`MoneyManagementPolicy`를 조합한다. `StrategyAdapter.analyze()`는 이행 기간의 legacy
`TradingSignal`도 허용한다.

계약 사례표는 각 전략의 계약 종류를 명시해야 한다. 신규 전략은 `DecisionIntent`를 요구하고,
legacy 전략을 유지해야 한다면 사례표에 호환 경계임을 명시한다. 공통 검사가 legacy 반환형을
신규 전략의 정상 계약으로 넓혀서는 안 된다.
...
허용 목록만으로는 유효한 인스턴스와 판단 시나리오를 만들 수 없다. 따라서 테스트 전용 계약
...
6. 사례는 목표 계약인지 명시적인 legacy 호환 계약인지 밝혀야 한다.
...
곧바로 계약 위반으로 단정하지 않는다. 필수 사례 전체에서도 관찰되지 않았다면 사례가 부족한지
...
각 계약 사례는 의도적으로 진입 증거를 만들어야 한다. 진입이 우연히 나오지 않았다는 이유로
검사를 건너뛰지 않는다.
...
`analyze()`의 시그니처를 규범 계약과 대조한다. `StrategyBase`를 상속하는 구현은 추상 메서드를
빠뜨렸을 때 인스턴스화가 거부되는지도 별도로 확인한다.

계약의 기본 여섯 성질과 함께 다음 사항을 확인한다.
...
- 허용 목록과 계약 사례표의 전략 id 집합이 정확히 같아야 한다.
...
공통 계약 검사 전체를 한 changeset에서 구현할지, 동일한 최종 계약을 유지하면서 검증 층별로
나눠 구현할지는 결정되지 않았다. 구현을 나누더라도 중간 단계가 전체 계약을 제공하는 것처럼
표시해서는 안 된다.
