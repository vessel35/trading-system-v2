...
구현됐다**(3·4·5·7·8·9·10·12절과 6절의 방어적 검사). Engine 조합 층(6절의 시간 무결성 주 검사, 지원하지
않는 조합의 Engine 수준 거부)은 아직 구현되지 않았으며, 전략 단위 층은 그것을 대신한다고 표시하지
않는다(모듈 docstring이 한계를 적는다).
...
사용자가 "층별"로 정했다. 첫 층(전략 단위)은 구현됐고 둘째 층(Engine 조합)이 남아 있다. 구현을
나누더라도 중간 단계가 전체 정책을 제공하는 것처럼 표시해서는 안 되며, 전략 단위 층의 모듈 docstring이
그 한계를 적는다.
