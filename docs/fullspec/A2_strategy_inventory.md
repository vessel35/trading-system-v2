...
> 전략 작성자 소유(이 설계 스코프 밖)다. 플랫폼은 전략을 끼우는 **계약**(Adaptee Protocol·Adapter Manager·
> StrategyConfig)만 설계한다. 이 노트는 그 계약 설계에 필요한 (1) `analyze` 계약 형태, (2) 호출·소비 구조
...
전략을 끼우는 계약뿐이다 — `StrategyAdapter`(typing.Protocol)·Adaptee·Adapter Manager·StrategyConfig.
...
`core_lib.strategy`로의 이식으로 표기하지만, 판단 로직은 작성자 소유·스코프 밖이라 **코드 이식이 아니라 계약
...
(라이브·백테스트 동일 형태·동일 호출 계약).

---

## 2. 이식하지 않는 현행 구조 (재설계 대상, 압축)

플랫폼 계약이 계승하지 않는 현행 골격. 계약 설계 입력으로만 기록한다.
...
## 3. Adaptee 판단 계약 형태 (계약 설계 입력)

플랫폼이 Adaptee Protocol을 설계하려면 판단 **내부 로직**이 아니라 판단의 **계약 형태**를 알아야 한다.
...
- **순수성·예외 접점**: precomputed 분기는 판단 전용(DB·루프·저장 없음). 예외 접점 = 시계 읽기
  ...
  지표는 Engine push로 외부화되고 판단 계약 형태만 남는다.
- **포지션 입력**: 현행 드라이버는 `analyze(market_data=...)`만 호출하고 `current_position`을 넘기지 않는다
  (`strategy_executor.py:1040`). 포지션 입력 계약은 플랫폼 설계에서 확정.
...
**참조 전용(이식 아님):** 판단 계약 형태(3)·호출 소비 구조(4)·config 스키마 패턴(아래 7)·전략 유니버스(7).
...
- **유니버스**: Vessel 5종(Alpha·Titan·Flux·FluxGen2·Vanguard) 외에도 `AbstractStrategy` concrete가 다수
  ...
  Adaptee 판단 전용 패턴(순수 `analyze`)을 따르며, 미독 디렉터리도 같은 계약 형태를 노출하는지 확인 대상이다.
...
- **미정리 항목(플랫폼 계약에서 확정)**: `current_position` 미전달, `MarketData` 미사용, base 비-stateless —
  현행 사실로만 기록하며 Adaptee 계약 설계에서 정리한다.
...
| 3 | 판단 출력 계약=TradingSignal(수량·방향 없음, 판단만) |

**정합성 확인 대상:** 계약 설계 입력이 "Adaptee=판단 전용" 원칙에 부합하는지 — 판단 계약 형태·호출 구조·config
패턴은 참조로, 판단 내부·파라미터 값은 작성자 소유로, Protocol·Manager·Config·프로파일은 신규로, 트레일링은
유보로 갈랐다. 이 노트는 이후 전략 설계 단계가 재인벤토리 없이 Adaptee 계약·Manager·Config를 설계하도록 한다.
