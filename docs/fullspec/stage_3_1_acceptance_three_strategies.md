...
| 공통 전략 규범 검사가 허용 목록 전체에 적용되고 결함 주입으로 검증된다 | 달성(전략 단위 층, 2026-09-25) | 처음 인수 때는 설계만 있었다. 병합 뒤 `services/trading-plugins/tests/test_strategy_contract_suite.py`가 발견된 전략 일곱 전부에, 각 전략이 선언한 모든 시간대에서, 결정성·정책 독립성·방어적 시간 무결성(순서 독립성)·금지된 의존(정적·실행 중)·선언과 접근의 일치·진입과 청산의 대응·시그니처·정책 선언과 사례표의 일치를 적용하고, 결함 전략 열아홉과 정적 검사 표본 여덟이 각각 대응하는 검사에서 실패하거나 통과함을 보인다. Codex 코드 리뷰 1차의 지적 다섯(선언한 시간대 전부, 순서 독립성의 실제 변경, 전략 패키지 helper 추적, 정책 없는 HOLD 전용 전략 허용, 사례표의 정책 기대값), 2차의 셋(포지션 불변성, 이미 적재된 모듈을 거친 벽시계 호출, 사례표의 legacy 방식 경계), 3차의 넷(별도 시간대로 선언한 series의 키 유지, 정책 입력을 전략의 해석 설정으로 구성, HOLD 전용 행의 대표 입력, 형 검사 전용 helper import 제외), 4차의 하나(인스턴스 상태의 호출 전후 비교)를 반영했다. Engine 조합 층(시간 무결성 주 검사)은 남아 있다 |
...
**요약.** 열두 항목 모두 달성이다(빈 계열과 Turtle 조합, 공통 규범 검사의 전략 단위 층은 병합 뒤
2026-09-25에 닫았다). 공통 규범 검사의 Engine 조합 층은 설계대로 다음 층으로 남아 있다. 새 전략을 문서에서 코드·등록·실행·화면·검산까지 옮기는 기반은 동작한다.
