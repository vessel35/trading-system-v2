...
| `config_hash` | 64자 고정 길이 16진 문자열 | 불가 | 없음 | 재현 입력 컬럼 전부(이 표의 `strategy_id`부터 `seed`·코드 버전까지 — `run_id`·`run_seq`·`run_name`·상태·경로·해시 자신·벽시계 시각은 제외)를 **Evidence 결정성 직렬화와 같은 형식 규칙**(컬럼 정의 순서·US 구분자·NUL 표기·SHA-256)으로 이어 낸 해시. Engine이 run 등록 전에 산출한다. **같은 값이면 같은 설정의 재실행**이라 스윕에서 중복 실행을 찾는 데 쓴다. 유일 제약은 걸지 않는다(의도적 재실행이 정상이다) |
