...
Evidence SQLite 두 가지뿐이고, 시장 데이터 `crypto_data`는 읽기 전용으로 쓰며, 신호 서비스 운영 데이터베이스
`signal_db`에서는 전략(Adaptee) 레지스트리로 쓸 **기존 빈 테이블 하나를 재정의**한다(새 테이블을 만들지 않는다).
...
않는다. `signal_db`에서는 전략 목록을 담을 기존 빈 테이블 하나를 재정의한다(새 테이블을 만들지 않는다). 전략 목록을 코드에서 데이터베이스로 옮기는 것은
...
| --------------- | ------------------------ | ----------------------------- | ------------------------------------- | ---------------------------- |
| `crypto_data`   | PostgreSQL + TimescaleDB | 기존 · **스키마 변경 없음, 보존 정책만 연장** | 확정 1분 OHLCV, 펀딩 실측 요율·마크 가격           | 읽기 전용. 쓰기 없음                 |
| `signal_db`     | PostgreSQL               | 기존 · 기존 테이블 하나 재정의(수정)        | 신호 운영 데이터 + 전략(Adaptee) 레지스트리         | 레지스트리 읽기 전용. 등록·수정은 신호 서비스 몫 |
...
쓰는 **펀딩 실측 요율·마크 가격**이다. 이 설계는 `crypto_data`를 만들지 않으므로 아래는 물리 스키마의 소유 선언이
...
해석·대사에 쓸 참조 마크는 캔들마다 필요하다(발동 판정 자체는 실행 계약이 last-price 캔들 극값 대조로 소유하며
...
데이터베이스라 여기 넣지 않고(§5.2), `crypto_data`는 스키마 변경 없이 보존 기간만 연장한다(§5.1.2).
...
- **추가**: 없음. 새 테이블을 만들지 않는다 — 기존 빈 테이블 하나를 재정의할 뿐이다.
...
- **혼동 주의(변경이 아님).** 이름이 비슷한 기존 `wallet_db.backtest_runs`는 실거래·페이퍼·백테스트를 한 테이블에서
  ...
  삭제하지도 않고 그대로 둔다.
- 공유 라이브러리 채택으로 실거래 wallet의 값 타입·계약이 바뀌면서 생길 수 있는 `wallet_db` 스키마 변경(예: 거래
...
- `config_hash`·`evidence_hash`에는 유일 제약을 걸지 않는다 — 같은 설정의 의도적 재실행이 정상이기 때문이다.
...
| `note` | 문자열 | 허용 | 없음 | 대체 펀딩 요율을 썼다는 등의 특기 사항 |
...
- run 시작 때, 사용한 원천 종류·주기 조합마다 한 행(전략 주기 캔들·1분 하위 캔들·펀딩·마크 가격).
...
- **마크 가격 규약.** 시장 데이터에는 마크 가격이 펀딩 정산 경계 시각에만 실측으로 있다. 강제청산의 발동 판정 자체는
  마크가 아니라 last-price 캔들 극값을 청산가와 대조해 내린다(실행 계약이 소유한 보수 방향 근사이며, 실측 마크가
...
기본키(엔티티 내 시퀀스)는 번호 부여가 결정적이라 빼지 않고 값으로 넣는다** — 위 산출 절차의 포함 규칙과 같다. 금액은 이미
...
| §5.3.3 마크 가격 규약과 출처 컬럼 | 강제청산 판정의 해석·대사 참조가 기록으로 남는다(실측과 대체를 구분. 발동 판정 자체는 실행 계약의 last-price 극값 대조) |
