...
설계서`(서비스·코드 트리·컴포넌트·클래스)는 이 스키마에 **쓰는 쪽**의 계약을 담고 있어 함께 읽으면 맥락이 서지만,
...
이 문서가 확정하는 것은 **네 저장소의 스키마**다. 새로 만드는 것은 카탈로그 메타 데이터베이스 `backtest_db`와 run별
Evidence SQLite 두 가지이고, 시장 데이터 `crypto_data`는 읽기 전용으로 쓰며, 신호 서비스 운영 데이터베이스
`signal_db`에는 전략(Adaptee) 레지스트리 테이블 하나를 더한다.
...
4. **Evidence 해시는 정규화 직렬화로 낸다.** 결정성 검증에 쓰는 해시는 SQLite 파일의 바이트가 아니라 정렬된 행의
   ...
   직렬화한다(이하 이 문서에서 "정해진 소수 자릿수로 반올림"은 값을 그 컬럼의 소수 자릿수에 맞춰 은행가 반올림하는
   것을 뜻한다 — 파이썬 십진 타입의 자릿수 맞춤 연산에 대응한다). 그래서
...
**새로 만드는 것은 최소로 한다.** `crypto_data`는 이미 시장 데이터를 적재하고 있으므로 읽기만 하고 스키마를 바꾸지
않는다. `signal_db`에는 전략 목록을 담을 테이블 하나만 더한다. 전략 목록을 코드에서 데이터베이스로 옮기는 것은
...
| 절 | 제목 | 담는 내용 |
|---|---|---|
| §5.1 | 데이터베이스 전체 구성 + `crypto_data`·`signal_db` | 네 저장소의 역할·접근 권한·경계 지도, 백테스트가 읽는 시장 데이터 테이블, `signal_db`에 더하는 전략 레지스트리 |
...
graph LR
    ...
    subgraph EXIST["기존 — 읽거나 최소로 더한다"]
        CD[("crypto_data<br/>시장 데이터 적재<br/>PostgreSQL + TimescaleDB")]
        SDB[("signal_db<br/>신호 운영 + 전략 레지스트리 추가<br/>PostgreSQL")]
...
|---|---|---|---|---|
| `crypto_data` | PostgreSQL + TimescaleDB | 기존 · **스키마 변경 없음, 보존 정책만 연장** | 확정 1분 OHLCV, 펀딩 실측 rate·마크 가격 | 읽기 전용. 쓰기 없음 |
| `signal_db` | PostgreSQL | 기존 · 테이블 하나 추가 | 신호 운영 데이터 + 전략(Adaptee) 레지스트리 | 레지스트리 읽기 전용. 등록·수정은 신호 서비스 몫 |
...
| `wallet_db` | PostgreSQL | 기존 · 이 설계의 범위 밖 | 실거래 체결·잔고 | 접근하지 않는다 |
...
쓰는 **펀딩 실측 rate·마크 가격**이다. 이 설계는 `crypto_data`를 만들지 않으므로 아래는 물리 스키마의 소유 선언이
아니라 **읽는 쪽이 의존하는 계약**이다 — 이 컬럼들이 이 의미로 존재해야 백테스트가 성립한다.
...
erDiagram
    ...
    FUNDING_RATES {
        ...
        varchar symbol PK "거래 심볼"
        varchar exchange PK "거래소"
        numeric funding_rate "그 경계의 실측 펀딩 rate"
...
| `timeframe` | 최대 10자. **적재되는 값은 `1m` 하나뿐** | 불가 | 없음 | 상위 주기는 이 컬럼의 다른 값이 아니라 아래 리샘플 규약으로 만든다 |
...
**전략 타임프레임 캔들을 만드는 방법 — 1분 베이스만 읽고 리샘플한다.** 이 저장소에는 5분~1일 상위 주기가
연속 집계 뷰로 함께 정의되어 있지만, **백테스트는 그 뷰를 읽지 않고 1분 베이스 테이블만 읽어 전략 타임프레임으로
직접 리샘플한다.** 세 가지 이유다. 첫째, 연속 집계 뷰의 자동 갱신 구간이 최근 1~30일로 좁아 과거 구간은 수동 갱신이
있어야 채워지므로, 뷰를 읽으면 **결과가 갱신 상태에 의존**해 같은 입력이 같은 결과를 낸다는 결정성이 깨진다. 둘째,
보존 기간 연장을 1분 베이스 테이블 하나에만 걸면 되어 보존 대상이 단순해진다. 셋째, 미완성 마지막 버킷을 버리는
규칙을 리샘플하는 한 곳에서 강제할 수 있다. 뷰 자체는 대시보드·실거래 조회용으로 남으며 폐지 대상이 아니다.

리샘플 규칙은 이렇다. 전략 주기 한 칸은 그 구간에 속한 1분 행 전부로 만들고, **1분 행이 하나라도 비면 그 칸은
...
— 이다.** 펀딩을 함께 늘리지 않으면 400일 이전 구간에는 실측 펀딩이 없어 장기 run이 전부 대체 rate로 떨어지고,
...
| `mark_price` | 십진 고정소수점, 정수부 12자리·소수부 8자리 | 허용 | 없음 | 그 경계 시각의 마크 가격. **강제청산 판정의 기준가**로 쓰며 펀딩 금액 산정에는 쓰지 않는다(펀딩 정산가는 경계를 포함하는 캔들의 시가를 쓴다). **정산 경계 시각에만 존재**하므로 캔들마다 필요한 청산 판정에는 그대로 쓸 수 없다(아래 규약) |
| `created_at` | 적재 시각 | 허용 | 현재 시각 | 판단·해시에 쓰지 않는다 |

**펀딩 rate를 소수 자릿수로 반올림하지 않는 이유.** 금액 정밀도 규약에는 소수 4자리로 깎는 자릿수가 둘 있다 —
수수료율 자릿수와 비율 자릿수다. 실측 펀딩 rate는 그보다 잔 자리를 갖는다(예: 0.00008750). 4자리로 뭉개면 실측값이
바뀌어 비용 차감 후 손익이 틀어지므로, **rate는 두 자릿수 어느 쪽으로도 깎지 않고 원천 정밀도로 읽으며, 소수 자릿수
...
rate를 쓰고, **대체값을 썼다는 사실을 자유 서술이 아니라 구조화된 값으로** 그 run의 Evidence에 남긴다(원천 스냅샷
엔티티의 대체 사용 플래그와 사용 건수). 자유 서술로만 남기면 "대체값이 섞인 run"을 판정에서 걸러 낼 수 없기
때문이다. 실측 없이 낸 비용 차감 후 손익은 스트레스 시나리오로만 취급한다.

**마크 가격을 캔들마다 쓰는 방법.** 이 테이블의 마크 가격은 정산 경계 시각에만 있는데, 강제청산 판정은 캔들마다
필요하다. 그래서 **실측이 있는 시각에는 그 값을, 없는 시각에는 그 캔들의 종가를 마크 가격으로 쓰고 어느 쪽인지를
Evidence에 출처로 남긴다**(시점별 포지션 엔티티의 마크 가격 출처 컬럼). 이 규약을 정하지 않으면 구현자마다 다른
값을 넣어 청산 여부가 갈리고, 갈린 청산은 그 뒤 모든 거래를 어긋나게 한다.

> **키 유일성.** 이 테이블은 `(time, symbol, exchange)`를 기본키로 선언하고 있어 한 시각·한 심볼·한 거래소에 행이
> 하나뿐임이 보장된다. 읽는 쪽은 이 유일성에 기대어 정산 경계마다 rate를 하나로 확정한다.
...
| `supported_timeframes` | 문자열 배열, 비어 있지 않음 | 불가 | `{1h}` | 이 전략이 지원하는 판단 주기. run 설정의 주기가 이 안에 없으면 거부한다 |
| `required_indicators_json` | JSON 배열. 각 원소는 지표 이름과 파라미터를 가진 객체 | 불가 | `[]` | 이 전략이 필요로 하는 지표. 지표 계산 대상을 `auto`로 둔 run이 계산 집합을 정하는 근거다 |
| `min_history` | 32비트 정수, 1 이상 | 불가 | `100` | 판단에 필요한 최소 이력 캔들 수. 워밍업 preload 길이를 정하는 두 입력 중 하나다(다른 하나는 지표의 최장 워밍업) |
...
외래키를 걸지 않는다 — 배치 테이블은 실거래 서비스가 소유·운영하는 기존 테이블이고, 이 설계는 레지스트리를 더할 뿐
...
| — | **추가** | `min_history` | 워밍업 preload 길이를 정하는 입력. 지금은 코드에만 있다 |
...
erDiagram
    ...
    BACKTEST_RUN {
        ...
        timestamptz warmup_start "워밍업 preload 시작"
...
| `run_seq` | 64비트 정수, 유일. 데이터베이스 시퀀스가 발급 | 불가 | 시퀀스 | `run_id`의 일련번호 부분. **유일성의 실제 근거**이며 병렬 스윕의 발급 경합을 막는다. 백만 번째 run부터는 6자리를 넘으므로 0을 채우지 않고 자릿수를 그대로 늘려 적는다(그래도 `run_id`는 유일하다) |
...
| `warmup_start` | 시간대 포함 시각(UTC) | 허용 | 없음 | 워밍업 preload를 시작한 실제 시각. 데이터 부족으로 늦춰졌으면 그 사실이 여기 남는다 |
| `warmup_candles` | 32비트 정수, 0 이상 | 불가 | `0` | preload에 쓴 캔들 수 |
| `data_source` | 최대 60자 | 불가 | 없음 | 원천 식별(예: `crypto_data.ohlcv_futures@1m→리샘플`) |
...
| `initial_capital` | 십진 고정소수점, 정수부 20자리·소수부 8자리. 0보다 큼 | 불가 | 없음 | 시작 자본 |
...
| `config_hash` | 64자 고정 길이 16진 문자열 | 불가 | 없음 | 위 입력들을 정규화 직렬화해 낸 해시. **같은 값이면 같은 설정의 재실행**이라 스윕에서 중복 실행을 찾는 데 쓴다. 유일 제약은 걸지 않는다(의도적 재실행이 정상이다) |
...
**상태 전이 규약.** `run_id` 발급과 함께 `RUNNING`으로 열린다. finalize에서 정규화 해시가 확정되면 `COMPLETED`가
되고, 판정 3단계까지 끝나면 `EVALUATED`가 된다. 실행 중 예외로 끝나면 `FAILED`이며 진단이 `error_message`에 남는다.
프로세스가 죽어 해시가 확정되지 못한 채 남은 run은 다음 기동 때 훑어 `ORPHANED`로 바꾸고, 이 run은 근거 부족으로
공식 평가에서 제외한다. 되돌아가는 전이는 없다.

**인덱스.** 검색·비교 진입점이므로 다섯 가지를 둔다. 전략·심볼·주기·구간 조합의 복합 인덱스
`(strategy_id, symbol, timeframe, period_start)`는 "이 전략을 이 구간에서 돌린 run" 조회용이고, `(status)`는 미완
run 훑기용, `(sweep_id)`는 스윕 묶음 조회용, `(config_hash)`는 같은 설정의 재실행 탐지용이다. `created_at` 내림차순
인덱스로 최근 run 목록을 받는다.
...
**행이 언제 생기는가.** 요약 행은 **finalize에서만 생긴다.** 실행 중 예외로 끝났거나 크래시로 해시가 확정되지 못한
run은 성과를 낼 수 없으므로 요약 행이 아예 없다. 그래서 run과 요약의 대응은 "정확히 하나"가 아니라 "0개 또는 1개"이며,
요약이 없는 run은 성과 비교에서 자동으로 빠진다. 빈 행을 미리 만들어 두지 않는 이유는 "성과가 0인 run"과 "성과를 낼 수
없었던 run"이 구분되어야 하기 때문이다.

성과 지표는 판단·평가 경로의 값이라 배정밀도 부동소수점으로, 실제로 돈이 오간 금액은 십진 고정소수점(소수부
8자리)으로 저장한다. 지표를 십진으로 저장하지 않는 이유는 이 값들이 통계량이지 장부 금액이 아니기 때문이고,
금액을 부동소수점으로 저장하지 않는 이유는 회계 항등식 검산이 끝자리에서 깨지기 때문이다.
...
| `annualization` | 최대 24자 | 불가 | `daily_resample_sqrt365` | 연율화 규약의 기록. **일간 리샘플 후 √365**가 표준이며, 하위 주기 수익률에 √365를 직접 적용하거나 √252를 섞어 쓰면 판정이 뒤집히므로 무엇으로 환산했는지를 남긴다 |
...
| `integrity_status` | 최대 24자. `passed`·`diagnostic_only`·`insufficient_evidence` 중 하나 | 불가 | `diagnostic_only` | 실패면 판정을 진행하지 않는다 |
| `integrity_failed_json` | JSON 배열 | 허용 | 없음 | 실패한 검사 이름들(회계 항등식·시점 순서·비용 1회 차감·비용 차감 후 손익·결정성·기록 완성도 중) |
...
**인덱스.** 순위·필터가 목적이므로 `(decision_route)`, `(gate_passed, pf)`, `(sortino)` 세 가지를 둔다. 스윕 결과에서
"통과한 run을 성과순으로" 뽑는 질의가 이 인덱스만으로 끝난다.

**여러 run에 걸친 증거는 누가 갖는가.** 표본 내외 분리·워크포워드·몬테카를로·확률적 샤프는 run 하나가 아니라 run
묶음의 산출물이다. 그런데 요약은 run마다 하나씩이므로 어디에 담을지를 정해야 한다. 규칙은 이렇다 — 같은 묶음
식별자를 가진 run 가운데 **표본 외 구간을 평가한 run**(fold 이름이 표본 외인 run, 없으면 묶음에서 가장 먼저 발급된
run)이 대표가 되어 그 요약에만 집합 증거를 담고, 나머지 run의 이 컬럼은 비운다. 모든 run에 같은 값을 복제하면 묶음
하나가 여러 번 통과한 것처럼 집계되기 때문이다. 표본 외 성능 저하와 확률적 샤프 두 스칼라 컬럼도 같은 규칙을 따른다.

**성과 지표가 비어 있을 수 있는 이유.** 거래가 없거나 표본이 통계량 성립 조건을 못 채우면(예: 거래 수 30 미만의
`sqn`) 값을 억지로 채우지 않고 비운다. 0으로 채우면 "성과가 0"과 "계산할 수 없음"이 구분되지 않아 집계·순위가
왜곡되기 때문이다.
...
**사전등록을 사전등록이게 하는 것은 시각 비교가 아니라 순서 규칙이다.** run은 번호를 먼저 발급받아야 파일 이름을
지을 수 있으므로 run 헤더가 사전등록보다 먼저 열린다. 그래서 "선언 시각이 run 시작 시각보다 이르다"를 제약으로 걸면
정상 흐름에서 항상 어긋난다. 대신 아래 두 규칙이 사후 합리화를 막는다.

- **사전등록은 첫 판단보다 먼저 잠긴다.** 캔들 루프가 첫 신호를 내기 전에 이 행이 저장되고 `locked_at`이 채워져야
  한다. 즉 이 run의 어떤 체결 기록도 존재하지 않는 시점에 잠금이 끝난다.
- **잠긴 뒤에는 고치지 않는다.** 기준을 바꾸고 싶으면 새 run을 열어 새로 선언한다. 같은 run의 사전등록을 고치는
  경로는 없다.
...
| 컬럼 | 제약 | 널 | 기본값 | 의미 |
|---|---|---|---|---|
| `tag_id` | 64비트 정수, 기본키. 데이터베이스가 자동 채번 | 불가 | 자동 증가 | 행 식별자. 의미를 담지 않는 대리 키다 |
...
같은 run에 같은 종류·같은 값의 라벨이 중복되지 않도록 `(run_id, tag_type, tag_value)`에 유일 제약을 둔다. 조회용으로
`(tag_type, tag_value)` 인덱스를 둬서 "이 약점을 가진 run 전부" 같은 질의를 받는다.
...
시각·자동 증가 행 번호·파일 경로는 직렬화에서 제외하며, 수치는 저장 정밀도의 소수 자릿수로 반올림한 표준형을 쓴다. 그래서 같은
입력·같은 seed로 두 번 돌리면 파일 바이트는 달라도 이 값은 같아야 한다. 값이 비어 있으면 아직 확정되지 않은
것이고, 그 상태로 남은 run은 다음 기동에서 `ORPHANED`가 된다.
...
- **조건 서명은 표준 어휘로 만든다.** `CONDITION_SIGNATURE`의 조건 정의는 run마다 제각각인 임의 축이 아니라, 미리
  ...
  문자열이 아니라 이 표준 어휘에서 고른다.
    - **서명은 전략과 무관하게 도출되어야 한다(이 규약의 전제).** "어느 전략의 run에서든 같은 키"가 성립하려면,
      ...
      대상**으로 남긴다 — 이 문서는 서명이 그 어휘로 만들어져 전략을 가로질러 안정적이어야 한다는 계약만 고정한다.
- **같은 데이터 위에서 비교한다.** 두 전략 run이 같은 구간·같은 원천을 봤는지는 `SOURCE_DATA_SNAPSHOT`의 내용
  해시로 확인한다. 한 run에는 원천 종류·주기마다 스냅샷 행이 여럿이므로(전략 주기 캔들·1분 하위 캔들·펀딩·마크),
...
> 부과됐는지, 어떤 rate로 부과됐는지, 실측이 없어 대체 rate를 썼는지가 복원되지 않으면 그 불변식의 검사가 성립하지
...
바꾸므로 그대로 두면 검사가 소리 없이 부정확해진다. **한 인자만 스케일 정수인 곱**(펀딩 부과액 `펀딩 명목가 × rate`,
여기서 rate는 스케일되지 않은 실수)은 결과가 이미 10⁸ 배라 되돌림이 필요 없다. 그래서 규칙은 이렇다.

- **두 스케일 정수의 곱**은 10⁸으로 나눠 스케일을 되돌리고 소수부 8자리로 은행가 반올림(끝자리가 5면 짝수 쪽으로)한다.
- **스케일 정수 하나에 실수를 곱한 값**(펀딩 부과액)은 이미 10⁸ 스케일이므로 되돌리지 않고 소수부 8자리로 은행가
...
1. **대상 선정.** 이 문서가 **컬럼으로 선언한** 모든 엔티티의 모든 행을 대상으로 하되, 아래 셋은 **직렬화에서
   뺀다** — 벽시계 시각 컬럼(정의서에서 그렇게 밝힌 것들), 파일 경로, 그리고 사후 주석층인 발견과 주장
   (`FINDING_CLAIM`) 엔티티 **전체**. 앞의 둘은 같은 입력으로 다시 돌려도 값이 달라지기 때문이고, 마지막은
...
2. **정렬.** 엔티티 이름의 사전순으로 엔티티를 정렬하고, 각 엔티티 안에서는 아래 표의 정렬 키를 순서대로 적용한다.
   ...
   > 같은 입력이면 같은 번호가 나온다. 즉 **채번은 결정적**이므로 빼야 할 이유가 없고, 오히려 빼면 그 번호를 값으로
...
**인덱스.** 무결성 검사와 분석 질의가 전체 훑기가 되지 않도록 아래를 둔다. 인덱스는 데이터가 아니므로 해시 대상이
아니며, 따라서 인덱스를 더하거나 빼도 결정성 해시는 변하지 않는다.

| 엔티티 | 인덱스 | 무엇을 위한 것인가 |
|---|---|---|
| `INDICATOR_SNAPSHOT` | `(indicator_key, feature_ts)`, `(feature_ts)` | 판단 근거 재구성, 시점 순서 검사의 조인 |
| `SIGNAL`·`DECISION` | `(decision_ts)` | 시점 순서 검사, 구간 조회 |
| `EXECUTION` | `(execution_ts)`, `(decision_id)` | 시점 순서 검사, 판단↔체결 조인 |
| `FUNDING_SETTLEMENT` | `(settled_at)`, `(trade_id)` | 비용 1회 차감 검사 |
| `TRADE` | `(entry_time)`, `(exit_reason)` | 성과 산출, 결과 유형 집계 |
| `POSITION`·`PORTFOLIO_PNL` | `(ts)` | 회계 항등식 검사의 시각 조인 |
| `OUTCOME_BUCKET` | `(bucket_name, bucket_value)` | 결과 유형 집계 |
| `TRADE_FEATURE_SNAPSHOT` | `(trade_id, phase)` (유일) | 거래별 스냅샷 조회 |
...
erDiagram
    ...
    SIGNAL {
        ...
        text reason
        text metadata_json
        text derived_intent "enter · exit · reverse · hold"
    ...
    DECISION {
        ...
        integer decision_ts
        text action "enter · exit · reverse · hold · skip"
        text skip_reason
        text intended_side
        real intended_qty
        real stop_price
...
| `warmup_start` | 정수 (epoch ms, UTC) | 허용 | 없음 | 워밍업 preload 시작 시각 |
| `warmup_candles` | 정수, 0 이상 | 불가 | `0` | preload 캔들 수 |
...
| `prereg_json` | JSON 객체 문자열 | 허용 | 없음 | 사전등록 사본(가설·주 지표·성공/실패 기준·선언 시각). 최종 라우팅이 무엇과 대조해 나온 판정인지를 파일 안에서 확인하기 위한 것이다 |
| `evidence_schema_version` | 문자열 | 불가 | 없음 | 이 파일의 스키마 버전. 나중에 엔티티가 늘어도 옛 파일을 읽을 수 있게 한다 |
...
- 원천 종류마다 한 행씩 남긴다(전략 주기 캔들, 1분 하위 캔들, 펀딩, 마크 가격).

| 컬럼 | 제약 | 널 | 기본값 | 의미 |
|---|---|---|---|---|
| `snapshot_id` | 정수 기본키(엔티티 내 시퀀스) | 불가 | 자동 증가 | |
| `run_id` | `BACKTEST_RUN_LOCAL` 외래키 | 불가 | 없음 | |
| `source_kind` | `ohlcv`·`funding`·`mark_price` 중 하나 | 불가 | 없음 | 원천 종류. **캔들 원천은 주기가 다르면 다른 행**이므로(전략 주기 캔들과 1분 하위 캔들), 1분 트리거를 켠 run은 `ohlcv` 행이 둘이다. 완성도 검사의 "원천 종류마다 한 행"은 종류와 주기의 조합마다 한 행을 뜻한다 |
...
| `resampled_from` | 문자열 | 허용 | 없음 | 리샘플로 만들었으면 그 원천 주기(`1m`). 원천 그대로면 비운다 |
| `range_start`·`range_end` | 정수 (epoch ms, UTC) | 불가 | 없음 | 실제로 읽어 쓴 구간. 워밍업 preload를 포함하므로 평가 구간보다 앞설 수 있다 |
| `row_count` | 정수, 0 이상 | 불가 | 없음 | 읽은 행 수 |
| `gap_count` | 정수, 0 이상 | 불가 | `0` | **채우지 않고 표시만 한 결측 구간 수.** 0이 아니면 그 사실이 판정 해석에 들어가야 한다 |
...
| `note` | 문자열 | 허용 | 없음 | 대체 펀딩 rate를 썼다는 등의 특기 사항 |
...
| `min_history` | 정수, 1 이상 | 불가 | 없음 | 이 지표의 워밍업 길이. preload 길이를 정한 근거 |
...
| `derived_intent` | `enter`·`exit`·`reverse`·`hold` 중 하나 | 불가 | 없음 | 엔진이 위 규칙으로 읽어 낸 의도 |
| `derived_side` | `LONG`·`SHORT` 중 하나 | 허용 | 없음 | 보호 수준의 기하로 도출한 방향. 청산·관망이면 비운다 |
...
| `action` | `enter`·`exit`·`reverse`·`hold`·`skip` 중 하나 | 불가 | 없음 | 결정한 행동 |
| `skip_reason` | 문자열. `action`이 `skip`일 때만 채운다 | 허용 | 없음 | 건너뛴 이유(노출 한도 초과·마진 부족·워밍업 구간·보유 포지션 없음 등) |
...
| `run_id` | `BACKTEST_RUN_LOCAL` 외래키 | 불가 | 없음 | |
| `decision_id` | `DECISION` 외래키 | 허용 | 없음 | 어느 판단에서 나온 체결인지 |
| `order_id` | 문자열 | 불가 | 없음 | 주문 식별자 |
| `execution_ts` | 정수 (epoch ms, UTC) | 불가 | 없음 | **체결 시각.** 시점 순서 검증의 오른쪽 항이며, 대응하는 판단의 `decision_ts`보다 반드시 나중이다 |
| `trigger_subcandle_ts` | 정수 (epoch ms, UTC) | 허용 | 없음 | 캔들 내 트리거로 체결됐다면 발동한 1분 하위 캔들의 시각. 신호 체결이면 비운다 |
| `symbol` | 문자열 | 불가 | 없음 | 대상 |
...
| `exit_reason` | `STOP_LOSS`·`TAKE_PROFIT`·`TRAILING_STOP`·`LIQUIDATION`·`SIGNAL_EXIT`·`REVERSAL`·`END_OF_DATA` 중 하나 | 허용 | 없음 | 청산 체결이면 그 사유. 진입 체결이면 비운다 |
...
확정된 체결가와 기준가의 차이에 수량을 곱해 장부에 달 슬리피지를 낸다. 모델이 낸 금액을 그대로 장부에 달면 가격
가격을 소수 자릿수로 반올림하는 과정에서 끝자리가 어긋나 위 관계가 깨지고, 그러면 정상 run이 비용 1회 차감 검사에서 떨어진다. 모델의 산출은
체결가를 만드는 입력으로만 쓴다.
...
| 컬럼 | 제약 | 널 | 기본값 | 의미 |
|---|---|---|---|---|
| `settlement_id` | 정수 기본키(엔티티 내 시퀀스) | 불가 | 자동 증가 | |
| `run_id` | `BACKTEST_RUN_LOCAL` 외래키 | 불가 | 없음 | |
...
| `funding_rate` | 실수 | 불가 | 없음 | 부과에 쓴 rate. **소수 4자리로 깎지 않고 배정밀도 한도까지 보존한다**(십진으로 완전히 정확해지는 것은 아니지만, 소수 4자리로 깎아 실측을 뭉개는 것과는 다르다) |
| `rate_source` | `measured`·`fallback` 중 하나 | 불가 | `measured` | 실측을 썼는지 대체값을 썼는지. **어느 거래가 대체값의 영향을 받았는지 여기서 가려진다** |
| `settle_price` | 스케일 정수, 0 초과 | 불가 | 없음 | **정산가.** 정산 경계를 포함하는 가장 작은 가용 주기 캔들의 **시가**를 쓰고, 그런 캔들이 없으면 직전 확정 캔들의 **종가**를 쓴다. 마크 가격이 아니다 — 마크 가격은 이 설계에서 강제청산 판정의 기준가이며 정산 경계 시각에만 존재한다 |
...
| `payment_amount` | 스케일 정수. **부호 있음** | 불가 | 없음 | 부과액. 지급이면 음수, 수취면 양수다. **펀딩 명목가(스케일 정수)에 rate(스케일되지 않은 실수)를 곱한** 값이라 결과가 이미 10⁸ 스케일이므로, 곱셈 규약 중 "되돌림 없이 8자리 은행가 반올림만" 갈래를 따른다(되돌림을 적용하면 10⁸배 작아진다). 부호는 포지션 방향과 rate 부호로 정한다 |

같은 거래에 같은 경계가 두 번 부과되지 않도록 `(trade_id, settled_at)`에 유일 제약을 둔다.

**경계를 세는 방법.** 부과 대상은 **경계 직전까지 보유하고 있던 포지션**이다. 경계와 같은 시각에 새로 체결된
포지션은 그 경계에서 물지 않고 다음 경계부터 문다. 청산이 경계와 같은 시각이면 그 경계까지는 보유한 것으로 보아
부과한다. 이 규칙이 있어야 "보유 구간에 포함된 경계 수"가 한 가지로 세어진다.

비용 1회 차감 검사가 보는 두 규칙은 이렇다 — **위 방법으로 센 경계 수와 그 거래의 부과 행 수가 같아야 하고**,
부과액 합의 부호를 뒤집은 값이 그 거래의 펀딩 비용과 일치해야 한다.

**행이 언제 생기는가.** 캔들 루프가 정산 경계를 지날 때마다 보유 중인 포지션 하나당 한 행이 생긴다. 기록 주기가
전략 주기 캔들이 아니라 **정산 경계**라는 점에서 다른 시계열 엔티티와 다르다.
...
| `exit_execution_id` | `EXECUTION` 외래키 | 허용 | 없음 | 청산 체결. 평가 구간이 끝나도 열려 있는 포지션이면 비운다 |
...
| `funding_cost` | 스케일 정수. **부호 있음** | 불가 | `0` | 펀딩 순비용. **음수일 수 있다** — 방향과 rate 부호에 따라 펀딩을 받는 구간이 있고, 그때는 비용이 아니라 수입이다. 0 이상으로 묶으면 그런 run이 기록되지 못한다. 정산 사건 행들의 부과액 합에 부호를 뒤집은 값(`funding_cost = −Σ payment_amount`)과 일치해야 한다 |
...
**사본 규칙.** 진입·청산의 가격·수량·시각·사유는 체결 엔티티에 있는 값의 **사본**이다. 집계 질의가 매번 조인하지
않아도 되게 두었을 뿐이므로, 두 값이 어긋나면 **언제나 체결 엔티티가 사실**이다. 기록 완성도 검사가 이 일치를
확인한다.

**부분 청산은 이 스코프에서 다루지 않는다.** 거래가 청산 체결을 하나만 가리키므로 나눠서 빠지는 청산은 표현되지
않는다. 첫 검증 스코프의 전략들이 전량 진입·전량 청산만 하기 때문이며, 부분 청산을 도입하려면 거래와 체결을
일대다로 바꿔야 한다. 그때가 오면 스키마를 고칠 지점이 여기라는 것을 명시해 둔다.
...
| 컬럼 | 제약 | 널 | 기본값 | 의미 |
|---|---|---|---|---|
| `position_seq` | 정수 기본키(엔티티 내 시퀀스) | 불가 | 자동 증가 | |
| `run_id` | `BACKTEST_RUN_LOCAL` 외래키 | 불가 | 없음 | |
...
| `mark_price` | 스케일 정수, 0 초과 | 불가 | 없음 | 강제청산 판정의 기준가. 아래 마크 가격 규약이 정한 값이다 |
| `mark_price_source` | `measured`·`candle_close` 중 하나 | 불가 | `candle_close` | 그 마크 가격이 실측인지 캔들 종가로 대체한 값인지. **출처를 남기지 않으면 청산 판정이 무엇에 근거했는지 복원되지 않는다** |
...
**마크 가격 규약.** 시장 데이터에는 마크 가격이 펀딩 정산 경계 시각에만 실측으로 있다. 그런데 강제청산 판정은
캔들마다 필요하므로, 실측이 없는 시각에는 **그 캔들의 종가를 마크 가격으로 쓰고 출처를 `candle_close`로 남긴다.**
경계 시각에 실측이 있으면 그 값을 쓰고 `measured`로 남긴다. 이 규약을 명시하지 않으면 구현자마다 다른 값을 넣어
청산 여부가 갈리고, 갈린 청산은 그 뒤 모든 거래를 어긋나게 한다. 실측이 아닌 값으로 낸 청산 판정은 출처 컬럼으로
가려낼 수 있으므로, 청산이 결과를 좌우한 run은 그 사실을 확인하고 해석한다.

**포지션과 자산곡선은 같은 시각 격자에 기록된다.** 둘 다 전략 주기 확정 캔들마다 한 행이므로 시각으로 조인해
회계 항등식을 검산할 수 있다. 격자가 어긋나면 검사 자체가 불가능해지므로 이 규칙은 선택이 아니다.
...
**표준 분류 축 `outcome_class`의 여섯 값.** 원인 분석은 이 여섯 갈래를 축으로 삼는다. 이름과 뜻을 고정하는 이유는
run마다 다른 이름을 붙이면 run 사이 비교가 불가능해지기 때문이다.
...
같은 대상에 같은 분류 축이 두 번 붙지 않도록 `(subject_kind, subject_id, bucket_name)`에 유일 제약을 둔다.
`subject_id`는 분류 대상이 신호냐 거래냐 놓친 기회냐에 따라 가리키는 엔티티가 달라지는 다형 참조라 **외래키를 걸 수
없다.**
파일 안의 참조는 전부 외래키로 강제한다는 규칙의 유일한 예외이며, 대신 대상 행이 실제로 존재하는지를 기록 완성도
검사가 확인한다.
...
| `timestamp_order` | 신호마다 `feature_ts ≤ decision_ts`이고, 그 판단에서 나온 체결마다 `decision_ts < execution_ts`인가. 신호에서 온 판단이면 `feature_ts`가 그 판단이 소비한 지표 스냅샷들의 시각 중 최댓값과 일치하는가. 지표 스냅샷마다 `candle_close_time ≤ feature_ts`인가. **신호 없이 트리거로 일어난 판단**에는 지표 시각 항이 없으므로 `decision_ts < execution_ts`와 "트리거 하위 캔들 시각이 판단 시각 이후"만 본다 |
...
자체에 비결정적 요소가 있다는 뜻이라 즉시 실패다). 그다음 카탈로그에 **같은 설정 해시를 가진 이전 run이 있으면 그
Evidence 해시와 비교**한다. 비교 대상이 없으면 검사는 통과로 남기되 상세에 "비교 대상 없음"을 적어, 나중에 같은
설정이 다시 돌 때 대조할 수 있게 한다.
...
- 평가 구간의 모든 전략 주기 확정 캔들마다 지표 값·포지션·자산곡선 행이 있다(무포지션 구간 포함).
- 거래에 사본으로 담긴 진입·청산의 가격·수량·시각·사유가 체결 엔티티의 값과 일치한다.
- 결과 유형 분류의 대상 행이 실제로 존재한다(다형 참조라 외래키가 없는 자리를 여기서 확인한다).
- 원천 스냅샷이 사용한 원천 종류마다 한 행씩 있다.

검사마다 한 행씩 남으므로 `(run_id, check_name)`에 유일 제약을 둔다. 행 수는 **정확히 여섯**이어야 하고, 트리거
세밀도가 1분 하위 캔들이면서 트레일링을 소비하는 전략이 활성인 run만 파리티가 더해져 **정확히 일곱**이다. 그보다
적으면 검사를 돌리지 않은 것이고 많으면 정의되지 않은 검사가 섞인 것이라, 어느 쪽이든 기록 완성도 위반이다.
...
이 엔티티는 **원본이 아니라 파생**이다. 값이 원본 시계열과 어긋나면 언제나 원본이 옳으며, finalize에서 다시 만든다.
...
같은 거래의 같은 순간이 중복 기록되지 않도록 `(trade_id, phase)`에 유일 제약을 둔다.
...
무한히 많은 놓친 기회를 갖게 되어 개선 방향이 사후편향으로 오염된다. 그래서 행은 **run 전에 선언해 둔 규칙에서만**
생성되며, 어느 규칙이 만들었는지를 널 불가 컬럼으로 남겨 이 제약이 기록으로 확인되게 한다. 규칙이 선언되지 않은
run에서는 이 엔티티가 비어 있는 것이 정상이다.
...
- **이 파일에서 유일하게 finalize 이후에 적히는 엔티티다.** 쓰는 주체도 백테스트 실행이 아니라 분석·개선
  시스템(외부)이다. 그래서 결정성 해시 대상에서 엔티티 전체가 제외된다(§5.3.1 기록 주체 규약) — 실행의 결정성은
  실행이 만든 기록으로만 판정하고, 사후 해석은 그 위에 얹히는 주석이다.
...
다른 사람에게 그대로 건네도 맥락이 따라간다는 뜻이며, 이것이 무거운 상세를 파일로 분리한 이유이기도 하다.
...
결과를 해싱한다. 직렬화에서 빼는 것은 벽시계 시각 컬럼(로컬 사본의 생성 시각, 무결성 검사 시각),
**선언하지 않은 암묵 rowid**(텍스트 기본키 테이블이 내부적으로 갖는 자동 행 번호), 파일 경로, 그리고 사후 주석층인
발견과 주장(`FINDING_CLAIM`) 엔티티 전체다(§5.3.1 기록 주체 규약·해시 산출 절차). **선언된 정수
기본키(엔티티 내 시퀀스)는 결정적 채번이라 빼지 않고 값으로 넣는다** — 위 산출 절차의 포함 규칙과 같다. 금액은 이미
소수부 8자리 스케일 정수로 저장되어 있으므로 직렬화가 표현을 바꾸지 않는다 — 부동소수점으로 담았다면 이 단계에서
표현이 흔들려 같은 입력이 다른 해시를 냈을 것이다.
...
**해시를 언제의 상태로 내는가.** 결정성 검사 행 자체도 해시 대상이라 순서를 못 박지 않으면 자기참조가 된다. finalize는
이 순서로 진행한다 — 먼저 시점별 상세와 차트 요약을 다 기록하고, 그다음 무결성 검사 여섯(조건부 일곱)을 돌려 그 결과
행을 기록하며(결정성 검사도 이때 자기 판정을 위해 직렬화를 두 번 산출해 비교한다), 마지막으로 **모든 검사 행까지
포함한 완결 상태를 한 번 직렬화해 최종 해시를 낸다.** 두 번 산출 비교와 최종 해시가 같은 완결 상태를 보므로
자기참조가 풀린다. 이 시점 이후로 **백테스트 실행이** 파일에 행을 더하는 일은 없다. 유일한 예외는 사후 주석층인
발견과 주장이며(분석·개선 시스템이 적는다, §5.3.1 기록 주체 규약), 그 엔티티는 해시 대상에서 통째로 빠져 있어
사후에 더해져도 저장된 해시와의 재검증이 깨지지 않는다.

### §5.3.7 쓰는 쪽 계약과의 정합

이 절의 스키마가 요구하는 값은 모두 쓰는 쪽 계약(값 타입과 기록 어댑터)에 대응 자리가 있다. 이 데이터베이스 설계가
새로 요구한 세 가지는 클래스 설계에 다음과 같이 반영되어 있다.
...
| 정산가와 그 대체 규칙 | 펀딩 비용을 산출하는 쪽(비용 계층의 펀딩 정산)이 정산가를 낸다. 경계를 포함하는 최소 가용 주기 캔들의 시가를 쓰고, 그런 캔들이 없으면 직전 확정 캔들 종가를 쓰며(출처를 `settle_price_source`로 남김), 이 대체 규칙은 데이터 갭 경계 사례의 마무리이지 정산가 계약의 재결정이 아니다 |
...
| 제약사항·방향 2, §5.1.2 확정 캔들·리샘플 규칙 | look-ahead 구조적 배제의 데이터 층 근거(확정 캔들만 적재, 미완성 버킷 폐기, 결측은 채우지 않고 표시) |
| 제약사항·방향 3, §5.2.2 `run_id`·`run_seq`, §5.2.6 발급 규약 | run 식별자 단독 발급(시퀀스가 유일 발급처, 파일명과 일치, 병렬 스윕의 발급 경합·파일명 충돌 차단) |
| 제약사항·방향 4, §5.3.1 저장 규약, §5.3.6 해시 대상 | 결정성(정렬된 행의 정규화 직렬화 해시, 벽시계·내부 행 번호·경로 제외, 같은 입력·같은 seed → 같은 해시) |
...
| §5.3.3 마크 가격 규약과 출처 컬럼 | 강제청산 판정의 근거가 기록으로 복원된다(실측과 대체를 구분) |
...
| §5.3.7 쓰는 쪽 계약 정합 | 설계 부채를 다음 단계로 조용히 넘기지 않는다(스키마가 요구한 세 항목이 값 타입·기록 어댑터에 대응 자리를 얻어 정합함을 확인) |
...
| §5.2.2 버전·seed·설정 해시 컬럼 | 재현 가능성(같은 입력·같은 seed·같은 코드 버전이면 같은 결과라는 보증의 조건을 전부 기록) |
| §5.2.3 통과선 컬럼과 판정 컬럼 | 판정 3단계(무결성 검사 → 통과선·형태 대조 → 최종 라우팅)와 그 결과의 카탈로그 보존, 미달은 종료가 아니라 원인 분석 경로 |
| §5.2.3 연율화 규약 컬럼 | 성과 수식 표준 하나와 연율화 규약(일간 리샘플 후 √365)의 기록 |
