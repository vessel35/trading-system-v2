> **폐기됨: 계산 근거가 아니다. 이 문서는 TA-Lib v0.7.1 기준으로 대체되기 전의 폐기된 전제와 수작업 판정 규칙 기록이다.**

# 캔들스틱 패턴 판정 명세서

> **범위**: TA-Lib v0.7.1이 `CDL` 접두사로 제공하는 캔들스틱 패턴 61종의 **판정 방법**을
> 플랫폼 독립적으로 기술한다. 2026-08-03 사용자가 전제를 확정했으므로 TA-Lib의 계산 값이
> 원본이다. 어긋나면 우리 문서나 구현이 틀린 것이다. TA-Lib은 오픈 소스이므로 원본은 추측
> 대상이 아니라 `v0.7.1` 태그의 C 소스다.
...
TA-Lib v0.7.1은 `BodyLong`, `BodyShort`, `Near`, `Far`, `Equal` 등 열한 개의 자체 임계표를
갖고 있으며, 직접 이식은 그 표와 `TA_CANDLERANGE`, `TA_CANDLEAVERAGE`를 그대로 승계한다.
이전 판은 이 표를 승계하지 않는다고 썼으나 2026-08-03 사용자 확정으로 뒤집혔다. TA-Lib이
오픈 소스라 원본 소스가 확인 가능하고, 포획값도 같은 0.7.1 판에서 나왔기 때문이다.
...
| 결정 | 내용 |
|---|---|
| A | 직접 이식의 수치와 임계는 TA-Lib v0.7.1 소스가 정본이다. 원전 기반 수치와 **우리 규약** 표시는 legacy 네 키 구현 설명으로만 남긴다 |
| B | 직접 이식은 각 `TA_CDL*` 소스가 구현한 추세 사용 여부를 그대로 따른다. 10기간 EMA 추세 판정은 legacy 네 키 구현의 계약이다 |
| C | TA-Lib v0.7.1 소스와 원전 기반 해석이 충돌하면 직접 이식에서는 TA-Lib을 따른다. 좁고 엄격한 쪽 선택은 legacy 네 키 구현 설명으로만 남긴다 |
| D | 직접 이식의 갭 판정은 각 `TA_CDL*` 소스의 실체·그림자 비교를 그대로 따른다. 기존 갭 정의 선택은 legacy 네 키 구현 설명으로만 남긴다 |
| F | 출력은 **성립·방향·강도·확인** 네 키다 |
| G | 패턴 이름은 지표 이름과 겹치지 않는다 |

결정 F는 기존 등록 패턴의 네 키 출력 계약이다. 직접 이식은 먼저 TA-Lib과 같은 단일 원시 정수
계열을 내고, 그 정수를 네 키로 옮기는 어댑터는 별도 단계에서 정의한다. 이 분리는 Hikkake의
확인 정수와 방향 없음 패턴의 부호처럼 원시 정수 의미가 네 키에 곧장 들어맞지 않는 경우를
섞어 검증하지 않기 위한 것이다.
...
- **워밍업이 퇴화 봉보다 앞선다. 【우리 규약】** 위의 두 규약이 한 봉에서 함께 걸릴 수 있다.
  워밍업 구간 안에 `Range = 0`인 봉이 있는 경우다. 첫 항목만 읽으면 불성립 `0.0`이고 NaN
  항목만 읽으면 `NaN`이라 순서를 정해야 한다. **이 표준은 워밍업을 먼저 적용해 `NaN`을 낸다.**
  근거는 두 값의 뜻에 있다. `0.0`은 판정을 수행했고 성립하지 않았다는 뜻인데, 워밍업 구간에서는
  창이 아직 차지 않아 판정 자체를 수행할 수 없다. 퇴화 봉을 이유로 `0.0`을 내면 하지 않은 판정을
  했다고 말하는 것이 되고, §5.3이 워밍업과 불성립을 갈라 두는 목적이 무너진다.
...
**개정 적용.** 위 문단은 legacy 네 키 출력의 강도 계약이다. 직접 TA-Lib 이식은 먼저 원시
정수 값을 보존한다. `CDLENGULFING` 소스 주석은 `100`을 둘째 봉 실체가 첫째 봉 실체를 양끝에서
감싼 경우, `80`을 한쪽 끝이 일치하는 경우로 설명하며, 포획값 실측도 legacy
`pat_engulfing_strength`와 1:1로 맞는다. `pat_engulfing`이 성립한 봉에서 강도 `0.5`인 169봉은
모두 TA-Lib 크기 `80`이고, 강도 `1.0`인 597봉은 모두 크기 `100`이다. 따라서 네 키 어댑터는 이
대응을 버리면 안 된다. 다만 Hikkake의 `±200`처럼 원시 정수에 확인 사건이 실리는 경우가 있으므로
모든 패턴의 원시 크기와 네 키 강도 매핑은 별도 어댑터 단계에서 패턴별로 고정한다.
...
**개정 적용.** 이 절의 아래 공식은 legacy 네 키 구현의 `min_history` 설명이다. 직접 TA-Lib
원시 정수 포트는 각 `TA_CDL*_Lookback` 값을 따른다. 입력과 같은 길이의 단일 정수 계열을 내고,
`lookback` 이전 워밍업 prefix는 TA-Lib 배열과 같이 `0`으로 둔다. 첫 의미 있는 출력 위치는
index `lookback`이며, 그 첫 출력을 계산하는 데 필요한 최소 확정 캔들 수는 `lookback + 1`이다.
...
5. `Black_{t−1}` **그리고** `ShortBody(t−1)` **그리고** `C_{t−1} < C_{t−2}` **그리고**
   ...
   저가 아래로는 내려가지 않는다.** 위쪽은 묶지 않는다. 규칙 3이 첫 실체 위로 갭 상승을
   요구하므로 이 구간의 봉들이 첫 양봉의 고가 위에 있는 것이 정상이기 때문이다.
...
> 번 성립하지 않게 하기 위한 규약이다. **다만 이 규약이 실제로 발동하는 입력은 없다.**
> `n` 형태의 첫날은 `n + 1` 형태에서 가운데 작은 캔들 자리에 놓이는데, 앞은 §2.1의 긴 실체를
> 뒤는 §2.2의 짧은 실체를 요구하므로 한 봉이 둘을 함께 만족할 수 없기 때문이다. 규약을 그대로
> 두는 것은 구현이 어느 순서로 시도해야 하는지를 정해 두기 위해서다.
...
3. **확인.** 강세 설정이면 이후 어느 봉의 가격이 **인사이드 바의 고가 위로** 올라설 때,
   ...
   **【우리 규약】 여기서 "가격"은 그 봉의 고가(강세 설정)와 저가(약세 설정)다.** 원전이 네
   가격 가운데 무엇인지 밝히지 않았다. 종가로 읽지 않는 근거는 둘이다. 첫째, Chesler의 기법이
   인사이드 바 바깥에 스톱을 두는 것이어서 봉 중간에 닿기만 해도 걸린다. 둘째, §5.5가 Hikkake
   두 종을 종가 규약에서 명시적으로 빼고 원전이 준 조건을 쓰라고 정했으므로, 여기서 종가를
   쓰면 그 배제가 무의미해진다. 이 규약은 §7.5.10에도 그대로 적용된다.
...
**TA-Lib v0.7.1은 직접 이식의 출처다.** 이전 판은 TA-Lib을 함수 목록과 대조군으로만 보았으나,
2026-08-03 사용자 확정으로 전제를 뒤집었다. TA-Lib은 오픈 소스이고 `v0.7.1` 태그의 C 소스를
읽을 수 있으므로 원본 판정 규칙과 임계값은 추측 대상이 아니다. 위 세 편과 이 표준의 원전 기반
해석은 legacy 네 키 구현과 최종 어댑터 정책을 설명하는 보조 맥락으로 남긴다.
...
이 절의 이전 문장은 "TA-Lib과 값이 어긋나는 것이 정상이고 TA-Lib을 따라 바꾸지 않는다"는
방침이었다. 2026-08-03 사용자가 반대 전제를 확정했으므로 그 방침을 폐기한다. TA-Lib v0.7.1은
오픈 소스이고 포획값도 같은 판에서 나왔으므로, 직접 이식의 대조는 원인 분류가 아니라 봉 단위
동등성 단언이다.

직접 이식의 출력 계약은 TA-Lib과 같은 **단일 원시 정수 계열**이다. 각 `TA_CDL*` 포트는 입력
캔들 수와 같은 길이의 정수 배열을 내며, 워밍업 구간과 불성립은 모두 `0`으로 표현한다. 워밍업은
별도 `lookback` 값으로 구분한다. 첫 의미 있는 출력은 index `lookback`에 있고, 그 값을 계산하는
데 필요한 최소 이력은 `lookback + 1`이다.

값이 어긋나면 우리 원시 포트, 포획 fixture, 또는 판 고정이 틀린 것이다. 네 키 출력은 이 원시
정수 계열 위에 놓일 어댑터의 책임이며, Hikkake 확인 정수와 Engulfing의 `80`/`100` 크기처럼
패턴별 의미를 보존해 따로 검증한다.
