...
echo "- TOP-DOWN, UML-FIRST STRUCTURE: every design doc leads with 제약사항·방향 (constraints + direction only), then descends service diagram·정의서 -> component diagram·정의서 (per service) -> class diagram·정의서 (per component) -> sequence/flow (inside the class definition). Shared components/classes in their own 공통 section. DB entities as ER diagrams. ALL UML in mermaid (classDiagram/sequenceDiagram/flowchart/erDiagram)."
echo "- UML-FIRST (default representation): express the design IN the mermaid diagram; use prose ONLY for what UML cannot encode. Attributes+types, method/port signatures, class relationships+cardinality, stereotypes, ER fields+keys, and sequence/flow order go INSIDE the diagram. Prose supplements ONLY the residue: constraints, defaults, nullability, formulas (expr·units·bounds), thresholds+tuning source, enforced invariants, semantics, service/component responsibility, and rationale. TWO PROHIBITIONS: do NOT restate in prose what the diagram already shows (no attr/method/entity table duplicating the diagram); do NOT hide in prose structure the diagram could show. Diagram + residual prose = the full self-contained contract, no overlap, no gap. Big structure BEFORE detail; the reader never jumps to another doc/chapter. B1 is the entry doc (service diagram + component map + reading map). Full standard: skills/backtest-v2-design/references/design-doc-standard.md."
